...
struct DPE_CLK_STRUCT {
	...
	struct clk *CLK_CAM_LARBX;
	struct clk *CLK_CAM_GALS;
	struct clk *CLK_CAM_CAMTG;
	struct clk *CLK_CAM_MRAW0;
	struct clk *CLK_CAM_MRAW1;
	struct clk *CLK_CAM_MRAW2;
	struct clk *CLK_CAM_MRAW3;
	struct clk *CLK_CAM_PDA0;
	struct clk *CLK_CAM_PDA1;
	struct clk *IPE_LARB19;
	struct clk *CLK_CAMSYS_DPE;
	struct clk *CLK_CAMSYS_FUS;
	struct clk *CLK_CAMSYS_DHZE;
	struct clk *CLK_CAMSYS_CLAS;
};
...
struct DPE_REQUEST_STRUCT {
	...
	struct DPE_Config_V2 DpeFrameConfig[_SUPPORT_MAX_DPE_FRAME_REQUEST_];
};
...
static struct MRAW_device *MRAW_devs;

//
#define CAMSYS_MAIN_BASE (CAM_devs[0].regs)
#define CAMMRAW_MAIN_BASE (MRAW_devs[0].regs)
...
#define CAMMRAW_REG_CG_CON              (CAMMRAW_MAIN_BASE + 0x0)
...
#define DVS_FIX_HIST_ADR_HW          (DPE_BASE_HW + 0x1A0)
...
#define DVP_EXT_SRC_24_WMF_RD_DV_ADR_HW  (DPE_BASE_HW + 0xBE0)
...
#define DVS_FIX_HIST_ADR_REG          (ISP_DPE_BASE + 0x1A0)
...
#define DVP_EXT_SRC_24_WMF_RD_DV_ADR_REG  (ISP_DPE_BASE + 0xBE0)
...
//		mutex_unlock(&gFDMutex);
		...
		if (success) {
			...
			get_dvs_iova[0] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_InBuf_SrcImg_Y_L iova = %llu, get_dvs_iova[0] = %d\n",
...
//		mutex_unlock(&gFDMutex);
		...
		if (success) {
			...
			get_dvs_iova[1] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_InBuf_SrcImg_Y_R iova = %llu, get_dvs_iova[1] = %d\n",
...
//		mutex_unlock(&gFDMutex);
		...
		if (success) {
			...
			get_dvs_iova[2] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_OutBuf_OCC = %llu, get_dvs_iova[4] = %d\n",
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_OCC, get_dvs_iova[2]);
				LOG_INF("=========================================\n");
			}
			#ifdef IOVA_TO_PA
...
//		mutex_unlock(&gFDMutex);
		...
		if (success) {
			...
			get_dvs_iova[3] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_OutBuf_OCC_Ext = %llu, get_dvs_iova[5] = %d\n",
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_OCC_Ext, get_dvs_iova[3]);
				LOG_INF("=========================================\n");
			}
		} else {
...
//		mutex_unlock(&gFDMutex);
		...
		if (success) {
			...
			(sg_dma_address(OutBuf_OCC_Hist_mmu[DVS_only_en-1].sgt->sgl) +
			(_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_Output_OCC_Hist_Ofs));
			get_dvs_iova[4] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_OutBuf_OCC_Hist = %llu, get_dvs_iova[6] = %d\n",
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_OCC_Hist, get_dvs_iova[4]);
				LOG_INF("=========================================\n");
			}
		} else {
...
//		mutex_unlock(&gFDMutex);
		...
		if (DPE_P4_EN == 1) {
			...
			if (success) {
				...
				get_dvs_iova[5] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_InBuf_SrcImg_Y_L_Pre = %llu iova[6]=%d\n",
					_req->m_pDpeConfig[ucnt].Dpe_InBuf_SrcImg_Y_L_Pre,
					get_dvs_iova[5]);
					LOG_INF("====================================\n");
				}
...
//				mutex_unlock(&gFDMutex);
			...
			if (success) {
				...
				get_dvs_iova[6] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_InBuf_SrcImg_Y_R_Pre = %llu iova[7]= %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_InBuf_SrcImg_Y_R_Pre,
					get_dvs_iova[6]);
					LOG_INF("===========================\n");
				}
...
//				mutex_unlock(&gFDMutex);
			...
			if (success) {
				...
				get_dvs_iova[7] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_InBuf_P4_L_DV = %llu iova[8]= %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_InBuf_P4_L_DV,
					get_dvs_iova[7]);
					LOG_INF("===========================\n");
				}
			...
			if (success) {
				...
				get_dvs_iova[8] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_InBuf_P4_R_DV = %llu iova[9] = %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_InBuf_P4_R_DV,
					get_dvs_iova[8]);
					LOG_INF("===================\n");
				}
	...
	if ((_req->m_pDpeConfig[ucnt].Dpe_engineSelect == MODE_DVP_ONLY) ||
		...
		if ((DVP_only_en == 0) && (DVP_Num == 0)) {
			...
			ASF_RM_mmu = kzalloc(sizeof(struct tee_mmu) * 4, GFP_KERNEL);
			...
			ASF_RM_Ext_mmu = kzalloc(sizeof(struct tee_mmu) * 4, GFP_KERNEL);
		...
		if (success) {
			...
			get_dvp_iova[0] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_InBuf_SrcImg_Y = %llu iova[0] = %d\n",
		...
		if (success) {
			...
			get_dvp_iova[2] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_InBuf_OCC = %llu iova[2] = %d\n",
		...
		if (success) {
			...
			get_dvp_iova[3] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_OutBuf_CRM = %llu iova[3] = %d\n",
		...
		//spin_lock(&(DPEInfo.SpinLockFD));
			...
			//mutex_lock(&gFDMutex);
			success = dpe_get_dma_buffer(&ASF_RM_mmu[DVP_only_en-1],
			_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_ASF_RM_fd);
			if (success) {
		...
		if (WMF_RD_EN == 1) {
			...
			//mutex_lock(&gFDMutex);
			success = dpe_get_dma_buffer(&WMF_RD_mmu[DVP_only_en-1],
			_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_WMF_RD_fd);
			if (success) {
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_WMF_RD =
				(sg_dma_address(WMF_RD_mmu[DVP_only_en-1].sgt->sgl) +
				(_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_WMF_RD_Ofs));
				get_dvp_iova[4] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_OutBuf_ASF_RD = %llu iova[4] = %d\n",
		...
		if (success) {
			...
			get_dvp_iova[5] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_OutBuf_ASF_HF = %llu iova[5] = %d\n",
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_ASF_HF,
				get_dvp_iova[5]);
				LOG_INF("=========================================\n");
			}
		...
		if (DVP_16bitMode == 0) {
			...
			if (success) {
				...
				get_dvp_iova[6] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_OutBuf_WMF_FILT = %llu iova[6] = %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_OutBuf_WMF_FILT,
					get_dvp_iova[6]);
					LOG_INF("==========================================\n");
				}
		...
		if (((DVP_16bitMode == 1) && (DVP_ASF_CONF_EN == 0)) ||
			...
			_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_ASF_RM_Ext_Ofs);

			//mutex_lock(&gFDMutex);
			success = dpe_get_dma_buffer(&ASF_RM_Ext_mmu[DVP_only_en-1],
			_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_ASF_RM_Ext_fd);
			LOG_INF("Dpe_OutBuf_ASF_RM_Ext 1 = %d\n", success);
			if (success) {
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_ASF_RM_Ext =
				(sg_dma_address(ASF_RM_Ext_mmu[DVP_only_en-1].sgt->sgl) +
				(_req->m_pDpeConfig[
				ucnt].DPE_DMapSettings.Dpe_OutBuf_ASF_RM_Ext_Ofs));
				LOG_INF("Dpe_OutBuf_ASF_RM_Ext 2 = %d\n", success);
				get_dvp_iova[12] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_OutBuf_ASF_RM_Ext = %llu iova[8] = %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_OutBuf_ASF_RM_Ext,
					get_dvp_iova[12]);
					LOG_INF("=======================================\n");
				}
			...
			if (success) {
				...
				get_dvp_iova[8] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_OutBuf_ASF_RD_Ext = %llu iova[8] = %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_OutBuf_ASF_RD_Ext,
					get_dvp_iova[8]);
					LOG_INF("=======================================\n");
				}
			...
			if (success) {
				...
				get_dvp_iova[9] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_OutBuf_ASF_HF_Ext = %llu iova[9] = %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_OutBuf_ASF_HF_Ext,
					get_dvp_iova[9]);
					LOG_INF("=============================================\n");
				}
	...
	if (_req->m_pDpeConfig[ucnt].Dpe_engineSelect == MODE_DVGF_ONLY) {
		...
		success = dpe_get_dma_buffer(&DVGF_SrcImg_Y_mmu[DVGF_only_en-1],
		_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_InBuf_SrcImg_Y_fd);
		if (success) {
			_req->m_pDpeConfig[ucnt].Dpe_InBuf_SrcImg_Y =
			(sg_dma_address(DVGF_SrcImg_Y_mmu[DVGF_only_en-1].sgt->sgl) +
			(_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_InBuf_SrcImg_Y_Ofs));
			get_dvgf_iova[0] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_InBuf_SrcImg_Y = %llu iova[0] = %d\n",
		...
		success = dpe_get_dma_buffer(&DVGF_SrcImg_C_mmu[DVGF_only_en-1],
		_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_InBuf_SrcImg_C_fd);
		if (success) {
			_req->m_pDpeConfig[ucnt].Dpe_InBuf_SrcImg_C =
			(sg_dma_address(DVGF_SrcImg_C_mmu[DVGF_only_en-1].sgt->sgl) +
			(_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_InBuf_SrcImg_C_Ofs));
			get_dvgf_iova[1] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("SrcImg_C = %llu iova[1] = %d\n",
			...
			if (success) {
				...
				sg_dma_address(SrcImg_Y_Pre_mmu[DVGF_only_en-1].sgt->sgl) +
				_req->m_pDpeConfig[0].DPE_DMapSettings.Dpe_InBuf_SrcImg_Y_Pre_Ofs;
				get_dvgf_iova[2] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_InBuf_SrcImg_Y_Pre = %llu iova[6]=%d\n",
					_req->m_pDpeConfig[ucnt].Dpe_InBuf_SrcImg_Y_Pre,
					get_dvgf_iova[2]);
					LOG_INF("====================================\n");
				}
			...
			if (success) {
				...
				sg_dma_address(SrcImg_C_Pre_mmu[DVGF_only_en-1].sgt->sgl) +
				_req->m_pDpeConfig[0].DPE_DMapSettings.Dpe_InBuf_SrcImg_C_Pre_Ofs;
				get_dvgf_iova[3] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_InBuf_SrcImg_C_Pre = %llu iova[7]= %d\n",
					_req->m_pDpeConfig[ucnt].Dpe_InBuf_SrcImg_C_Pre,
					get_dvgf_iova[3]);
					LOG_INF("===========================\n");
				}
		...
		//mutex_lock(&gFDMutex);
		success = dpe_get_dma_buffer(&DVGF_OutBuf_OCC_Ext_mmu[DVGF_only_en-1],
		_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_OCC_Ext_fd);
		if (success) {
			_req->m_pDpeConfig[ucnt].Dpe_OutBuf_OCC_Ext =
			(sg_dma_address(DVGF_OutBuf_OCC_Ext_mmu[DVGF_only_en-1].sgt->sgl) +
			(_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_OCC_Ext_Ofs));
			get_dvgf_iova[4] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_OutBuf_OCC_Ext = %llu, get_dvs_iova[5] = %d\n",
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_OCC_Ext, get_dvs_iova[4]);
				LOG_INF("=========================================\n");
			}
		} else {
			LOG_INF("get Dpe_OutBuf_OCC_Ext fail\n");
		...
		//WMT_Fnl
		if (DPE_debug_log_en == 1) {
			LOG_INF("OutBuf_WMT_Fnl fd= %x ,offset = %x\n",
		...
		success = dpe_get_dma_buffer(&WT_Fnl_mmu[DVGF_only_en-1],
		_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_WT_Fnl_fd);
		if (success) {
			_req->m_pDpeConfig[ucnt].Dpe_OutBuf_WT_Fnl =
			sg_dma_address(WT_Fnl_mmu[DVGF_only_en-1].sgt->sgl) +
			_req->m_pDpeConfig[0].DPE_DMapSettings.Dpe_OutBuf_WT_Fnl_Ofs;
			get_dvgf_iova[6] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("WT_Fnl_mmu = %llu iova[6]=%d\n",
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_WT_Fnl,
				get_dvgf_iova[6]);
				LOG_INF("====================================\n");
			}
		...
		success = dpe_get_dma_buffer(&RW_IIR_mmu[DVGF_only_en-1],
		_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_RW_IIR_fd);
		if (success) {
			_req->m_pDpeConfig[ucnt].Dpe_OutBuf_RW_IIR =
			sg_dma_address(RW_IIR_mmu[DVGF_only_en-1].sgt->sgl) +
			_req->m_pDpeConfig[0].DPE_DMapSettings.Dpe_OutBuf_RW_IIR_Ofs;
			get_dvgf_iova[7] += 1;
			if (DPE_debug_log_en == 1) {
				LOG_INF("Dpe_OutBuf_RW_IIR = %llu iova[6]=%d\n",
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_RW_IIR,
				get_dvgf_iova[7]);
				LOG_INF("====================================\n");
			}
		...
		_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_WMF_FILT_Ofs;
			//mutex_lock(&gFDMutex);
			success = dpe_get_dma_buffer(&DVGF_WMF_FILT_mmu[DVGF_only_en-1],
			_req->m_pDpeConfig[ucnt].DPE_DMapSettings.Dpe_OutBuf_WMF_FILT_fd);
			if (success) {
				_req->m_pDpeConfig[ucnt].Dpe_OutBuf_WMF_FILT =
				(sg_dma_address(DVGF_WMF_FILT_mmu[DVGF_only_en-1].sgt->sgl) +
				WMF_FILT_Ofs);
				get_dvgf_iova[8] += 1;
				if (DPE_debug_log_en == 1) {
					LOG_INF("Dpe_OutBuf_WMF_FILT = %llu iova[6] = %d\n",
	...
	ucnt++;
		...
		//LOG_ERR("[%s] request queued with 11 frame(%d)", __func__, f);
		//LOG_INF("[%s] DPE_CTRL_REG:0x%x!\n", __func__,
	//						pDpeConfig->DPE_CTRL);
...
signed int dpe_deque_cb(struct frame *frames, void *req)
{
	...
	if (frames == NULL || _req == NULL)
		return -1;
	if (DPE_debug_log_en == 1)
		LOG_INF("dpe_deque start2\n");

	/*TODO: m_ReqNum is FrmNum; FIFO only thus f starts from 0 */
	ucnt = 0;
	fcnt = _req->m_ReqNum;
	for (f = 0; f < fcnt; f++) {
		pDpeConfig = (struct DPE_Config_V2 *) frames[f].data;
...
//		mutex_unlock(&gFDMutex);
	...
	if (pDpeConfig->Dpe_engineSelect == MODE_DVGF_ONLY) {
		...
		if (get_dvgf_iova[1] >= 1) {
			get_dvgf_iova[1]--;
			memcpy(&temp_dvgf, &DVGF_SrcImg_C_mmu[i], sizeof(struct tee_mmu));
			mmu_release(&temp_dvgf, 1);
			if (DPE_debug_log_en == 1)
		...
		if (get_dvgf_iova[2] >= 1) {
			get_dvgf_iova[2]--;
			memcpy(&temp_dvgf, &SrcImg_Y_Pre_mmu[i], sizeof(struct tee_mmu));
			mmu_release(&temp_dvgf, 2);
			if (DPE_debug_log_en == 1)
				LOG_INF("dpe_deque SrcImg_Y_Pre_mmu put fd\n");
		...
		if (get_dvgf_iova[3] >= 1) {
			get_dvgf_iova[3]--;
			memcpy(&temp_dvgf, &SrcImg_C_Pre_mmu[i], sizeof(struct tee_mmu));
			mmu_release(&temp_dvgf, 3);
			if (DPE_debug_log_en == 1)
		...
		if (get_dvgf_iova[4] >= 1) {
			get_dvgf_iova[4]--;
			memcpy(&temp_dvgf, &DVGF_OutBuf_OCC_Ext_mmu[i], sizeof(struct tee_mmu));
			mmu_release(&temp_dvgf, 4);
			if (DPE_debug_log_en == 1)
		...
		if (get_dvgf_iova[6] >= 1) {
			get_dvgf_iova[6]--;
			memcpy(&temp_dvgf, &WT_Fnl_mmu[i], sizeof(struct tee_mmu));
			mmu_release(&temp_dvgf, 6);
			if (DPE_debug_log_en == 1)
		...
		if (get_dvgf_iova[7] >= 1) {
			get_dvgf_iova[7]--;
			memcpy(&temp_dvgf, &RW_IIR_mmu[i], sizeof(struct tee_mmu));
			mmu_release(&temp_dvgf, 7);
			if (DPE_debug_log_en == 1)
		...
		if (get_dvgf_iova[8] >= 1) {
			get_dvgf_iova[8]--;
			memcpy(&temp_dvgf, &DVGF_WMF_FILT_mmu[i], sizeof(struct tee_mmu));
			mmu_release(&temp_dvgf, 8);
			if (DPE_debug_log_en == 1)
...
void DPE_Config_DVS(struct DPE_Config_V2 *pDpeConfig,
	struct DPE_Kernel_Config *pConfigToKernel)
{
	...
	LOG_INF(
	"DVS param: frm w/h(%d/%d), engStart X_L/X_R/Y(%d/%d/%d), eng w/h(%d/%d), occ w(%d), occ startX(%d), pitch(%d), main eye(%d), 16bit mode(%d), sbf/conf/occ_en(%d/%d/%d), Dpe_InBuf_SrcImg_Y_L: (%llu), Dpe_InBuf_SrcImg_Y_R(%llu), Dpe_OutBuf_CONF(%llu), Dpe_OutBuf_OCC(%llu)\n",
	...
	//pConfigToKernel->DVS_CTRL00 = 0xD0002371;
	...
	if (DPE_P4_EN == 1) {
		...
		pDpeConfig->Dpe_InBuf_P4_R_DV;
		#else
		if (pDpeConfig->DVS_SRC_26_P4_R_DV_ADR != 0x0) {
			pConfigToKernel->DVS_SRC_26_P4_R_DV_ADR =
			pDpeConfig->DVS_SRC_26_P4_R_DV_ADR;
		} else
...
void DPE_Config_DVP(struct DPE_Config_V2 *pDpeConfig,
	struct DPE_Kernel_Config *pConfigToKernel)
{
	...
	LOG_INF(
	"DVP param: frm w/h(%d/%d), engstart X/Y(%d/%d), eng w/h(%d/%d), occ w/h(%d/%d), occstart X/Y(%d/%d), pitch(%d), eng start offset Y/C(0x%x/0x%x), Dpe_InBuf_SrcImg_Y: (%llu), Dpe_InBuf_SrcImg_C(%llu), Dpe_InBuf_OCC(%llu), Dpe_OutBuf_CRM(%llu), Dpe_OutBuf_ASF_RD(%llu), Dpe_OutBuf_ASF_HF(%llu), DVP_SRC_18_ASF_RMDV(%lu)\n",
	...
	if (DVP_WMF_RD_EN == 1) {
		if (pDpeConfig->Dpe_OutBuf_WMF_RD != 0x0) {
			...
			LOG_ERR("No WMF_RD Output Buffer!\n");
	...
	if (pDpeConfig->Dpe_is16BitMode == 0) { //for WMF
		...
		if (pDpeConfig->DVP_SRC_25_WMF_HFDV != 0x0) {
			pConfigToKernel->DVP_SRC_25_WMF_HFDV =
			pDpeConfig->DVP_SRC_25_WMF_HFDV;
		} else
	...
	if (((pDpeConfig->Dpe_is16BitMode == 1) && (DVP_ASF_CONF_EN == 0)) ||
		...
		LOG_INF("Dpe_is16BitMode = %d ,DVP_ASF_CONF_EN =%d\n",
		pDpeConfig->Dpe_is16BitMode, DVP_ASF_CONF_EN);
		spin_lock(&(DPEInfo.SpinLockFD));
		DVP_is16BitMode = 1;
		spin_unlock(&(DPEInfo.SpinLockFD));
...
void DPE_Config_DVGF(struct DPE_Config_V2 *pDpeConfig,
	struct DPE_Kernel_Config *pConfigToKernel)
{
	...
	pDpeConfig->Dpe_DVGFSettings.TuningBuf_CORE.DVGF_DRAM_PITCH;

	pConfigToKernel->DVGF_DRAM_SEC_0 =
	pDpeConfig->Dpe_DVGFSettings.TuningBuf_CORE.DVGF_DRAM_SEC_0;

	pConfigToKernel->DVGF_DRAM_SEC_1 =
	pDpeConfig->Dpe_DVGFSettings.TuningBuf_CORE.DVGF_DRAM_SEC_1;

	pConfigToKernel->DVGF_CTRL_RESERVED =
	pDpeConfig->Dpe_DVGFSettings.TuningBuf_CORE.DVGF_CTRL_RESERVED;
...
void DPE_DumptoHWReg(struct DPE_Kernel_Config *pConfigToKernel)
{
	...
	LOG_INF("[0x%08X %08X]\n", (unsigned int)(DVS_FIX_HIST_ADR_HW),
	pConfigToKernel->DVS_FIX_HIST_ADR);
...
void DPE_DumpUserSpaceReg(struct DPE_Kernel_Config *pDpeConfig)
{
	...
	LOG_INF("DVS_SRC_21 = %llu\n", pDpeConfig->DVS_SRC_21_P4_L_DV_ADR);
	//LOG_INF("DVS_SRC_26 = 0x%08X\n", pDpeConfig->DVS_SRC_22_OCCDV0);
	//LOG_INF("DVS_SRC_27 = 0x%08X\n", pDpeConfig->DVS_SRC_23_OCCDV1);
	//LOG_INF("DVS_SRC_28 = 0x%08X\n", pDpeConfig->DVS_SRC_24_OCCDV2);
	//LOG_INF("DVS_SRC_29 = 0x%08X\n", pDpeConfig->DVS_SRC_25_OCCDV3);
	//LOG_INF("DVS_SRC_34 = 0x%08X\n", pDpeConfig->DVS_SRC_34_P4_R_DV_ADR);
	//LOG_INF("DVS_SRC_17 = 0x%08X\n", pDpeConfig->DVS_SRC_17_OCCDV_EXT0);
	//LOG_INF("DVS_SRC_18 = 0x%08X\n", pDpeConfig->DVS_SRC_18_OCCDV_EXT1);
	//LOG_INF("DVS_SRC_19 = 0x%08X\n", pDpeConfig->DVS_SRC_19_OCCDV_EXT2);
	//LOG_INF("DVS_SRC_20 = 0x%08X\n", pDpeConfig->DVS_SRC_20_OCCDV_EXT3);
	//LOG_INF("DVS_SRC_46 = 0x%08X\n", pDpeConfig->DVS_SRC_46);
	//LOG_INF("DVS_DRAM_SEC_0 = 0x%08X\n", pDpeConfig->DVS_DRAM_SEC_0);
  ...
  //
	...
	//LOG_INF("DVS_ME_40 = 0x%08X\n", pDpeConfig->DVS_ME_42);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_43);
	//LOG_INF("DVS_ME_40 = 0x%08X\n", pDpeConfig->DVS_ME_44);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_45);
	//LOG_INF("DVS_ME_40 = 0x%08X\n", pDpeConfig->DVS_ME_46);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_47);
	//LOG_INF("DVS_ME_40 = 0x%08X\n", pDpeConfig->DVS_ME_48);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_49);
	//LOG_INF("DVS_ME_40 = 0x%08X\n", pDpeConfig->DVS_ME_50);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_51);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_52);
	//LOG_INF("DVS_ME_40 = 0x%08X\n", pDpeConfig->DVS_ME_53);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_54);
	//LOG_INF("DVS_ME_40 = 0x%08X\n", pDpeConfig->DVS_ME_55);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_56);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_57);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_58);
	//LOG_INF("DVS_ME_41 = 0x%08X\n", pDpeConfig->DVS_ME_59);
  //
...
signed int CmdqDPEHW(struct frame *frame)
{
	...
	struct DPE_Config_V2 *pDpeUserConfig;
...
/************** Pass User info to DPE_Kernel_Config **************/
  // LOG_INF("%s CmdqtoHw statr2111", __func__);
	if (pDpeUserConfig->Dpe_engineSelect == MODE_DVS_DVP_BOTH) {
		// LOG_INF("%s CmdqtoHw statr3333", __func__);
		DPE_Config_DVS(pDpeUserConfig, pDpeConfig);
			// LOG_INF("%s CmdqtoHw statr31111", __func__);
		DPE_Config_DVP(pDpeUserConfig, pDpeConfig);
			// LOG_INF("%s CmdqtoHw statr32222", __func__);
		...
		DPE_Config_DVGF(pDpeUserConfig, pDpeConfig);
		pDpeConfig->DPE_MODE = 3;
	}

	if (DPE_debug_log_en == 1) {
		LOG_INF("%s CmdqtoHw statr4444", __func__);
...
cmdq_pkt_write(handle, dpe_clt_base, DVS_CTRL00_HW, 0x00000000, 0x20000000);
	...
	LOG_INF("cmd_pkt start\n");
		...
		LOG_INF("cmd_pkt[0x1A170000 %08X]\n",
		(unsigned int)DPE_RD32(CAMMRAW_MAIN_BASE));
		LOG_INF("cmd_pkt[0x1A1D0000 %08X]\n",
...
signed int dpe_feedback(struct frame *frame)
{
	...
	pDpeConfig = (struct DPE_Config_V2 *) frame->data;
...
#endif
unsigned int Compute_Para(struct DPE_Config_V2 *pDpeConfig,
...
void Get_Tile_Info(struct DPE_Config_V2 *pDpeConfig)
{
...
static inline void DPE_Prepare_Enable_ccf_clock(void)
{
	...
	ret = clk_prepare_enable(dpe_clk.CLK_CAM_LARBX);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_LARBX clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_GALS);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_GALS clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_CAMTG);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_CAMTG clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_MRAW0);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_MRAW0 clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_MRAW1);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_MRAW1 clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_MRAW2);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_MRAW2 clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_MRAW3);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_MRAW3 clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_PDA0);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_PDA0 clock\n");

	ret = clk_prepare_enable(dpe_clk.CLK_CAM_PDA1);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAM_PDA1 clock\n");
	...
	ret = clk_prepare_enable(dpe_clk.CLK_CAMSYS_CLAS);
	if (ret)
		LOG_INF("cannot prepare and enable CLK_CAMSYS_CLAS clock\n");
...
static long DPE_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	static struct DPE_Config_V2 dpe_DpeConfig;
	...
	case DPE_ENQUE_REQ:
		{
			if (copy_from_user(&dpe_DpeReq, (void *)Param,
				...
				if (kDpeReq.m_pDpeConfig->Dpe_engineSelect == MODE_DVGF_ONLY) {
					req_temp = dpe_request_running_isp7s(&dpe_reqs_dvgf);
					LOG_INF("[DPE ioctl]dpe_request_running_isp7s star = %d\n",
					req_temp);
					if (!req_temp) {
						LOG_INF("[DPE ioctl DVP]direct request_handler\n");
	...
	case DPE_DEQUE_REQ:
		{
			if (copy_from_user(&dpe_DpeReq, (void *)Param,
				...
				if ((kDpeReq.m_pDpeConfig->Dpe_engineSelect == MODE_DVGF_ONLY) ||
					(kDpeReq.m_pDpeConfig->Dpe_engineSelect == 0))
					dpe_deque_request_isp7s(&dpe_reqs_dvgf,
					&kDpeReq.m_ReqNum, &kDpeReq);
...
//		mutex_unlock(&(MutexDPERef));
		...
		/*  */
		//open dvs
		dpe_register_requests_isp7s(&dpe_reqs_dvs, sizeof(struct DPE_Config_V2));
		dpe_set_engine_ops_isp7s(&dpe_reqs_dvs, &dpe_ops);
		//open dvp
		dpe_register_requests_isp7s(&dpe_reqs_dvp, sizeof(struct DPE_Config_V2));
...
static int vidioc_qbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	static struct DPE_Config_V2 cfgs[3];//[MAX_FRAMES_PER_REQUEST];
	struct DPE_Config_V2 *pcfgs;
	//unsigned long flags;
	...
	if (cfgs[0].Dpe_engineSelect == MODE_DVGF_ONLY)
		dpe_enque_request_isp7s(&dpe_reqs_dvgf, kreq.m_ReqNum, &kreq, pUserInfo->Pid);
	...
	if ((cfgs[0].Dpe_engineSelect == MODE_DVP_ONLY) ||
		...
		if (cfgs[0].Dpe_engineSelect == MODE_DVGF_ONLY) {
			temp_req = dpe_request_running_isp7s(&dpe_reqs_dvgf);
			if (DPE_debug_log_en == 1)
				LOG_INF("[vidioc qbuf]dpe_request_running_isp7s stat = %d\n",
				temp_req);
...
static int vidioc_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	//LOG_INF("[%s]buf address/len = 0x%llu/0x%x, ureq =0x%x\n",
	...
	if (Ret == 0) {
		...
		if (cfgs[0].Dpe_engineSelect == MODE_DVGF_ONLY)
			dpe_deque_request_isp7s(&dpe_reqs_dvgf, &kreq.m_ReqNum, &kreq);
...
///
		...
		dpe_clk.CLK_CAM_GALS = devm_clk_get(&pDev->dev,
							"CLK_CAM_GALS");
		if (IS_ERR(dpe_clk.CLK_CAM_GALS))
			LOG_INF("cannot get CLK_CAM_GALS clock\n");


		dpe_clk.CLK_CAM_CAMTG = devm_clk_get(&pDev->dev,
							"CLK_CAM_CAMTG");
		if (IS_ERR(dpe_clk.CLK_CAM_CAMTG))
			LOG_INF("cannot get CLK_CAM_CAMTG clock\n");


		dpe_clk.CLK_CAM_MRAW0 = devm_clk_get(&pDev->dev,
							"CLK_CAM_MRAW0");
		if (IS_ERR(dpe_clk.CLK_CAM_MRAW0))
			LOG_INF("cannot get CLK_CAM_MRAW0 clock\n");


		dpe_clk.CLK_CAM_MRAW1 = devm_clk_get(&pDev->dev,
							"CLK_CAM_MRAW1");
		if (IS_ERR(dpe_clk.CLK_CAM_MRAW1))
			LOG_INF("cannot get CLK_CAM_MRAW1 clock\n");


		dpe_clk.CLK_CAM_MRAW2 = devm_clk_get(&pDev->dev,
							"CLK_CAM_MRAW2");
		if (IS_ERR(dpe_clk.CLK_CAM_MRAW2))
			LOG_INF("cannot get CLK_CAM_MRAW2 clock\n");


		dpe_clk.CLK_CAM_MRAW3 = devm_clk_get(&pDev->dev,
							"CLK_CAM_MRAW3");
		if (IS_ERR(dpe_clk.CLK_CAM_MRAW3))
			LOG_INF("cannot get CLK_CAM_MRAW3 clock\n");


		dpe_clk.CLK_CAM_PDA0 = devm_clk_get(&pDev->dev,
							"CLK_CAM_PDA0");
		if (IS_ERR(dpe_clk.CLK_CAM_PDA0))
			LOG_INF("cannot get CLK_CAM_PDA0 clock\n");


		dpe_clk.CLK_CAM_PDA1 = devm_clk_get(&pDev->dev,
							"CLK_CAM_PDA1");
		if (IS_ERR(dpe_clk.CLK_CAM_PDA1))
			LOG_INF("cannot get CLK_CAM_PDA1 clock\n");


		dpe_clk.IPE_LARB19 = devm_clk_get(&pDev->dev,
							"IPE_LARB19");
		if (IS_ERR(dpe_clk.IPE_LARB19))
			LOG_INF("cannot get IPE_LARB19 clock\n");
		...
		if (IS_ERR(dpe_clk.CLK_CAMSYS_DPE))
			LOG_INF("cannot get CLK_CAMSYS_DPE clock\n");


		dpe_clk.CLK_CAMSYS_FUS = devm_clk_get(&pDev->dev,
							"CLK_CAMSYS_FUS");
		if (IS_ERR(dpe_clk.CLK_CAMSYS_FUS))
			LOG_INF("cannot get CLK_CAMSYS_FUS clock\n");


		dpe_clk.CLK_CAMSYS_DHZE = devm_clk_get(&pDev->dev,
							"CLK_CAMSYS_DHZE");
		if (IS_ERR(dpe_clk.CLK_CAMSYS_DHZE))
			LOG_INF("cannot get CLK_CAMSYS_DHZE clock\n");


		dpe_clk.CLK_CAMSYS_CLAS = devm_clk_get(&pDev->dev,
							"CLK_CAMSYS_CLAS");
		if (IS_ERR(dpe_clk.CLK_CAMSYS_CLAS))
			LOG_INF("cannot get CLK_CAMSYS_CLAS clock\n");
	...
	//LOG_INF("g_dpewb_dvme_int_Buffer_pa = %llu\n",
