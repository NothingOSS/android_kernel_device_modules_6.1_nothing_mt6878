...
void venc_dump_frame_data(struct mtk_vcodec_ctx *ctx, unsigned long frm_va)
{
	...
	char *pbuf;
	struct mtk_video_enc_buf *frm_info;
	...
	src_vb2_v4l2 = NULL;
	if (frm_va != 0 && virt_addr_valid(frm_va)) {
		pfrm = (struct venc_frm_buf *)frm_va;
		...
		if (src_vb2_v4l2 != NULL) {
			...
			pfrm->fb_addr[0].va = vb2_plane_vaddr(src_buf, 0) +
				(size_t)src_buf->planes[0].data_offset;
		}

		if (pfrm->fb_addr[0].va != NULL) {
			mtk_v4l2_debug(0,"venc dump frm_buf %d VA=%p PA=%llx Size=%zx =>",
			pfrm->index,
			pfrm->fb_addr[0].va,
...
static int vidioc_venc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_ENCODE_QPVBR:
		...
		p->qpvbr_enable = ctrl->p_new.p_s32[0];
		p->qpvbr_qpthreshold = ctrl->p_new.p_s32[1];
		p->qpvbr_qpbrratio = ctrl->p_new.p_s32[2];
		break;
			       ...
			       struct venc_enc_param *param)
{
	...
	param->qpvbr_enable = enc_params->qpvbr_enable;
	param->qpvbr_qpthreshold = enc_params->qpvbr_qpthreshold;
	param->qpvbr_qpbrratio = enc_params->qpvbr_qpbrratio;
			    ...
			    struct v4l2_buffer *buf)
{
	...
	if (buf->flags & V4L2_BUF_FLAG_HAS_META &&
		...
		ret = dma_buf_vmap(mtkbuf->frm_buf.metabuffer_dma, &meta_map);
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	ctx->enc_params.qpvbr_enable = -1;
	ctx->enc_params.qpvbr_qpthreshold = -1;
	ctx->enc_params.qpvbr_qpbrratio  = -1;
