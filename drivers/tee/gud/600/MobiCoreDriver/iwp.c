...
static inline void tee_uuid_to_mcuuid(const struct teec_uuid *in,
				      struct mc_uuid_t *out)
{
	struct teec_uuid tmp;

	memcpy((void *)&tmp, in, sizeof(struct teec_uuid));

#ifndef __BIG_ENDIAN
	/* now byte-swap the endian specific parts of the UUID */
	/* example UUID in device endian: efbeadde-adde-efbe-... */
	/* translates to network endian:  deadbeef-dead-beef-... */
	/* efbeadde -> deadbeef */
	tmp.time_low = __builtin_bswap32(tmp.time_low);
	/* adde -> dead */
	tmp.time_mid = __builtin_bswap16(tmp.time_mid);
	/* efbe -> beef */
	tmp.time_hi_and_version = __builtin_bswap16(tmp.time_hi_and_version);
#endif
	memcpy(out, (void *)&tmp, sizeof(struct teec_uuid));
}
			    ...
			    struct gp_return *gp_ret)
{
	...
	if (protocol_is_fe())
		return protocol_gp_register_shared_mem(mmu, sva, gp_ret);
...
{
	if (protocol_is_fe())
		return protocol_gp_release_shared_mem(map);
	...
	const char vm_id[16])
{
	...
	if (ta_mmu) {
		...
		{
			struct mclf_header *header = (struct mclf_header *)
				&obj->data;
			...
			/* Convert UUID */
			mcuuid_to_tee_uuid(&header->uuid,
					   &op_iws->target_uuid);
	...
	if (ta_mmu) {
		...
		tee_mmu_buffer(ta_mmu, &map);
		iwp_iws_set_tmpref(iws, 0, &map);
	...
	if (protocol_is_fe())
		ret = protocol_gp_open_session(iwp_session, uuid, ta_mmu, maps,
					       iws, op_iws, gp_ret);
	else
	...
	struct iwp_session *iwp_session)
{
	...
	mutex_lock(&iwp_session->iws_lock);
	if (protocol_is_fe()) {
		ret = protocol_gp_close_session(iwp_session);
	} else {
	...
	struct gp_return *gp_ret)
{
	...
	if (protocol_is_fe())
		ret = protocol_gp_invoke_command(iwp_session, maps, iws,
						 gp_ret);
	else
	...
	u64 slot)
{
	...
	if (protocol_is_fe())
		return protocol_gp_request_cancellation(
