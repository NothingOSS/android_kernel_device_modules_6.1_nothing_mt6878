...
/* Used by frontend to allocate VM data struct to control comm.
 * Also in Xen PV model we have it's the Frontend who physically allocates the
 * Xen Ring and share it, more a ref to it, to the Backend through the xenbus
 */
static inline struct tee_xfe *xfe_create(struct xenbus_device *xdev)
{
	...
	/* Create shared information buffer, Xen Ring */
	...
	/* Set the local pfe header with FE versions */
	mc_dev_devel("%s Set the FE version 0x%x", __func__, MC_PV_VERSION);
	set_fe_driver_version(&xfe->pfe.hdr, MC_PV_VERSION);

	/* Update the ring with FE versions for the BE */
	xfe->ring->fe2be_data.hdr_version = xfe->pfe.hdr.hdr_version;
	xfe->ring->fe2be_data.fe_version = xfe->pfe.hdr.fe_version;
	...
	mc_dev_devel("evtchn domu=%u dom0=%u version=%u",
		     xfe->evtchn_domu, xfe->evtchn_dom0, MC_PV_VERSION);
...
/* Called when Guest VM/frontend is initialized */
				   ...
				   enum xenbus_state be_state)
{
	...
	case XenbusStateConnected:
		/* Update the local pfe header with BE version from the ring */
		xfe->pfe.hdr.be_version = xfe->ring->be2fe_data.be_version;
		/* Get the BE version */
		mc_dev_info("%s Connected to BE version 0x%x",
			    __func__, get_be_driver_version(&xfe->pfe.hdr));
		if (l_ctx.start())
