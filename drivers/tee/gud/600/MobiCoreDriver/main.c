...
static struct {
	 ...
	 * Warning mixing internal TEE states and internal APIs errors:
	 * - 0 for success/no error
	 * - TEE_START_NOT_TRIGGERED (1) if TEE not ready
	 * - any errno.h value returned by kernel APIs
	 */
	int start_ret;
#ifdef MC_PM_RUNTIME
	...
	/* main_ctx can be either accessed from driver probe or directly
	 * from Kernel through teec_ kernel APIs (no init in this case).
	 * Enforcing start_ret initialization to "default" value (not 0!)
	 */
	.start_ret = TEE_START_NOT_TRIGGERED
};
...
static inline int check_version(void)
{
	...
	if (version_info.version_mci != MC_VERSION(2, 0)) {
		ret = -EHOSTDOWN;
...
static int mobicore_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		mc_dev_err(ret, "FFA probe failed");
		return ret;
	}
#endif
...
#ifdef MC_PROBE_WITHOUT_DEVICE_TREE
		/* load TEE even if not declared in Device Tree */
		ret = mobicore_probe_not_of();
#else
		mc_dev_devel("not found in Device Tree");
		ret = 0;
#endif
	}
	return ret;
...
#ifndef MC_PROBE_WITHOUT_DEVICE_TREE
	/* if !MC_PROBE_WITHOUT_DEVICE_TREE then
	 * not in DeviceTree means no clean required, just return.
	 */
	else
		return;
#endif
