		      ...
		      const struct mc_ioctl_buffer *buf)
{
	...
	wsm->mmu = client_mmu_create(session->client, buf);
	if (IS_ERR(wsm->mmu))
...
static void wsm_free(struct tee_session *session, struct tee_wsm *wsm)
{
	...
	mc_dev_devel("free wsm %p: mmu %p va %lx len %u sva %x",
		     wsm, wsm->mmu, wsm->va, wsm->len, wsm->sva);
	/* Free MMU table */
	tee_mmu_put(wsm->mmu);
				   ...
				   const struct mc_identity *identity)
{
	...
	struct identity mcp_identity = {0};
			     ...
			     int no)
{
	...
	ret = kasnprintf(buf, "\t\t");
	if (no < 0)
		ret = kasnprintf(buf, "tci %pK: va %pK len %u\n",
				 wsm, (void *)wsm->va, wsm->len);
	else if (wsm->in_use)
		ret = kasnprintf(buf,
				 "wsm #%d: va %pK len %u sva %x\n",
				 no, (void *)wsm->va, wsm->len,
				 wsm->sva);
			    ...
			    struct mcp_open_info *info)
{
	...
	if (info->tci_va) {
		...
		mc_dev_devel(
			"created tci: mmu %p va %lx len %u flags %x",
			wsm->mmu, wsm->va, wsm->len, wsm->flags);
	}
		   ...
		   struct mc_ioctl_buffer *buf)
{
	...
	mc_dev_devel("created wsm #%d: mmu %p va %lx len %u flags %x",
		     i, wsm->mmu, wsm->va, wsm->len, wsm->flags);
			    ...
			    struct gp_return *gp_ret)
{
	...
	/* Open/call TA */
	ret = iwp_open_session(&session->iwp_session, uuid, ta_mmu,
			       operation, maps, NULL, NULL, gp_ret,
			      ...
			      struct gp_return *gp_ret)
{
	...
	client_gp_operation_remove(session->client, &client_operation);
	unmap_gp_bufs(session, maps);
	mutex_unlock(&session->iwp_session.iws_lock);
	return ret;
}
