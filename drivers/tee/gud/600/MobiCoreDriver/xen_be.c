...
/*
 * Copyright (c) 2017-2022 TRUSTONIC LIMITED
 * All Rights Reserved.
 *
...
/* Called when a front-end is 1st created, used as initialization before
 * calling immediately after Frontend_attach() callback
 */
			...
			const struct xenbus_device_id *id)
{
	...
	/* Allocate Main VM data struct to control comm
	 * Xen Ring/VMs shared buffer to be received later, when frontend attach
	 */
...
static inline void frontend_attach(struct tee_xfe *xfe)
{
	...
	/* Use xenbus as initial sharing methods to get Frontend info */
	...
	/* Remap locally in Backend the Xen Ring shared by the Frontend */
	...
	/* Update the local pfe header with FE versions from the ring */
	xfe->pfe.hdr.fe_version = xfe->ring->fe2be_data.fe_version;
	xfe->pfe.hdr.hdr_version = xfe->ring->fe2be_data.hdr_version;

	/* Get the FE version */
	domu_version = get_fe_driver_version(&xfe->pfe.hdr);
	mc_dev_info("%s Connected to FE version 0x%x",
		    __func__, domu_version);

	mc_dev_devel("ring ref %llu evtchn domu=%u dom0=%u version=%u",
		     xfe->ring_ref, xfe->evtchn_domu, xfe->evtchn_dom0,
		     domu_version);

	/* Set the local pfe header with BE version */
	mc_dev_devel("%s Set the BE version 0x%x", __func__, MC_PV_VERSION);
	if (!set_be_driver_version(&xfe->pfe.hdr, MC_PV_VERSION))
		mc_dev_info("%s BE version is not set", __func__);

	/* Update the ring with BE version for the FE */
	xfe->ring->be2fe_data.be_version = xfe->pfe.hdr.be_version;

#if KERNEL_VERSION(5, 15, 0) <= LINUX_VERSION_CODE
	ret = bind_interdomain_evtchn_to_irqhandler_lateeoi(
		xfe->xdev, xfe->evtchn_domu,
...
static int xen_be_early_init(int (*probe)(void), int (*start)(void))
{
	...
	size_copied = strscpy(l_ctx.vm_id, be_vm_id, sizeof(l_ctx.vm_id));
	if (size_copied == -E2BIG) {
		mc_dev_info("vm_id size bigger than %zd",
			    sizeof(l_ctx.vm_id));
	}
	kfree(be_vm_id);
	return 0;
