...
//modify camera info by baitao 2023/10/13 begin
...
//modify camera info by baitao 2023/10/13 end
...
char nfc_version[32] = "nfc unknow";
EXPORT_SYMBOL(nfc_version);
...
extern char slave_charger_info_3[32];
extern char slave_charger_info_4[32];
		...
		struct kobj_attribute *attr, char * buf)
{
	...
	else if (slave_charger_info_3[0]
			&& (memcmp(slave_charger_info_3, "unknown", 7) != 0))
		s += sprintf(s, "%s\n", slave_charger_info_3);
	else if (slave_charger_info_4[0]
			&& (memcmp(slave_charger_info_4, "unknown", 7) != 0))
		s += sprintf(s, "%s\n", slave_charger_info_4);
	else
		s += sprintf(s, "%s\n", "unknown");
...
static ssize_t tele_camera_info_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	if(0 == strlen(tele_camera)){
		sprintf(buf, "tele_camera error to find!\n");
	} else {
		sprintf(buf, "%s\n", tele_camera);
	}
...
static ssize_t uw_camera_info_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	if(0 == strlen(depth_camera)){
		sprintf(buf, "uw_camera error to find!\n");
	} else {
		sprintf(buf, "%s\n", depth_camera);
	}
...
static ssize_t uw_camera_info_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	return n;
}
...
static struct kobj_attribute tele_camera_info_attr = {
	.attr = {
		.name = "cam_tele_lens_mfr",
		.mode = 0644,
	},
...
static struct kobj_attribute uw_camera_info_attr = {
	.attr = {
		.name = "cam_uw_lens_mfr",
		.mode = 0644,
	},
	.show =&uw_camera_info_show,
	.store= &uw_camera_info_store,
};
...
static ssize_t tele_camera_msn_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	if(0 == strlen(tele_camera_sn)){
		sprintf(buf, "tele_camera_sn error to find!\n");
	} else {
		sprintf(buf, "%s\n", tele_camera_sn);
	}
...
static ssize_t tele_camera_msn_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	return n;
}
...
static struct kobj_attribute tele_camera_msn_attr = {
	.attr = {
		.name = "tele_camera_msn",
		.mode = 0644,
	},
...
static struct kobj_attribute uw_camera_msn_attr = {
	...
	.show =&uw_camera_msn_show,
	.store= &uw_camera_msn_store,
};
...
static ssize_t cam_vendor_info_list_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	if(0 == strlen(main_module_id)){
		sprintf(buf, "main_module_id error to find!\n");
	} else {
...
static ssize_t cam_vendor_info_list_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	return n;
}
...
static struct kobj_attribute cam_vendor_info_list_attr = {
	.attr = {
		.name = "cam_vendor_info_list",
		.mode = 0644,
	},
	.show =&cam_vendor_info_list_show,
	.store= &cam_vendor_info_list_store,
};
