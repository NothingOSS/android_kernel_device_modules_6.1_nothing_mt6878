...
static int mt6681_efuse_read(void *context, unsigned int offset,
			     void *_val, size_t bytes)
{
	struct mt6681_efuse *efuse = context;
	...
	for (; offset < offset_end; offset += EFUSE_REG_WIDTH) {
		...
		ret = mt6681_efuse_poll_busy(efuse);
		if (ret)
			goto disable_efuse;
...
static int mt6681_efuse_probe(struct platform_device *pdev)
{
	...
	econfig.name = "mt6681-efuse";
	econfig.size = efuse->data->reg_num * EFUSE_REG_WIDTH;
	econfig.reg_read = mt6681_efuse_read;
	econfig.priv = efuse;
...
static const struct of_device_id mt6681_efuse_of_match[] = {
	{
		.compatible = "mediatek,mt6681-efuse",
		.data = &mt6681_efuse_data
	}, {
		/* sentinel */
	}
};
static struct platform_driver mt6681_efuse_driver = {
	.probe = mt6681_efuse_probe,
	.driver = {
		.name = "mt6681-efuse",
...
MODULE_DESCRIPTION("MediaTek PMIC eFuse Driver for MT6681 PMIC");
