...
static void show_readahead_status(void *data, struct readahead_control *ractl, unsigned long *max_pages)
{
	...
	ractl->file->android_kabi_reserved2++;

	if (ractl->file->android_kabi_reserved2 >= 8) {
		...
		ractl->file->android_kabi_reserved2 = 0;
	}
...
static bool find_dir_sub(struct dir_context *ctx, const char *name, int namlen,
...
ERR_OUT:
	NT_frag_err_print("=== it's a lost file: %s %d %llu alloc_dentry: %p return_dentry: %p ,ret %d %d ===\n", real_name, namlen, ino, subdir_info->alloc_dentry, subdir_info->find_dentry, ret, is_dir);
OUT:
	return 1;
}
