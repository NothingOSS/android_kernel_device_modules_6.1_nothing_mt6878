...
static bool printdir_sub(struct dir_context *ctx, const char *name, int namlen,
...
ERR_OUT:
	NT_disk_usage_err_print("=== it's a lost file:  %s %d %llu alloc_dentry: %p return_dentry: %p ,ret %d %d %d ===\n", real_name, namlen, ino, alloc_dentry, return_dentry, ret, is_dir, stack_size);
OUT:
	if (!is_dir) {
		dput(alloc_dentry);
		dput(return_dentry);
