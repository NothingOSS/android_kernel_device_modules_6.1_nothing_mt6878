 ...
 *	0x2_0000_0000~0x3_FFFF_FFFF (8GB)
 * 1,GCE:            0x1_0400_0000~0x1_05FF_FFFF
 * 2,LK_RESV:        0x1_0600_0000~0x1_07FF_FFFF
 * 3,CCU0:           0x1_0800_0000~0x1_0BFF_FFFF
 * 4,CCU1:           0x1_0C00_0000~0x1_0FFF_FFFF
 * 5,VDO_UP:         0x1_1000_0000~0x1_6FFF_FFFF
 ...
 * @pm_sta: iommu pm runtime status
			...
			unsigned long flags, void *data)
{
	...
	if (flags == GENPD_NOTIFY_ON || flags == GENPD_NOTIFY_PRE_OFF) {
		mtk_iommu_pm_trace(((flags == GENPD_NOTIFY_ON) ?
				   IOMMU_POWER_ON : IOMMU_POWER_OFF),
				   nb->priority, pd_sta[nb->priority],
				   flags, NULL);
	}
#endif
...
static struct mtk_iommu_data *mtk_iommu_get_data(u32 type, u32 id)
{
	struct list_head *hw_list;
	struct mtk_iommu_data *data;
	...
	case APU_IOMMU:
		...
		break;
	default:
		pr_info("%s failed, type is invalid, %d\n", __func__, type);
		return NULL;
	}
...
static int mtk_iommu_pm_status_update(u32 type, u32 id, bool pm_sta)
{
	struct mtk_iommu_data *data;
	unsigned long flags;
	int iommu_ids;

	/* Only support APU IOMMU */
	if (type != APU_IOMMU)
		return -1;
	...
	data = mtk_iommu_get_data(type, id);
	if (!data)
		return -1;

	spin_lock_irqsave(&data->tlb_lock, flags);
	apu_pm_sta[id] = (pm_sta ? POWER_ON_STA : POWER_OFF_STA);
	spin_unlock_irqrestore(&data->tlb_lock, flags);

#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_DBG)
	iommu_ids = MTK_IOMMU_ID_FLAG(data->plat_data->iommu_type,
				      data->plat_data->iommu_id);
	mtk_iommu_pm_trace((pm_sta ? IOMMU_POWER_ON : IOMMU_POWER_OFF),
			   iommu_ids, apu_pm_sta[id], pm_sta, NULL);
#endif
...
static int mtk_iommu_probe(struct platform_device *pdev)
{
	...
	if (!atomic_cmpxchg(&init_once_flag, 0, 1)) {
		...
		mtk_iommu_set_ops(&mtk_iommu_export_ops);
	}
...
skip_smi:
	...
	} else if (data->plat_data->iommu_type == APU_IOMMU) {
		apu_pm_sta[data->plat_data->iommu_id] = POWER_OFF_STA;
	}
...
#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_DBG)
	mtk_iommu_pm_trace(IOMMU_SUSPEND, data->plat_data->iommu_id,
...
static const struct mtk_iommu_plat_data mt6886_data_apu0 = {
	...
	.flags          = TLB_SYNC_EN | GET_DOM_ID_LEGACY | IOMMU_SEC_EN |
			  IOVA_34_EN | LINK_WITH_APU | PGTABLE_PA_35_EN | PM_OPS_SKIP,
	.hw_list        = &apu_iommu_list,
	.inv_sel_reg    = REG_MMU_INV_SEL_GEN2,
	.iommu_id	= APU_IOMMU0,
	.iommu_type     = APU_IOMMU,
...
static const struct mtk_iommu_plat_data mt6897_data_disp = {
	...
	.tab_id		= MM_TABLE,
	.normal_dom	= 0,
	.iova_region    = mt6897_multi_dom_mm,
	.iova_region_nr = ARRAY_SIZE(mt6897_multi_dom_mm),
	.mau_count	= 4,
};
...
static const struct mtk_iommu_plat_data mt6897_data_apu0 = {
	...
	.tab_id		= APU_TABLE,
	.normal_dom	= 0,
	.iova_region    = mt6897_multi_dom_apu,
	.iova_region_nr = ARRAY_SIZE(mt6897_multi_dom_apu),
	.mau_count	= 4,
};
...
static const struct of_device_id mtk_iommu_of_ids[] = {
	...
	{ .compatible = "mediatek,mt6897-apu-iommu0", .data = &mt6897_data_apu0},
	{ .compatible = "mediatek,mt6897-apu-iommu1", .data = &mt6897_data_apu1},
	{ .compatible = "mediatek,mt6897-disp-iommu", .data = &mt6897_data_disp},
	{ .compatible = "mediatek,mt6897-mdp-iommu", .data = &mt6897_data_mdp},
