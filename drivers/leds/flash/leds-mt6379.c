				       ...
				       enum led_brightness brightness)
{
	...
	if (data->strobe_enabled) {
		ret = -EINVAL;
		dev_info(led_cdev->dev, "Storbe in used [%lu]\n", data->strobe_enabled);
		goto out_torch_set;
	}
...
static int mt6379_flash_set_strobe(struct led_classdev_flash *flash, bool state)
{
	...
	if (data->torch_enabled) {
		ret = -EINVAL;
		dev_info(flash->led_cdev.dev, "Torch in used [%lu]\n",
			data->torch_enabled);
		goto out_strobe_set;
	}
	...
	ret = regmap_update_bits(regmap, MT6379_REG_FLED_EN, mask, enable);
	if (ret) {
		dev_info(flash->led_cdev.dev, "Failed to set FLED_EN\n");
		goto out_strobe_set;
	}
...
static int __maybe_unused mt6379_cooling_set_cur_state(int channel, unsigned long state)
{
	struct mt6379_flash *mtflash;
					...
					struct mt6379_flash *mtflash)
{
	...
	if (ret)
		return rt_err_probe(dev, ret, "Failed to get reg prop\n");

	if (src >= MT6379_FLASH_MAX_LED || data->flash_used & BIT(src))
		return rt_err_probe(dev, -EINVAL, "Invalid reg prop\n");
...
static int mt6379_leds_irq_register(struct platform_device *pdev, void *irqdata)
{
	...
	for (i = 0; i < ARRAY_SIZE(irqts); i++) {
		...
		if (rv) {
			dev_info(&pdev->dev, "failed to request irq [%s]\n", irqts[i].name);
			return rv;
		}
	}
...
static int mt6379_flash_probe(struct platform_device *pdev)
{
	...
	if (!data->regmap)
		return rt_err_probe(dev, -EINVAL, "Failed to init regmap\n");
	...
	fwnode_for_each_available_child_node(fwnode, child) {
		...
		if (ret) {
			fwnode_handle_put(child);
			return rt_err_probe(dev, ret, "Failed to register flash %d\n", count);
		}
		...
		if (IS_ERR(mtflash->v4l2)) {
			...
			return rt_err_probe(dev, ret, "Failed to register v4l2 flash %d\n", count);
		}
		...
		if (ret) {
			fwnode_handle_put(child);
			return rt_err_probe(dev, ret, "Failed to add release action\n");
		}
	...
	if (ret)
		return rt_err_probe(&pdev->dev, ret, "failed to register led irqs\n");
