...
#define UPM6720_NAME "upm6720_cp"
char slave_charger_info_2[32] = {"unknown"};
EXPORT_SYMBOL(slave_charger_info_2);
...
static const u32 upm6720_adc_accuracy_tbl[UPM_ADC_MAX] = {
	250000,	/* IBUS */
	35000,	/* VBUS */
	35000,	/* VAC1 */
    35000,	/* VAC2 */
...
static int __upm6720_read_byte(struct upm6720 *upm, u8 reg, u8 *data)
{
    ...
    if (ret < 0) {
        upm_err("i2c read fail: can't read from i2c address:0x%02X, retry other address 0x66\n", upm->client->addr);
        upm->client->addr = 0x66;
        ret = i2c_smbus_read_byte_data(upm->client, reg);
        if (ret < 0) {
...
static int __upm6720_write_byte(struct upm6720 *upm, int reg, u8 val)
{
    ...
    if (ret < 0) {
        upm_err("i2c write fail: can't write from i2c address:0x%02X, retry other address 0x66\n", upm->client->addr);
        upm->client->addr = 0x66;
        ret = i2c_smbus_write_byte_data(upm->client, reg, val);
        if (ret < 0) {
...
static int upm6720_get_adc_data(struct upm6720 *upm, adc_channel_t channel,  int *result)
{
	...
	if ((channel == UPM_ADC_VBUS) && (sval >= 10500)) {
		upm_err("VBUS ADC abnormal data = %d\n", sval);
		ret = upm6720_read_byte(upm, ADC_REG_BASE + (UPM_ADC_VAC2 << 1), &val_h);
		if (ret < 0) {
			return ret;
		}

		ret = upm6720_read_byte(upm, ADC_REG_BASE + (UPM_ADC_VAC2 << 1) + 1, &val_l);
		if (ret < 0) {
			return ret;
		}
		val = (val_h << 8) | val_l;
		sval = val * UPM6720_VAC2_ADC_LSB / UPM6720_VAC2_ADC_PRECISION + UPM6720_VAC2_ADC_BASE + UPM6720_VAC2_ADC_OFFSET;
		upm_err("VBUS ADC correction data = %d\n", sval);
	}
...
static int upm6720_parse_dt(struct upm6720 *upm, struct device *dev)
{
    ...
    if (of_property_read_string(np, "charger_name", &upm->chg_dev_name) < 0) {
		upm->chg_dev_name = "primary_dvchg";
...
static int upm6720_dump_registers(struct upm6720 *upm)
{
	int ret;
	u8 val;
	u8 addr;
...
static int upm6720_is_vbuslowerr(struct upm6720 *upm, bool *err)
{
	int ret;
	u8 val;
...
static inline int to_upm6720_adc(enum adc_channel chan)
{
	switch (chan) {
...
static int mtk_upm6720_is_chg_enabled(struct charger_device *chg_dev, bool *en)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    int ret;
    upm_info("mtk_upm6720_is_chg_enabled\n");
...
static int mtk_upm6720_enable_chg(struct charger_device *chg_dev, bool en)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    int ret;
    upm_info("mtk_upm6720_enable_chg\n");
    ret = upm6720_enable_charge(upm, en);

    return ret;
}

static int mtk_upm6720_set_vbusovp(struct charger_device *chg_dev, u32 uV)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    int mv;
    mv = uV / 1000;
    upm_info("mtk_upm6720_set_vbusovp\n");

    return upm6720_set_busovp_th(upm, CHG_BYPASS_MODE, mv);
}

static int mtk_upm6720_set_ibusocp(struct charger_device *chg_dev, u32 uA)
{
    //struct upm6720 *upm = charger_get_data(chg_dev);
    //int ma;
    //ma = uA / 1000;
    //upm_info("mtk_upm6720_set_ibusocp\n");
    //
    //return upm6720_set_busocp_th(upm, CHG_BYPASS_MODE, ma);
    return 0;
}

static int mtk_upm6720_set_vbatovp(struct charger_device *chg_dev, u32 uV)
{   
    struct upm6720 *upm = charger_get_data(chg_dev);
    int ret;
    upm_info("mtk_upm6720_set_vbatovp\n");

    ret = upm6720_set_batovp_th(upm, uV/1000);
    if (ret < 0)
        return ret;
...
static int mtk_upm6720_set_ibatocp(struct charger_device *chg_dev, u32 uA)
{   
    //struct upm6720 *upm = charger_get_data(chg_dev);
    //int ret = 0;
    //upm_info("mtk_upm6720_set_ibatocp\n");
    //
    //ret = upm6720_set_batocp_th(upm, uA/1000);
    //if (ret < 0)
    //    return ret;
    return 0;
}

static int mtk_upm6720_get_adc(struct charger_device *chg_dev, enum adc_channel chan,
			  int *min, int *max)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    upm_info("mtk_upm6720_get_adc\n");
    upm6720_get_adc_data(upm, to_upm6720_adc(chan), max);
...
static int mtk_upm6720_get_adc_accuracy(struct charger_device *chg_dev,
				   enum adc_channel chan, int *min, int *max)
{
    *min = *max = upm6720_adc_accuracy_tbl[to_upm6720_adc(chan)];
    return 0;
}

static int mtk_upm6720_is_vbuslowerr(struct charger_device *chg_dev, bool *err)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    upm_info("mtk_upm6720_is_vbuslowerr\n");

    return upm6720_is_vbuslowerr(upm,err);
}

static int mtk_upm6720_set_vbatovp_alarm(struct charger_device *chg_dev, u32 uV)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    int ret;
    upm_info("mtk_upm6720_set_vbatovp_alarm\n");

    ret = upm6720_set_bat_ovp_alm_th(upm, uV/1000);
    if (ret < 0)
        return ret;
...
static int mtk_upm6720_reset_vbatovp_alarm(struct charger_device *chg_dev)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    upm_info("mtk_upm6720_reset_vbatovp_alarm\n");
    return 0;
}

static int mtk_upm6720_set_vbusovp_alarm(struct charger_device *chg_dev, u32 uV)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    int ret;
    upm_info("mtk_upm6720_set_vbusovp_alarm\n");

    ret = upm6720_set_bus_ovp_alm_th(upm, CHG_BYPASS_MODE, uV/1000);
    if (ret < 0)
        return ret;
...
static int mtk_upm6720_reset_vbusovp_alarm(struct charger_device *chg_dev)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    upm_info("mtk_upm6720_reset_vbusovp_alarm\n");
    return 0;
}

static int mtk_upm6720_init_chip(struct charger_device *chg_dev)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    upm_info("mtk_upm6720_init_chip\n");
    return upm6720_init_device(upm);
}

static int mtk_upm6720_get_ovpgate_state(struct upm6720 *upm)
{
	int ret;
	u8 val;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_0F, &val);
	if (ret < 0) {
		upm_err("upm6720 get ovpgate state fail\n");
		return -1;
	}
	val = !!(val & UPM6720_ACDRV1_STAT_MASK);
	return val;
}

static int mtk_upm6720_get_wpcgate_state(struct upm6720 *upm)
{
	int ret;
	u8 val;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_0F, &val);
	if (ret < 0) {
		upm_err("upm6720 get wpcgate state fail\n");
		return -1;
	}
	val = !!(val & UPM6720_ACDRV2_STAT_MASK);
	return val;
}

static int mtk_upm6720_get_mode_state(struct upm6720 *upm)
{
	int ret;
	u8 val;
	...
	ret = upm6720_read_byte(upm, UPM6720_REG_0F, &val);
	if (ret < 0) {
		upm_err("upm6720 get bypass state fail\n");
		return -1;
	}
	val = !!(val & UPM6720_EN_BYPASS_MASK);
	return val == 1 ? 1 : 2;
}

static int mtk_upm6720_get_cp_status(struct charger_device *chg_dev, u32 evt)
{
	int val = 0;
	struct upm6720 *upm = charger_get_data(chg_dev);
	...
	case CP_DEV_REVISION:
		...
		break;
	case CP_DEV_OVPGATE:
		val = mtk_upm6720_get_ovpgate_state(upm);
		break;
	case CP_DEV_WPCGATE:
		val = mtk_upm6720_get_wpcgate_state(upm);
		break;
	case CP_DEV_WORKMODE:
		val = mtk_upm6720_get_mode_state(upm);
		break;
	default:
		break;
	}
...
static int mtk_upm6720_dump_registers(struct charger_device *chg_dev)
{
	int ret;
	u8 val;
	u8 addr;
...
static int upm6720_otg_ovp_enable(struct upm6720 *upm)
{
	int ret;
	u8 val;

	upm_info("%s\n", __func__);
	val = UPM6720_EN_OTG_ENABLE;
	val <<= UPM6720_EN_OTG_SHIFT;
	ret = upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_EN_OTG_MASK, val);
	if (ret < 0) {
		dev_err(upm->dev, "upm6720 write OTG_EN:1 fail: %d\n", ret);
		return ret;
	}

	val = UPM6720_DIS_ACDRV_BOTH_DISABLE;
	val <<= UPM6720_DIS_ACDRV_BOTH_SHIFT;
	ret = upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_DIS_ACDRV_BOTH_MASK, val);
	if (ret < 0) {
		dev_err(upm->dev, "upm6720 write DIS_ACDRV_BOTH_DISABLE fail: %d\n", ret);
		return ret;
	}

	val = UPM6720_ACDRV1_STAT_ON;
	val <<= UPM6720_ACDRV1_STAT_SHIFT;
	ret = upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_ACDRV1_STAT_MASK, val);
	if (ret < 0) {
		dev_err(upm->dev, "upm6720 write ACDRV1_STAT:1 fail: %d\n", ret);
		return ret;
	}
	upm_err("upm6720 enable OTG successfully!\n");

	return ret;
}

static int upm6720_otg_ovp_disable(struct upm6720 *upm)
{
	int ret;
	u8 val;

	upm_info("%s\n", __func__);
	val = UPM6720_EN_OTG_DISABLE;
	val <<= UPM6720_EN_OTG_SHIFT;
	ret = upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_EN_OTG_MASK, val);
	if (ret < 0) {
		dev_err(upm->dev, "upm6720 write OTG_EN:0 fail: %d\n", ret);
		return ret;
	}

	upm_err("upm6720 disable OTG successfully!\n");

	return ret;
}

static int mtk_upm6720_enable_vac_otgovp(struct charger_device *chg_dev, bool en)
{
	struct upm6720 *upm = charger_get_data(chg_dev);

	upm_info("%s\n", __func__);
	if (en)
		return upm6720_otg_ovp_enable(upm);
	else
		return upm6720_otg_ovp_disable(upm);
}

static const struct charger_ops upm_chg_ops = {
	 .enable = mtk_upm6720_enable_chg,
	 .is_enabled = mtk_upm6720_is_chg_enabled,
	 .get_adc = mtk_upm6720_get_adc,
     .get_adc_accuracy = mtk_upm6720_get_adc_accuracy,
	 .set_vbusovp = mtk_upm6720_set_vbusovp,
	 .set_ibusocp = mtk_upm6720_set_ibusocp,
	 .set_vbatovp = mtk_upm6720_set_vbatovp,
	 .set_ibatocp = mtk_upm6720_set_ibatocp,
	 .init_chip = mtk_upm6720_init_chip,
     .is_vbuslowerr = mtk_upm6720_is_vbuslowerr, 
	 .set_vbatovp_alarm = mtk_upm6720_set_vbatovp_alarm,
	 .reset_vbatovp_alarm = mtk_upm6720_reset_vbatovp_alarm,
	 .set_vbusovp_alarm = mtk_upm6720_set_vbusovp_alarm,
	 .reset_vbusovp_alarm = mtk_upm6720_reset_vbusovp_alarm,
	 .get_cp_status	= mtk_upm6720_get_cp_status,
	 .dump_registers = mtk_upm6720_dump_registers,
	 .enable_vac_otgovp = mtk_upm6720_enable_vac_otgovp,
};
                ...
                struct device_attribute *attr, char *buf)
{
    ...
    for (addr = 0x0; addr <= 0x38; addr++) {
        ret = __upm6720_read_byte(upm, addr, &val);
        if (ret == 0) {
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret == 2 && reg <= 0x38)
        __upm6720_write_byte(upm, (unsigned char)reg, (unsigned char)val);
...
static int upm6720_psy_register(struct upm6720 *upm)
{
    ...
    if (upm->mode == UPM6720_ROLE_MASTER)
        ...
        upm->psy_desc.name = "upm6720-standalone";
...
static irqreturn_t upm6720_charger_interrupt(int irq, void *dev_id)
{
    ...
    if (!upm->resume_completed) {
        dev_err(upm->dev, "IRQ triggered before device-resume\n");
        if (!upm->irq_disabled) {
...
#if 0
static int upm6720_otg_enable(struct regulator_dev *rdev)
{
	int ret;
	u8 val;
	struct upm6720 *upm = rdev_get_drvdata(rdev);
	...
	val = UPM6720_DIS_ACDRV_BOTH_ENABLE;
	val <<= UPM6720_DIS_ACDRV_BOTH_SHIFT;
	ret = upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_DIS_ACDRV_BOTH_MASK, val);
	if (ret < 0) {
		dev_err(upm->dev, "upm6720 write DIS_ACDRV_BOTH_ENABLE fail: %d\n", ret);
		return ret;
	}
	...
	upm_info("upm6720 enable OTG successfully!\n");

	return upm6720_dump_registers(upm);
}

static int upm6720_otg_disable(struct regulator_dev *rdev)
{
	int ret;
	u8 val;
	struct upm6720 *upm = rdev_get_drvdata(rdev);
	...
	val = UPM6720_VBUS_PD_ENABLE;
	val <<= UPM6720_VBUS_PD_EN_SHIFT;
	ret = upm6720_update_bits(upm, UPM6720_REG_06, UPM6720_VBUS_PD_EN_MASK, val);
	if (ret < 0) {
		dev_err(upm->dev, "upm6720 write VBUS_PD_EN:1 fail: %d\n", ret);
		val = UPM6720_AC1_PD_ENABLE;
		val <<= UPM6720_AC1_PD_EN_SHIFT;
		ret = upm6720_update_bits(upm, UPM6720_REG_0E, UPM6720_AC1_PD_EN_MASK, val);
		if (ret < 0) {
			dev_err(upm->dev, "upm6720 write VAC1_PD_EN:1 fail: %d\n", ret);
			return ret;
		}
	}
	...
	val = UPM6720_ACDRV1_STAT_OFF;
	val <<= UPM6720_ACDRV1_STAT_SHIFT;
	ret = upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_ACDRV1_STAT_MASK, val);
	if (ret < 0) {
		dev_err(upm->dev, "upm6720 write ACDRV1_EN:0 fail: %d\n", ret);
		return ret;
	}
...
*/
	upm_info("upm6720 disable OTG successfully!\n");

	return upm6720_dump_registers(upm);
}

static int upm6720_boost_is_enabled(struct regulator_dev *rdev)
{
	return 0;
}

static const struct regulator_ops upm6720_chg_otg_ops = {
	//.list_voltage = regulator_list_voltage_linear,
	.enable = upm6720_otg_enable,
	.disable = upm6720_otg_disable,
	.is_enabled = upm6720_boost_is_enabled,
};

static const struct regulator_desc upm6720_otg_rdesc = {
	.name = "upm6720-otg-vbus",
	.of_match = "upm6720,otg-vbus",
	.type = REGULATOR_VOLTAGE,
	.owner = THIS_MODULE,
	.ops = &upm6720_chg_otg_ops,
	.fixed_uV = 5000000,
	.n_voltages = 1,
};
#endif
                    ...
                    const struct i2c_device_id *id)
{
    ...
    if (!upm) {
		dev_err(&client->dev,"upm6720 probe fail\n");
        return -ENOMEM;
    }
	...
	/* upm6720 otg regulator */
	config.dev = &client->dev;
	config.driver_data = upm;
#if 0
	upm->otg_rdev = devm_regulator_register(&client->dev,
						&upm6720_otg_rdesc, &config);
	if (IS_ERR(upm->otg_rdev)) {
		ret = PTR_ERR(upm->otg_rdev);
		upm_err("upm6720 otg_rdev error\n");
		goto err_register_mtk_charger;
	}
#endif

	/* update cp chg hw info  */
	memset(slave_charger_info_2, 0, sizeof(slave_charger_info_2));
	memcpy(slave_charger_info_2, UPM6720_NAME,
		strlen(UPM6720_NAME) > (sizeof(slave_charger_info_2) - 1) ?
		(sizeof(slave_charger_info_2) - 1) : strlen(UPM6720_NAME));
...
static void upm6720_charger_remove(struct i2c_client *client)
{
    struct upm6720 *upm = i2c_get_clientdata(client);
