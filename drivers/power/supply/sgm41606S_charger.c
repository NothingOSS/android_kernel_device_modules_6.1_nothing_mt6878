...
static int mtk_sgm41606S_enable_vac_otgovp(struct charger_device *chg_dev, bool en)
{
	struct sgm41606S_chip *sgm = charger_get_data(chg_dev);
	int ret = 0;
	...
	if (en) {
		...
		ret += sgm41606S_field_write(sgm, B_DIS_ACDRV_BOTH, 1);
...
//
	 ...
	 .enable_vac_otgovp = mtk_sgm41606S_enable_vac_otgovp,
};
#endif /*CONFIG_MTK_CLASS*/
