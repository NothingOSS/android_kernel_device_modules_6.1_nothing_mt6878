...
static bool support_fast_charging(struct mtk_charger *info)
{
	...
	for (i = 0; i < MAX_ALG_NO; i++) {
		...
		if (state == ALG_READY || state == ALG_RUNNING) {
			ret = true;
			info->fastchg_rerun = false;
			break;
		} else if (state == ALG_NOT_READY) {
			info->fastchg_rerun = true;
			break;
		} else if (state == ALG_DONE) {
			break;
	...
	struct chg_limit_setting *setting)
{
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		*/
		...
		//PE PD HVDCP
		if(get_vbus(info) > 6500){
			pdata->input_current_limit = 2000000;
			pdata->charging_current_limit = 3150000;
		}
	} else if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
	    info->usb_type == POWER_SUPPLY_USB_TYPE_DCP) {
		/* NONSTANDARD_CHARGER */
		pdata->input_current_limit =
			info->data.non_std_charger_input_current;
		pdata->charging_current_limit =
			info->data.non_std_charger_current;
		is_basic = true;
	} else {
	...
	chr_err("[BC12] INPUT : %d ma,ICHG : %d ma \n",
		...
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			*/
			if (info->enable_nt_sw_jeita) {
				if (info->sw_jeita.sm == NT_TEMP_T0_TO_T1) {/* 0~10 */
					pdata->charging_current_limit = min(1476000 , pdata->charging_current_limit);
				} else if (info->sw_jeita.sm == NT_TEMP_T4_TO_T5) {/* 45~60 */
					pdata->charging_current_limit = min(1722000 , pdata->charging_current_limit);
				} else if(info->sw_jeita.sm == NT_TEMP_BELOW_T0 || info->sw_jeita.sm == NT_TEMP_ABOVE_T5) {/* >60 || <0 */
					pdata->charging_current_limit = 0;
				} else if((info->sw_jeita.sm >= NT_TEMP_T1_TO_T2) && (info->sw_jeita.sm <= NT_TEMP_T3_TO_T4)) {
					if(info->fastchg_rerun) {
...
//		}
		...
		if (info->enable_nt_sw_jeita) {
			if (info->sw_jeita.sm == NT_TEMP_T0_TO_T1)
				info->setting.charging_current_limit1 = min(1476000 , pdata->thermal_charging_current_limit);
			else if (info->sw_jeita.sm == NT_TEMP_T4_TO_T5)
				info->setting.charging_current_limit1 = min(1722000 , pdata->thermal_charging_current_limit);
			else
...
//		}
	...
	if (pdata2->thermal_input_current_limit != -1) {
		pdata->input_current_limit = min(pdata->input_current_limit,pdata2->thermal_input_current_limit);
	}
done:
...
static bool mtk_switch_check_charging_time(struct mtk_charger *info)
{
	ktime_t now_time = 0, dtime = 0 ,begin_time = 0;
	struct timespec64 tmp_dtime, tmp_now_time;
	union power_supply_propval status;
	struct power_supply *chg_psy = NULL;
	int ret;
	int vbus = get_vbus(info);;

	if(info->is_chg_done || info->aging_mode)
		return true;
	...
	if ((vbus > 2500) && (status.intval == POWER_SUPPLY_STATUS_CHARGING)){
		begin_time = timespec64_to_ktime(info->charging_begin_time);
	}else{
		info->charging_begin_time = ktime_to_timespec64(now_time);
		begin_time = now_time;
	}

	dtime = ktime_sub(now_time, begin_time);
	tmp_dtime = ktime_to_timespec64(dtime);
	tmp_now_time = ktime_to_timespec64(now_time);
	chr_err("%s: diff_time: %d sec ,now: %d sec\n",
				__func__, (int)tmp_dtime.tv_sec, (int)tmp_now_time.tv_sec);

	if (tmp_dtime.tv_sec >= info->data.max_charging_time) {
		chr_err("%s: SW safety timeout: %d sec > %d sec\n",
			__func__, (int)tmp_dtime.tv_sec,  info->data.max_charging_time);
...
static int do_algorithm(struct mtk_charger *info)
{
		...
		if ((ret == ALG_TA_CHECKING) && (mtk_chg_get_area_id() == 3)) {
			if (g_nt_chg) {
				...
				if (g_nt_chg->count >= NT_CHG_CYCLE) {
					...
					chr_info("%s: Abnormal current for more than 20 times, used the input 1500mA!\n", __func__);
				} else {
					chr_info("%s: TA checking, used the input 100mA!\n", __func__);
				}
			}
	...
	if (pdata->input_current_limit == 0 ||
		...
		*/
		alg = get_chg_alg_by_name("pe5");
		ret2 = chg_alg_is_algo_ready(alg);
		if ((ret2 == ALG_DONE) || (ret2 == ALG_NOT_READY) ||
			!(ret2 == ALG_READY || ret2 == ALG_RUNNING))
