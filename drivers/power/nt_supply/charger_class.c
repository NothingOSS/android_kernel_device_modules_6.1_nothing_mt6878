...
int charger_dev_enable_vac_otgovp_spec(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->enable_vac_otgovp_spec)
		return chg_dev->ops->enable_vac_otgovp_spec(chg_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_enable_vac_otgovp_spec);
