...
#define SC8541_DRV_VERSION              "1.1.0_G"

#define SC8541_NAME "sc8541_cp"
char slave_charger_info_1[32] = {"unknown"};
EXPORT_SYMBOL(slave_charger_info_1);
...
__maybe_unused static int sc8541_enable_charge(struct sc8541_chip *sc, bool en)
{
    ...
    if (!en) {
        ...
        ret = sc8541_get_adc_data(sc, ADC_VBUS, &vbus_value);
        ret |= sc8541_get_adc_data(sc, ADC_VOUT, &vout_value);
        dev_info(sc->dev,"%s: vbus/vout:%d / %d = %d \r\n", __func__, vbus_value, vout_value, vbus_value*100/vout_value);

        ret |= sc8541_field_read(sc, MODE, &value);
        dev_info(sc->dev,"%s: mode:%d %s \r\n", __func__, value, (value == 0 ?"4:1":(value == 1 ?"2:1":"else")));

        ret |= sc8541_field_read(sc, VBUS_ERRORLO_STAT, &vbus_low);
        ret |= sc8541_field_read(sc, VBUS_ERRORHI_STAT, &vbus_hi);
        ...
        ret |= sc8541_field_read(sc, CP_SWITCHING_STAT, &value);
        if (!value) {
...
static int mtk_sc8541_get_ovpgate_state(struct sc8541_chip *sc)
{
	int ret;
	int val;

	ret = sc8541_field_read(sc, ACDRV1_STAT, &val);
	if (ret < 0) {
		dev_err(sc->dev, "sc8541 get ovpgate state fail\n");
		return -1;
	}
	return val;
}

static int mtk_sc8541_get_wpcgate_state(struct sc8541_chip *sc)
{
	int ret;
	int val;

	ret = sc8541_field_read(sc, ACDRV2_STAT, &val);
	if (ret < 0) {
		dev_err(sc->dev, "sc8541 get ovpgate state fail\n");
		return -1;
	}
...
static int mtk_sc8541_get_mode_state(struct sc8541_chip *sc)
{
	int ret;
	int val;
	...
	if (ret < 0) {
		dev_err(sc->dev, "sc8541 get bypass state fail\n");
		return -1;
	}
	return val == 0 ? 2 : 1;
}

static int mtk_sc8541_get_cp_status(struct charger_device *chg_dev, u32 evt)
{
	int val = 0;
	struct sc8541_chip *sc = charger_get_data(chg_dev);
	...
	case CP_DEV_REVISION:
		val = SC8541_DEVICE_ID;
		break;
	case CP_DEV_OVPGATE:
		val = mtk_sc8541_get_ovpgate_state(sc);
		break;
	case CP_DEV_WPCGATE:
		val = mtk_sc8541_get_wpcgate_state(sc);
		break;
	case CP_DEV_WORKMODE:
		val = mtk_sc8541_get_mode_state(sc);
		break;
	default:
		break;
	}
...
static int mtk_sc8541_dump_registers(struct charger_device *chg_dev)
{
    int ret;
    int i;
    int val;
...
static const struct charger_ops sc8541_chg_ops = {
	 ...
	 .dump_registers = mtk_sc8541_dump_registers,
	 .get_cp_status	= mtk_sc8541_get_cp_status,
};
#endif /*CONFIG_MTK_CLASS*/
...
__maybe_unused
static void sc8541_dump_check_fault_status(struct sc8541_chip *sc)
{
    int ret;
    u8 flag = 0;
    int i,j,k;
    ...
    for (i = 0; i <= SC8541_REGMAX; i++) {
        ret = sc8541_read_block(sc, i, &flag, 1);
        dev_err(sc->dev, "%s reg[0x%02x] = 0x%02x\n", __func__, i, flag);
...
static irqreturn_t sc8541_irq_handler(int irq, void *data)
{
    ...
    sc8541_dump_check_fault_status(sc);
...
static int sc8541_parse_dt(struct sc8541_chip *sc, struct device *dev)
{
    ...
    sc->en_adc_ctl = of_property_read_bool(np, "sc,sc8541,en_adc_ctl");
    dev_err(sc->dev, "en adc ctl: %d\n", sc->en_adc_ctl);
                    ...
                    const struct i2c_device_id *id)
{
...
#ifdef CONFIG_SOUTHCHIP_DVCHG_CLASS
    sc->charger_pump = dvchg_register(sc->mode == SC8541_STANDALONG 
        ? "sc_master_dvchg" : (sc->mode == SC8541_MASTER ? "sc_master_dvchg" : "sc_slave_dvchg"),
	...
	memset(slave_charger_info_1, 0, sizeof(slave_charger_info_1));
	memcpy(slave_charger_info_1, SC8541_NAME,
		strlen(SC8541_NAME) > (sizeof(slave_charger_info_1) - 1) ?
		(sizeof(slave_charger_info_1) - 1) : strlen(SC8541_NAME));
