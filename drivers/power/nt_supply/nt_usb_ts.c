...
#define DISCHG_TIMEOUT_MS	60000
...
struct mtk_usb_ts_info {
	...
	bool usb_dischg_timeout;
	struct tcpc_device *tcpc_dev;
	struct notifier_block pd_nb;
	struct power_supply *bat_psy;
	struct power_supply *chg_psy;
	struct delayed_work dischg_timeout_work;
	struct wakeup_source *usb_ts_wakelock;
	int chr_type;
	bool usb_ts_debug;
	bool is_fast_charge;
	bool is_aging_mode;
...
static void wake_up_usb_ts_check(struct mtk_usb_ts_info *ts_info, bool en)
{
	...
	chr_err("set usb check state:%d\n", atomic_read(&ts_info->charger_state));
}
...
static int usb_ts_ctrl_power_off(void)
{
	...
	chr_err("set usb vbus off successful!\n");
#else
	chr_err("factory version, keep vbus online\n");
#endif
...
static void usb_ts_dischg_action(struct mtk_usb_ts_info *ts_info)
{
	...
	if ((ts_info->is_need_disable_usb == false) && (!ts_info->is_aging_mode)) {
		...
		if (ret) {
			...
			ts_info->usb_dischg_timeout = false;
			cancel_delayed_work(&ts_info->dischg_timeout_work);
			if (!ts_info->usb_ts_wakelock->active)
				__pm_stay_awake(ts_info->usb_ts_wakelock);
			schedule_delayed_work(&ts_info->dischg_timeout_work, msecs_to_jiffies(DISCHG_TIMEOUT_MS));
...
static void usb_ts_enchg_action(struct mtk_usb_ts_info *ts_info)
{
	...
	if (!ts_info->is_aging_mode) {
		...
		if (ret) {
			...
			ts_info->usb_dischg_timeout = false;
			cancel_delayed_work(&ts_info->dischg_timeout_work);
			__pm_relax(ts_info->usb_ts_wakelock);
			ts_info->usb_ts_state = USB_TS_TEMP_NORMAL;
	...
	if ((ts_info->is_need_disable_usb == true) &&
		...
		do {
			...
			count_num++;
						...
						|| (ts_info->usb_dischg_timeout == true)));
		...
		} else if (ts_info->usb_dischg_timeout == true) {
			usb_ts_enchg_action(ts_info);
			pr_err("usb ts temp recover, reason: dischg timeout\n"); //recover
		} else {
			usb_ts_enchg_action(ts_info);
...
static void update_usb_ts_current_status(struct mtk_usb_ts_info *ts_info)
{
	...
	if ((ts_info->is_need_disable_usb == true) &&
			(ts_info->is_cool_down == true || ts_info->usb_dischg_timeout == true) &&
			(ts_info->usb_ts_state == USB_TS_TEMP_INCREASE_FAST)) {
...
static void dischg_timeout_work(struct work_struct *work)
{
	struct mtk_usb_ts_info *ts_info = container_of(work,
			struct mtk_usb_ts_info, dischg_timeout_work.work);

	if ((ts_info->is_need_disable_usb == true)
			&& (ts_info->usb_ts_state != USB_TS_TEMP_NORMAL))
	{
		pr_err("set timeout flag\n");
		ts_info->usb_dischg_timeout = true;
	}
}
	...
	while (!kthread_should_stop()) {
		...
		if (is_temperature_normal(ts_info) && ts_info->temp_parameter.enable) {
			...
			if ((ts_info->temp_parameter.usb_temp_print_log_enable == 1)
				|| (ts_info->usb_ts_state != USB_TS_TEMP_NORMAL)) {
				chr_err("ts v:%d mv, usb temp:%d, need dis:%d, bat temp:%d, usb in:%d TS state:%d, timeout:%d\n",
					ts_info->usb_ts_voltage / 1000, ts_info->usb_temp, ts_info->is_need_disable_usb,
					ts_info->bat_temp, ts_info->is_usb_in, ts_info->usb_ts_state, ts_info->usb_dischg_timeout);
			}
...
static int mtk_usb_ts_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&ts_info->dischg_timeout_work, dischg_timeout_work);
	name = devm_kasprintf(&pdev->dev, GFP_KERNEL, "%s",	"nt_usb ts wakelock");
	ts_info->usb_ts_wakelock = wakeup_source_register(NULL, name);
	ts_info->chg1_dev = get_charger_by_name("primary_chg");
	if (ts_info->chg1_dev) {
