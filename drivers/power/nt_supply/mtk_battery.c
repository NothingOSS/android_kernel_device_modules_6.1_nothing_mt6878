...
#define BATTERY_NAME_1 "PSL466689D-1"
#define BATTERY_NAME_2 "MFP-466588-001"
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		val->intval = (gm->precise_soc + 50) / 100;
		break;
...
static bool mtk_battery_get_charging_by_uisoc(void)
{
	...
	if (nt_chg->info)
		return nt_chg->info->charging_by_uisoc;

	return false;
}

static int mtk_battery_get_area_id(void)
{
	static struct nt_chg_info *nt_chg = NULL;
	struct power_supply *psy;
...
static void mtk_battery_external_power_changed(struct power_supply *psy)
{
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		if (!online.intval) {
			...
			if ((mtk_battery_get_area_id() == 3)
					&& (mtk_battery_get_charging_by_uisoc() == true))
				full_flag = true;
			if (full_flag)
...
#define BAT_ID_CHECK_TH 750
void fg_check_bat_id(struct platform_device *dev,
	struct mtk_battery *gm)
{
	int val = 0;
	int ret = 0;

	gm->chan_bat_id = devm_iio_channel_get(&dev->dev, "bat_id");
	if (IS_ERR(gm->chan_bat_id)) {
		ret = PTR_ERR(gm->chan_bat_id);
		if (ret != -EPROBE_DEFER)
			bm_err("IIO channel not found: %d\n", ret);
		gm->chan_bat_id = NULL;
		return;
	}

	ret = iio_read_channel_processed(gm->chan_bat_id, &val);
	if (ret < 0) {
		bm_err("IIO bat_id channel read failed %d\n", ret);
		return;
	}

	if (val < BAT_ID_CHECK_TH)
		gm->battery_id = 0;
	else
		gm->battery_id = 1;
	bm_err("[%s] init battery id val:%d %d\n",
		__func__, gm->battery_id, val);
}
...
int battery_init(struct platform_device *pdev)
{
	...
	gm->nt_bat_health = FAKE_BATT_MAGIC;
	...
	/* Get the project id */
	if (!of_property_read_u32(pdev->dev.of_node, "project-id",
				&gm->prj_id))
		bm_err("%s: project-id=%d", __func__, gm->prj_id);
	else
		gm->prj_id = 0;

	if (gm->prj_id == 4)
		fg_check_bat_id(pdev, gm);
	...
	if (gm->prj_id == 4) {
		if (gm->battery_id > 0)
			memcpy(bat_info, BATTERY_NAME_2,
				strlen(BATTERY_NAME_2) > (sizeof(bat_info) - 1) ?
				(sizeof(bat_info) - 1) : strlen(BATTERY_NAME_2));
		else
			memcpy(bat_info, BATTERY_NAME_1,
				strlen(BATTERY_NAME_1) > (sizeof(bat_info) - 1) ?
				(sizeof(bat_info) - 1) : strlen(BATTERY_NAME_1));
	}
