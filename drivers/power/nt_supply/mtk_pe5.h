...
struct pe50_algo_desc {
	...
	int tbat_t4_to_t5_cv[PE50_JEITA_MAX];		/* BAT T4~T5 CV level */
	int tbat_t4_to_t5_curlmt[PE50_JEITA_MAX];	/* BAT T4~T5 CV current limit */
...
extern int pe50_hal_set_ieoc(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA);
extern int pe50_hal_get_jeita_state(struct chg_alg_device *alg);
extern bool pe50_hal_get_nt_jeita_enable(struct chg_alg_device *alg);
#endif /* __MTK_PE5_H */
