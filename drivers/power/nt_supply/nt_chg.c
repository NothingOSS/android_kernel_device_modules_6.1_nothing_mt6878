...
#define NT_ADP_POWER_VID_CHECK 0x2E87
#define NT_ADP_VERSION_CHECK	0x99
extern int g_nt_adp_power_vid;
extern int g_nt_adp_power_pid;
extern int g_nt_adp_ver_fw;
...
static int check_chg_status(struct nt_chg_info *nci)
{
	...
	if (vchr > (nci->is_hvcharger ? HVDCP_SW_VBUSOV_UV : (is_hvcharger_detect ? HVDCP_SW_VBUSOV_UV : info->data.max_charger_voltage))) {
		...
		msleep(200);
		charger_dev_get_vbus(info->chg1_dev, &vchr);
		pr_err("[%s]vbus : retry %d\n", __func__, vchr);
		if (vchr <= info->data. min_charger_voltage)
...
*/
	...
	}else if (nci->cmd_discharging != true){
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			nci->pre_aicl_limmit = -1;
			nci->count = 0;
			g_nt_adp_power_vid = 0;
			g_nt_adp_power_pid = 0;
			g_nt_adp_ver_fw = 0;
			g_nt_adp_ver_hw = 0;
		}
		break;
	...
	const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if(sscanf(desc, "%d", &cmd_discharging) == 1){
		cmd_discharging = !cmd_discharging;
...
static int usb_real_type_proc_show(struct seq_file *m, void *v)
{
	...
	if ((nci->is_hvcharger) && (type == POWER_SUPPLY_USB_TYPE_DCP))
		seq_printf(m, "%s\n", "HVDCP");
	else
...
static int typec_cc_orientation_proc_show(struct seq_file *m, void *v)
{
	...
	if (!tcpm_inquire_typec_attach_state(nci->tcpc_dev))
		ret = -1;
...
static int nt_adp_power_proc_show(struct seq_file *m, void *v)
{
	struct nt_chg_info *nci = m->private;
	bool nt_adp = false;
	...
	pr_info("%s: power_vid: 0x%04x, power_pid: 0x%04x, hw_ver: 0x%x, fw_ver: 0x%x\n", __func__,
		g_nt_adp_power_vid, g_nt_adp_power_pid, g_nt_adp_ver_hw, g_nt_adp_ver_fw);

	if ((g_nt_adp_power_vid == NT_ADP_POWER_VID_CHECK) ||
	    ((g_nt_adp_ver_hw == g_nt_adp_ver_fw) && (g_nt_adp_ver_fw == NT_ADP_VERSION_CHECK))) {
		nt_adp = true;
	}

	seq_printf(m, "%d\n", nt_adp ? g_nt_adp_power_pid : 0);

	return 0;
}

PROC_FOPS_RO(nt_adp_power);
...
static int battery_health_proc_show(struct seq_file *m, void *v)
{
	...
	if(gaugepsy) {
			...
			if (gauge) {
				gm = gauge->gm;
				ret = gm->nt_bat_health / 100;
			}
	}
	pr_info("%s: bat_health = %d!\n", __func__, ret);
	seq_printf(m, "%d\n", ret);
	return 0;
}
PROC_FOPS_RO(battery_health);
...
static int ui_soc_proc_show(struct seq_file *m, void *v)
{
	struct nt_chg_info *nci = m->private;
	if(nci){
		pr_info("%s: ui_soc_show %d ma\n",__func__, nci->ui_soc);
		seq_printf(m, "%d\n", nci->ui_soc);
	}
	return 0;
}
static ssize_t ui_soc_proc_write(struct file *file,
	const char __user *buffer, size_t count, loff_t *pos)
{
	int ui_soc = 0;
	int len = 0;
...
/*static void nt_charger_start_timer(struct nt_chg_info *nci)
{
	...
	pr_info("%s: alarm timer start [%d][%d]: %lld %ld\n", __func__,nci->typec_attach,
...
static void check_battery_psy_change_status(struct nt_chg_info *nci, bool need_updated)
{
	struct power_supply *batpsy = power_supply_get_by_name("battery");
	...
	if (need_updated) {
		...
		pm_wakeup_dev_event(nci->dev, 500, true);
	}
}
...
static void nt_update_status_function_work(struct work_struct *work)
{
	...
	int pump = 0, htemp_charge = 0,battTemp = 0, battVol = 0, plugGpio = 0,chg_status = 0,AICR = 0,CV = 0, MIVR = 0, ICHG  = 0;
	char BattInit = 1, soc_time = 0;
	static int soc = 0, soc_pre = 0;
	bool pump_en = false, chg_en = false,boost_en = false,ovp = false, wpc = false, wls_sus = false, need_update = false;
	...
	//while (1) {
		...
		battVol = chg_check_battery_info(nci,POWER_SUPPLY_PROP_VOLTAGE_NOW);
		...
		need_update = (battVol < LOW_BAT_THR) || (battTemp >= HIGH_TEMP_THR) || (battTemp <= LOW_TEMP_THR) || (soc_pre != soc) || (nci->ui_soc != soc);
		check_battery_psy_change_status(nci, need_update);
		scnprintf(keyinfo_buffer_temp,DUMP_MSG_BUF_SIZE - 1,"Pump:0x%x,pump_en:%d,ovp:%d,wpc:%d, fcc:%d,fv:%d,icl:%d,ibus_ma:%d,usb_sus:%d,wls_sus:%d,BattInit:%d, chg_type:%d,soc:%d,soc_time:%d,soc_pre:%d,real_cap:%d,htemp_charge:%d,plug_in:%d,plugGpio:%d,usbTemp:%d,battTemp:%d,CHG_EN:%x,CHG_STATUS:%x,AICL:%d,AICR:%d,CV:%d,MIVR:%d,ICHG:%d,BOOST_CV:%d,BOOST_EN:%d,BOOST_CC:%d,aging_mode:%d,NT_CHG_TYPE:%s,CAM_LMT:{[ON_OFF]%d,[HV_CHG]%d,[LMT_C]%d,hvcharger:%d} \n",\
			...
			nci->cam_on_off,nci->info->enable_hv_charging,nci->cam_lmt,nci->is_hvcharger);
		pr_info("%s \n",keyinfo_buffer_temp);
...
static int nt_chg_probe(struct platform_device *pdev)
{
	...
	/* Get the project id */
	if (!of_property_read_u32(nci->dev->of_node, "project-id",
				&nci->prj_id))
		pr_info("%s: project-id=%d", __func__, nci->prj_id);
	else
		nci->prj_id = PROJECT_DATA_ID;
...
//	kthread_run(nt_charger_routine_thread, nci, "nt_chg_thread");
	INIT_DELAYED_WORK(&nci->nt_update_status_work, nt_update_status_function_work);
	queue_delayed_work(system_wq, &nci->nt_update_status_work,
								round_jiffies(10 * HZ));
...
static int nt_chg_resume(struct device *dev)
{
	struct nt_chg_info *nci = dev_get_drvdata(dev);
...
static SIMPLE_DEV_PM_OPS(nt_chg_pm_ops, nt_chg_suspend, nt_chg_resume);
