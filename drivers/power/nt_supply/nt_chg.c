...
static int nt_adp_power_proc_show(struct seq_file *m, void *v)
{
	...
	pr_info("%s: g_nt_adp_power_vid: %d, g_nt_adp_power_pid: %d\n", __func__,
		g_nt_adp_power_vid, g_nt_adp_power_pid);

	seq_printf(m, "%d\n", (g_nt_adp_power_vid == NT_ADP_POWER_VID_CHECK) ? g_nt_adp_power_pid : 0);
...
static int nt_charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		need_update = (battVol < LOW_BAT_THR) || (battTemp >= HIGH_TEMP_THR) || (battTemp <= LOW_TEMP_THR) || (soc_pre != soc);
