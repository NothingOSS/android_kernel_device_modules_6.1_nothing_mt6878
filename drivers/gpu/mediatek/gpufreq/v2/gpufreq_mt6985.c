...
void __gpufreq_dump_infra_status(void)
{
	...
	GPUFREQ_LOGI("%-11s (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x",
		...
		0x1002C1A4, readl(IFR_MFGSYS_PROT_EN_W1S_0),
		0x1002C1A8, readl(IFR_MFGSYS_PROT_EN_W1C_0),
	...
	unsigned int vsram_old, unsigned int vsram_new)
{
	 ...
	 * 3. Assume Vgpu always >= Vstack at same OPP idx.
	 */

	/* scale-up: Vstack -> (Vsram) -> Vgpu -> Freq */
	if (vstack_new > vstack_old) {
		/* Volt */
		while (vstack_new != vstack_old) {
			/* config DVFS SEL */
			...
			target_vsram = __gpufreq_get_vsram_by_vlogic(target_vgpu);
		...
		if (unlikely(ret))
			goto done;
	/* scale-down: Freq -> Vgpu -> (Vsram) -> Vstack */
	} else if (vstack_new < vstack_old) {
		/* Freq */
		ret = __gpufreq_freq_scale_gpu(fgpu_old, fgpu_new);
		if (unlikely(ret))
			goto done;
	...
	unsigned int target_volt, enum gpufreq_dvfs_state key)
{
	int ret = GPUFREQ_SUCCESS;
	/* GPU & STACK OPP are 1-1 mapping */
	int cur_oppidx = 0,  target_oppidx = 0, opp_num = g_stack.opp_num;
	/* GPU */
	struct gpufreq_opp_info *working_gpu = g_gpu.working_table;
	unsigned int cur_fgpu = 0, cur_vgpu = 0, target_fgpu = 0, target_vgpu = 0;
	/* STACK */
...
static unsigned int __gpufreq_calculate_pcw(unsigned int freq, enum gpufreq_posdiv posdiv)
{
	...
	if ((freq >= POSDIV_16_MIN_FREQ) && (freq <= POSDIV_2_MAX_FREQ)) {
		...
		GPUFREQ_LOGE("out of range Freq: %d", freq);
	}
...
static void __gpufreq_dump_bringup_status(struct platform_device *pdev)
{
	...
	GPUFREQ_LOGI("[GPU] MALI_GPU_ID: 0x%08x, SPM_SPM2GPUPM_CON: 0x%08x",
		readl(MALI_GPU_ID),
		readl(SPM_SPM2GPUPM_CON));

	GPUFREQ_LOGI("[GPU] MFG_0_19_PWR_STATUS: 0x%08x, MFG_RPC_MFG1_PWR_CON: 0x%08x",
		MFG_0_19_PWR_STATUS,
...
static unsigned int __gpufreq_get_fmeter_main_fgpu(void)
{
	...
	writel(0x00FF0000, MFG_PLL_FQMTR_CON1);
	val = readl(MFG_PLL_FQMTR_CON0);
	writel((val & 0x00FFFFFF), MFG_PLL_FQMTR_CON0);
	writel(0x00009000, MFG_PLL_FQMTR_CON0);

	ckgen_load_cnt = readl(MFG_PLL_FQMTR_CON1) >> 16;
	ckgen_k1 = readl(MFG_PLL_FQMTR_CON0) >> 24;

	val = readl(MFG_PLL_FQMTR_CON0);
	writel((val | 0x1010), MFG_PLL_FQMTR_CON0);

	/* wait fmeter finish */
	while (readl(MFG_PLL_FQMTR_CON0) & 0x10) {
		udelay(10);
		i++;
		if (i > 1000) {
	...
	val = readl(MFG_PLL_FQMTR_CON1) & 0xFFFF;
	/* KHz */
...
static unsigned int __gpufreq_get_fmeter_main_fstack(void)
{
	...
	writel(0x00FF0000, MFGSC_PLL_FQMTR_CON1);
	val = readl(MFGSC_PLL_FQMTR_CON0);
	writel((val & 0x00FFFFFF), MFGSC_PLL_FQMTR_CON0);
	writel(0x00009000, MFGSC_PLL_FQMTR_CON0);

	ckgen_load_cnt = readl(MFGSC_PLL_FQMTR_CON1) >> 16;
	ckgen_k1 = readl(MFGSC_PLL_FQMTR_CON0) >> 24;

	val = readl(MFGSC_PLL_FQMTR_CON0);
	writel((val | 0x1010), MFGSC_PLL_FQMTR_CON0);

	/* wait fmeter finish */
	while (readl(MFGSC_PLL_FQMTR_CON0) & 0x10) {
		udelay(10);
		i++;
		if (i > 1000) {
	...
	val = readl(MFGSC_PLL_FQMTR_CON1) & 0xFFFF;
	/* KHz */
	freq = (val * 26000 * (ckgen_k1 + 1)) / (ckgen_load_cnt + 1);
	...
	u32 *a_t0_efuse2, u32 *a_t0_efuse3, u32 *efuse_error)
{
	...
	/* efuse1 address: 0x11E805CC */
	efuse1 = readl(EFUSE_ASENSOR_RT);
	/* efuse2 address: 0x11E805D0 */
	efuse2 = readl(EFUSE_ASENSOR_HT);
	/* efuse3 address: 0x11E805DC */
	...
	*a_t0_efuse1 = (efuse1  & GENMASK(9, 0)) + 0x5DC;
	/* A_T0_LVT_RT: 0x11EE0AA8 [19:10], shift value: 0x4B0 */
	*a_t0_efuse2 = ((efuse1 & GENMASK(19, 10)) >> 10) + 0x4B0;
	/* A_T0_ULVT_RT: 0x11EE0AA8 [29:20], shift value: 0x708 */
	*a_t0_efuse3 = ((efuse1 & GENMASK(29, 20)) >> 20) + 0x708;
	/* efuse_error   : 0x11EE0AA8 [31] */
	...
	u32 *a_tn_sensor2, u32 *a_tn_sensor3)
{
	 ...
	 * MFG_SENSOR_BCLK_CG 0x13FBFF98 = 0x1
	 */
	writel(0x1, MFG_SENSOR_BCLK_CG);
	/*
	 * Config window setting
	 * CPE_CTRL_MCU_REG_CPEMONCTL 0x13FB9C00 = 0x0901031F
	 */
	writel(0x0901031F, MFG_CPE_CTRL_MCU_REG_CPEMONCTL);
	/*
	 * Enable CPE
	 * CPE_CTRL_MCU_REG_CEPEN 0x13FB9C04 = 0xFFFF
	 */
	...
	/*
	 * Readout the data
	 * MFG_ASENSORDATA0 0x13FB6008
	 * MFG_ASENSORDATA1 0x13FB600C
	 */
	...
	/* A_TN_ULVTLL_CNT 0x13FB6008 [15:0] */
	*a_tn_sensor1 = (aging_data1 & GENMASK(15, 0));
	/* A_TN_LVT_CNT 0x13FB6008 [31:16] */
	*a_tn_sensor2 = (aging_data1 & GENMASK(31, 16)) >> 16;
	/* A_TN_ULVT_CNT 0x13FB600C [15:0] */
	*a_tn_sensor3 = (aging_data2 & GENMASK(15, 0));
...
static unsigned int __gpufreq_compute_avs_volt(u32 val)
{
	...
	volt |= (val & GENMASK(3, 0)) >> 14; /* Get volt[3:0] from efuse[17:14] */
	volt |= (val & GENMASK(5, 4));       /* Get volt[5:4] from efuse[5:4]   */
	volt |= (val & GENMASK(7, 6)) << 4;  /* Get volt[7:6] from efuse[3:2]   */
