...
//#include "../mediatek/mediatek_v2/mtk_corner_pattern/panel-rm69220-vxn.h"
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx,  0x2F, 0x07);		//60hz
	...
	lcm_dcs_write_seq_static(ctx, 0x4F, 0xbb);
	lcm_dcs_write_seq_static(ctx, 0x50, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x51, 0x07);
	lcm_dcs_write_seq_static(ctx, 0x53, 0x0c);
	lcm_dcs_write_seq_static(ctx, 0x54, 0x0d);
	lcm_dcs_write_seq_static(ctx, 0x55, 0xb7);
	lcm_dcs_write_seq_static(ctx, 0x56, 0x0c);
	lcm_dcs_write_seq_static(ctx, 0x58, 0xb7);
...
static int panel_ata_check(struct drm_panel *panel)
{
	...
	unsigned char id[3] = {0x13, 0x21, 0x01};
	ssize_t ret;
	...
	void *dsi, dcs_write_gce cb, void *handle)
{
	...
	for (i = 0; i < (sizeof(lcm_aod_to_normal) /
		...
		case REGFLAG_DELAY:
			...
			break;
		case REGFLAG_UDELAY:
			usleep_range((lcm_aod_to_normal[i].count) * 1000, (lcm_aod_to_normal[i].count + 10) * 1000);
			break;
...
static struct mtk_panel_params ext_params_60hz = {
	...
	.dyn_fps = {
		...
		.vact_timing_fps = 60,
		.dfps_cmd_table[0] = {0, 2, {0xFE, 0x00} },
		.dfps_cmd_table[1] = {0, 2, {0x2F, 0x07} },
	},
};
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	snprintf(panel_name_find, sizeof(panel_name_find), "rm69220_vxn");
