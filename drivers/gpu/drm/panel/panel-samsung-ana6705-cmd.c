...
static struct drm_display_mode wqhd_90_mode = {
	.clock		= 437870, //90Hz
	.hdisplay	= HACT,
	.hsync_start	= HACT + HFP,
	.hsync_end	= HACT + HFP + HSA,
...
static struct drm_display_mode wqhd_72_mode = {
	.clock		= 349807, //72Hz
	.hdisplay	= HACT,
	.hsync_start	= HACT + HFP,
	.hsync_end	= HACT + HFP + HSA,
...
static struct drm_display_mode wqhd_60_mode = {
	.clock		= 291098, //60Hz
	.hdisplay	= HACT,
	.hsync_start	= HACT + HFP,
	.hsync_end	= HACT + HFP + HSA,
...
static struct mtk_panel_params ext_params_wqhd = {
	.data_rate = PLL_CLOCK * 2,
	.pll_clk = PLL_CLOCK,
	.cust_esd_check = 0,
	.esd_check_enable = 1,
...
static void mode_switch_to_wqhd_72(struct drm_panel *panel,
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	struct lcm *ctx = panel_to_lcm(panel);
...
static void mode_switch_to_wqhd_90(struct drm_panel *panel,
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	struct lcm *ctx = panel_to_lcm(panel);
...
static void mode_switch_to_wqhd_120(struct drm_panel *panel,
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	struct lcm *ctx = panel_to_lcm(panel);
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	switch (pmode_id) {
	case 0:
		mode_switch_to_wqhd_120(panel, stage);
		break;
	case 1:
		mode_switch_to_wqhd_90(panel, stage);
		break;
	case 2:
		mode_switch_to_wqhd_72(panel, stage);
		break;
	case 3:
			...
			struct drm_connector *connector)
{
	...
	ctx->pmode_id = 0;
	list_add_tail(&default_mode.head, &ctx->probed_modes);

	mode1 = drm_mode_duplicate(connector->dev, &wqhd_90_mode);
	if (!mode1) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			wqhd_90_mode.hdisplay,
			wqhd_90_mode.vdisplay,
			drm_mode_vrefresh(&wqhd_90_mode));
		return -ENOMEM;
	}
	...
	list_add_tail(&wqhd_90_mode.head, &ctx->probed_modes);

	mode2 = drm_mode_duplicate(connector->dev, &wqhd_72_mode);
	if (!mode2) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			wqhd_72_mode.hdisplay,
			wqhd_72_mode.vdisplay,
			drm_mode_vrefresh(&wqhd_72_mode));
		return -ENOMEM;
	}
	...
	list_add_tail(&wqhd_72_mode.head, &ctx->probed_modes);

	mode3 = drm_mode_duplicate(connector->dev, &wqhd_60_mode);
	if (!mode3) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			wqhd_60_mode.hdisplay,
			wqhd_60_mode.vdisplay,
			drm_mode_vrefresh(&wqhd_60_mode));
		return -ENOMEM;
	}
	...
	list_add_tail(&wqhd_60_mode.head, &ctx->probed_modes);


	mode4 = drm_mode_duplicate(connector->dev, &fhd_120_mode);
	if (!mode4) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
	...
	mode5 = drm_mode_duplicate(connector->dev, &fhd_60_mode);
	if (!mode5) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			fhd_60_mode.hdisplay,
			fhd_60_mode.vdisplay,
