...
static int g_last_hbm_status = 0;
void mtk_lcm_update_state_to_fingerprint(int hbm_status)
{
	bool is_status_chg = 0;
	struct fp_notify_event event;

	if (g_last_hbm_status != hbm_status) {
		printk("[%s] last_hbm_status:%d change to new_hbm_status:%d\n", __func__,
			g_last_hbm_status,
			hbm_status);
		g_last_hbm_status = hbm_status;
		is_status_chg = 1;
	}
...
{
	mtk_lcm_update_state_to_fingerprint(hbm_status);
}
EXPORT_SYMBOL(mtk_panel_proc_local_hbm);
