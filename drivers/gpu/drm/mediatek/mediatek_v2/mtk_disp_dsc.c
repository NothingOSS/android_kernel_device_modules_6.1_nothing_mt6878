		...
		if (dsc->data->need_obuf_sw && enc_pic_width < 1440) {
			if (dsc->data->decrease_outstream_buf)
				mtk_ddp_write_relaxed(comp,
					0x80000200, DISP_REG_DSC_OBUF + DISP_REG_DSC1_OFFSET, handle);
			else
				mtk_ddp_write_relaxed(comp,
		...
		if (dsc_params->ext_pps_cfg.enable) {
			if (rc_buf_thresh) {
				for (i = 0; i < RC_BUF_THRESH_NUM/4; i++) {
					reg_val = 0;
					reg_val += (*(rc_buf_thresh+i*4+0))<<0;
					reg_val += (*(rc_buf_thresh+i*4+1))<<8;
					reg_val += (*(rc_buf_thresh+i*4+2))<<16;
					reg_val += (*(rc_buf_thresh+i*4+3))<<24;
