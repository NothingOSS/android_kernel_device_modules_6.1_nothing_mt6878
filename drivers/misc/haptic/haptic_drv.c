...
unsigned int is_x_axis_motor = false;
EXPORT_SYMBOL(is_x_axis_motor);
	...
	struct file *filp, const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (is_x_axis_motor)
	{
		if (abs((int32_t)haptic_data->nt_cmdline_f0 - DEFAULT_X_F0) > 150){
			haptic_data->f0 = haptic_data->nt_backup_f0;
			pr_info("%s() is_x_axis_motor haptic_data->nt_cmdline_f0 = %u, haptic_data->f0 = nt_backup_f0 = %u", __func__, haptic_data->nt_cmdline_f0, haptic_data->f0);
		}
	}
	else
	{
		if (abs((int32_t)haptic_data->nt_cmdline_f0 - DEFAULT_F0) > 150){
			haptic_data->f0 = haptic_data->nt_backup_f0;
			pr_info("%s() no is_x_axis_motor haptic_data->nt_cmdline_f0 = %u, haptic_data->f0 = nt_backup_f0 = %u", __func__, haptic_data->nt_cmdline_f0, haptic_data->f0);
		}
	}
...
static void load_chip_config(const struct firmware *config_fw, void *context)
{
	...
	ics_info("chip config firmware size = %zu\n", config_fw->size);
...
static int ics_haptic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = of_property_read_u32(dev->of_node, "x-axis-motor", &is_x_axis_motor);
	if (ret)
		ics_err("%s: x-axis-motor = %d\n", __func__, is_x_axis_motor);
