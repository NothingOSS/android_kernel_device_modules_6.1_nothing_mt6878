...
extern unsigned int is_x_axis_motor;
...
const int8_t rt6010_f0_x_wave_data[] =
{
	0,  27,  54,  77,  97, 113, 123, 126, 124, 116,
	102,  83,  61,  35,   7, -19, -46, -71, -92,-109,
	-120,-126,-125,-119,-107, -89, -68, -43, -15,  11,
	39,  64,  86, 105, 118, 125, 126, 121, 111,  95,
	74,  50,  23,  -3, -31, -57, -80,-100,-114,-123,
	-127,-123,-114,-100, -80, -57, -31,  -3,  23,  50,
	74,  95, 111, 121, 126, 125, 118, 105,  86,  64,
	39,  11, -15, -43, -68, -89,-107,-119,-125,-126,
	-120,-109, -92, -71, -46, -19,   7,  35,  61,  83,
	102, 116, 124, 126, 123, 113,  97,  77,  54,  27,
	0, -27, -54, -77, -97,-113,-123,-126,-124,-116,
	-102, -83, -61, -35,  -7,  19,  46,  71,  92, 109,
	120, 126, 125, 119, 107,  89,  68,  43,  15, -11,
	-39, -64, -86,-105,-118,-125,-126,-121,-111, -95,
...
static int rt6010_private_init(struct ics_haptic_data *haptic_data, struct ics_haptic_chip_config *chip_config_data)
{
	...
	if (is_x_axis_motor)
	{
		if (abs((int32_t)haptic_data->f0 - DEFAULT_X_F0) > 150)
		{
			haptic_data->f0 = DEFAULT_X_F0;
			ics_dbg("%s() is_x_axis_motor = %d", __func__, is_x_axis_motor);
		}
	}
	else
	{
		if (abs((int32_t)haptic_data->f0 - DEFAULT_F0) > 150)
		{
			...
			ics_dbg("%s() no is_x_axis_motor = %d", __func__, is_x_axis_motor);
		}
	}
...
static int32_t rt6010_get_f0_by_bemf(struct ics_haptic_data *haptic_data)
{
	...
	if (is_x_axis_motor)
	{
		ics_dbg("%s() is_x_axis_motor = %d, reg_val = 0x34", __func__, is_x_axis_motor);
		reg_val = 0x34;
	}
	else
	{
		ics_dbg("%s() no is_x_axis_motor = %d, reg_val = 0x50", __func__, is_x_axis_motor);
		reg_val = 0x50;
	}
	ret = regmap_write(haptic_data->regmap, RT6010_REG_GAIN_CFG, reg_val);
	check_error_return(ret);
	...
	if (is_x_axis_motor)
	{
		ics_dbg("%s() is_x_axis_motor = %d, use rt6010_f0_x_wave_data", __func__, is_x_axis_motor);
		rt6010_set_stream_data(haptic_data, (uint8_t*)rt6010_f0_x_wave_data, sizeof(rt6010_f0_x_wave_data));
	}
	else
	{
		ics_dbg("%s() no is_x_axis_motor = %d, use rt6010_f0_wave_data", __func__, is_x_axis_motor);
...
static int32_t rt6010_get_lra_resistance_ex(struct ics_haptic_data *haptic_data, int32_t voltage_mode)
{
	...
	if( is_x_axis_motor )
	{
		...
		ics_dbg("is x_axis_motor = %d ntc = cam_ntc", is_x_axis_motor);
	}
	else
	{
		tzd = thermal_zone_get_zone_by_name("sc_buck_ntc");
		ics_dbg("not is x_axis_motor = %d ntc = sc_buck_ntc", is_x_axis_motor);
	}
