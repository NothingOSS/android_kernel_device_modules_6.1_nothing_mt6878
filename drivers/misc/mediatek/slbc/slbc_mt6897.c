...
static int slbc_request_check(struct slbc_data *d)
{
	...
	slbc_uid_used = slbc_sram_read(SLBC_UID_USED);
	if (BIT_IN_MM_BITS_3(_BIT_(uid)) &&
		BIT_IN_MM_BITS_3(slbc_uid_used)) {
...
static int slbc_release_check(struct slbc_data *d)
{
	...
	slbc_uid_used = slbc_sram_read(SLBC_UID_USED);
	if (BIT_IN_MM_BITS_3(_BIT_(uid)) &&
		BIT_IN_MM_BITS_3(slbc_uid_used & ~_BIT_(uid))) {
		ret = UID_MM_BITS_3 & ~_BIT_(uid);
	}
...
static void slbc_deactivate_timer_fn(struct timer_list *timer)
{
	...
	SLBC_TRACE_REC(LVL_NORM, TYPE_B, 0, 0,
			"slbc_sid_rel_q %lx, slbc_uid_timeout %lx",
			slbc_sid_rel_q, slbc_uid_timeout);
	...
	if (slbc_sid_rel_q) {
		...
		for (sid = 0; sid < ARRAY_SIZE(p_config); sid++) {
			...
			if (test_bit(sid, &slbc_sid_rel_q)) {
				...
				trace_slbc_api((void *)__func__, slbc_uid_str[uid]);
#endif /* SLBC_TRACE */
				SLBC_TRACE_REC(LVL_QOS, TYPE_B, 0, 0,
						"%s", slbc_uid_str[uid]);
				...
				if (test_bit(uid, &slbc_uid_used)) {
					...
					SLBC_TRACE_REC(LVL_WARN, TYPE_B, 0, -1,
							"%s not released!", slbc_uid_str[uid]);
				} else {
					clear_bit(sid, &slbc_sid_rel_q);
					SLBC_TRACE_REC(LVL_QOS, TYPE_B, 0, 0,
							"slbc_sid_rel_q %lx", slbc_sid_rel_q);

					SLBC_TRACE_REC(LVL_QOS, TYPE_B, 0, 0,
							"%s released!", slbc_uid_str[uid]);
				}
			}
		}
	}
	...
	if (slbc_uid_timeout) {
		for (sid = 0; sid < ARRAY_SIZE(p_config); sid++) {
			...
			if (test_bit(uid, &slbc_uid_timeout))
				SLBC_TRACE_REC(LVL_ERR, TYPE_B, 0, -1,
						"%s slb req timeout!", slbc_uid_str[uid]);
		}
	}
...
static int slbc_activate_thread(void *arg)
{
	...
	if (ops->activate) {
		...
		if (ops->activate(d) == CB_DONE) {
			SLBC_TRACE_REC(LVL_ERR, TYPE_B, uid, -1,
...
static unsigned long slbc_find_next_low_used(struct slbc_data *d_old)
{
	...
	SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid_old, ret,
			"slbc_uid_used %lx", slbc_uid_used);

	/* check p_old have higher priority thans all user that have conflict sram */
	if (uid_check) {
		SLBC_TRACE_REC(LVL_WARN, TYPE_B, uid_old, ret,
				"%s block by high priority user %s",
				slbc_uid_str[uid_old], slbc_uid_str[uid_check]);
		return 0;
	}
...
#endif /* SLBC_TRACE */
			SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid_old, ret,
					"%s find low priority user %s",
					slbc_uid_str[uid_old], slbc_uid_str[uid]);
			if (ops->deactivate) {
#ifdef SLBC_CB_THREAD
				...
				SLBC_TRACE_REC(LVL_ERR, TYPE_B, uid_old, ret,
						"%s block by no CB user %s",
						slbc_uid_str[uid_old], slbc_uid_str[uid]);
				return 0;
			}
		}
						...
						unsigned long slbc_sid_req_list)
{
	...
	SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid_old, 0,
			"slbc_sid_req_list %lx", slbc_sid_req_list);
	...
	if (!res_ops) {
		SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid_old, 0,
				"%s no user found", slbc_uid_str[uid_old]);
		return NULL;
	}
...
#endif /* SLBC_TRACE */
	SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid_old, 0,
			"%s find user %s", slbc_uid_str[uid_old], slbc_uid_str[res_ops->data->uid]);
...
static int slbc_activate_check(void *arg)
{
	...
	if (uid <= UID_ZERO || uid >= UID_MAX) {
		SLBC_TRACE_REC(LVL_WARN, TYPE_B, uid, 0,
				"uid %x error!", uid);
		return 0;
	}
#ifdef SLBC_TRACE
...
#endif /* SLBC_TRACE */
	SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid, 0,
			"trigger by %s release", slbc_uid_str[uid]);
	...
	do {
		...
		if (!slbc_find_high_in_used(res_ops->data) &&
			...
			SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid, 0,
					"slbc_act_slot %x, %s res_slot %x",
					slbc_act_slot, slbc_uid_str[res_ops->data->uid],
					res_config->res_slot);
...
#endif /* SLBC_TRACE */
	SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid, 0,
			"trigger by %s request", slbc_uid_str[uid]);
...
int slbc_register_activate_ops(struct slbc_ops *u_ops)
{
	...
	if (sid == SID_NOT_FOUND) {
		SLBC_TRACE_REC(LVL_WARN, TYPE_B, u_ops->data->uid, -EFAULT,
				"register slbc ops fail: invalid uid %d!", u_ops->data->uid);
		return -EFAULT;
	}

	list_for_each_entry(check_ops, &slbc_ops_list, node) {
		if (u_ops->data->uid == check_ops->data->uid) {
			SLBC_TRACE_REC(LVL_QOS, TYPE_B, u_ops->data->uid, 0,
					"%s register slbc ops has been done",
					slbc_uid_str[u_ops->data->uid]);
			return 0;
		}
	}
	...
	if (u_ops->data->type == TP_BUFFER) {
		...
		if (!ops || !d) {
			SLBC_TRACE_REC(LVL_ERR, TYPE_B, u_ops->data->uid, -EFAULT,
					"%s register slbc ops fail: kmalloc error!",
					slbc_uid_str[u_ops->data->uid]);
			kfree(ops);
			kfree(d);
			return -EFAULT;
		...
		*ops = *u_ops;
		...
		SLBC_TRACE_REC(LVL_QOS, TYPE_B, u_ops->data->uid, 0,
				"%s register slbc ops success", slbc_uid_str[u_ops->data->uid]);
	} else {
		SLBC_TRACE_REC(LVL_WARN, TYPE_B, u_ops->data->uid, -EFAULT,
				"%s register slbc ops fail: type error!",
				slbc_uid_str[u_ops->data->uid]);
		return -EFAULT;
	}
#endif /* SLBC_CB */
...
static int slbc_request_buffer(struct slbc_data *d)
{
	...
	SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid, 0, "TP_BUFFER");
	...
	if (!SLBC_UID_VALID(uid))
		...
		d->config = &p_config[sid];
		config = (struct slbc_config *)d->config;
		SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid, 0, "%s req slb", slbc_uid_str[uid]);
#ifdef SLBC_CB
		set_bit(sid, &slbc_sid_req_q);
	...
	if (SLBC_UID_VALID(uid) && SLBC_SID_VALID(sid) &&
		...
		if (!d->timeout) {
			...
			if (deact_sid_list) {
				...
				ret = 0;
				time = ktime_get_ns();
				while ((ktime_get_ns() - time) < (d->timeout * 1000000)) {
					slbc_sspm_sram_update();
					if (!SLBC_SID_SRAM_IN_USED(sid) ||
							(deact_sid_list & slbc_sid_cb_fail)) {
						ret = 1;
				...
				SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid, ret,
						"%s stop to wait slb, ret %d",
						slbc_uid_str[uid], ret);
				if (ret) {
					slbc_sspm_sram_update();
					if (!SLBC_SID_SRAM_IN_USED(sid)) {
						SLBC_TRACE_REC(LVL_QOS, TYPE_B, uid, ret,
							"%s wait success", slbc_uid_str[uid]);
						ret = _slbc_request_buffer_scmi(d);
					} else {
						ret = -EREQ_WAIT_FAIL;
						SLBC_TRACE_REC(LVL_ERR, TYPE_B, uid, ret,
							"%s wait fail: %s %lx", slbc_uid_str[uid],
				...
				ret = -ENOT_AVAILABLE;
				SLBC_TRACE_REC(LVL_WARN, TYPE_B, uid, ret,
					"%s no need to wait slb release!", slbc_uid_str[uid]);
			}
		} else {
			ret = -ENOT_AVAILABLE;
			SLBC_TRACE_REC(LVL_WARN, TYPE_B, uid, ret,
					"%s need to wait slb release(invalid timeout:%u ms)!",
					slbc_uid_str[uid], d->timeout);
		}
	} else {
		ret = _slbc_request_buffer_scmi(d);
...
static int slbc_release_buffer(struct slbc_data *d)
{
	...
	SLBC_TRACE_REC(LVL_QOS, TYPE_B, d->uid, ret, "TP_BUFFER");
	slbc_sspm_sram_update();
#ifdef SLBC_CB
	sid = slbc_get_sid_by_uid(d->uid);
	slbc_uid_used = slbc_sram_read(SLBC_UID_USED);
	if (!test_bit(d->uid, &slbc_uid_used) &&
		...
		clear_bit(sid, &slbc_sid_req_q);
		SLBC_TRACE_REC(LVL_QOS, TYPE_B, d->uid, ret,
				"%s clr sid %d from slbc_sid_req_q %lx",
				slbc_uid_str[d->uid], sid, slbc_sid_req_q);
	}
#endif /* SLBC_CB */
...
static void slbc_dump_data(struct seq_file *m, struct slbc_data *d)
{
	...
	if (!SLBC_UID_VALID(uid)) {
		pr_info("slbc: invalid uid %d\n", uid);
		return;
	}

	if (uid >= sizeof(slbc_uid_used) * 8) {
		pr_info("slbc: uid size %d >= slbc_uid_used size %zu\n",
				uid, sizeof(slbc_uid_used) * 8);
		return;
	}
...
MODULE_DESCRIPTION("SLBC Driver mt6897 v0.1");
MODULE_LICENSE("GPL");
