		...
		unsigned int pkt_buf_sz)
{
	...
	if (g_page_pool_is_on) {
		if (atomic_read(&g_alloc_from_kernel) >= sysctl_devalloc_threshold)
			atomic_set(&g_alloc_from_kernel, 0);
		else
			atomic_inc(&g_alloc_from_kernel);
	}
#endif
	if (unlikely(!(*ppskb))) {
...
#ifdef RX_PAGE_POOL
		if (g_page_pool_is_on) {
			if (!atomic_read(&g_alloc_from_kernel)) {
...
#ifdef RX_PAGE_POOL
		if (g_page_pool_is_on && atomic_read(&g_create_another_pp) && !g_page_pool_arr[1]) {
			ccci_dpmaif_cma_mem_alloc(1);
			ccci_dpmaif_create_page_pool(1);
		}
#endif
