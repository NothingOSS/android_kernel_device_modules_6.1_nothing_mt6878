	...
	while (1) {
		...
		if (!ret) { /* timeout */
			...
			if (md_state == READY) {
				...
				/*
				 * mtk09077: q0.mp2 ccci_hif_ccif.c:
				...
				if (assert_md_type
					...
					ccci_md_dump_info(DUMP_FLAG_IRQ_STATUS, NULL, 0);
				ccci_md_dump_info(DUMP_FLAG_QUEUE_0, NULL, 0);
				ccci_md_force_assert(assert_md_type, NULL, 0);
