...
static int lpm_mbrain_get_sys_res_data(void *address, uint32_t size)
{
	...
	if (!address ||
		...
		pr_info("[name:spm&][SPM][Mbrain] mbrain address/buffer size too small\n");
		ret = -1;
	}
	...
	if (!sys_res_ops ||
		...
		pr_info("[name:spm&][SPM][Mbrain] Not get sys res operations\n");
		ret = -1;
	}
	...
	if (sys_res_update)
		pr_info("[name:spm&][SPM][Mbrain] SWPM data is not updated, ret=%d\n", sys_res_update);
		...
		uint32_t *threshold, uint32_t threshold_num)
{
	...
	for (i = 0; i < NR_SPM_GRP; i++){
		...
		if(k >= threshold_num) {
			ret = -1;
			pr_info("[name:spm&][SPM][Mbrain] number of threshold is wrong\n");
			break;
		}
