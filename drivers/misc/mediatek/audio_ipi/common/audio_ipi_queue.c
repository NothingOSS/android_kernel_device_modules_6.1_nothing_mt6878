	...
	struct ipi_msg_t *p_ipi_msg)
{
	...
	/* send to dsp directly (bypass audio queue, but still in IPC queue) */
	if (p_ipi_msg->ack_type == AUDIO_IPI_MSG_DIRECT_SEND)
		return send_message_to_dsp(p_ipi_msg);
	...
	/* process queue */
	if (is_queue_empty == true) { /* just send message to dsp */
	...
	} else { /* wait until processed, and then send message to dsp */
	...
	int idx_msg)
{
	...
	case AUDIO_IPI_MSG_BYPASS_ACK: {
		...
		retval = send_message_to_dsp(p_ipi_msg);
		break;
	}
