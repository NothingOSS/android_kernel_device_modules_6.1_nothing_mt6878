 ...
 */
#define MAIN3_VCM_FOCUS_STEPS			1
...
{
	return container_of(ctrl->handler, struct main3_vcm_device, ctrls);
}

static inline struct main3_vcm_device *sd_to_main3_vcm_vcm(struct v4l2_subdev *subdev)
{
	return container_of(subdev, struct main3_vcm_device, sd);
}
...
static int main3_vcm_init(struct main3_vcm_device *main3_vcm)
{
	struct i2c_client *client = v4l2_get_subdevdata(&main3_vcm->sd);
	int ret = 0;
...
/* Power handling */
static int main3_vcm_power_off(struct main3_vcm_device *main3_vcm)
{
	int ret = 0;
	int ldo_num = 0;
	int i = 0;
...
static int main3_vcm_set_ctrl(struct v4l2_ctrl *ctrl)
{
	int ret = 0;
	struct main3_vcm_device *main3_vcm = to_main3_vcm_vcm(ctrl);

	if (ctrl->id == V4L2_CID_FOCUS_ABSOLUTE) {
		LOG_INF("pos(%d)\n", ctrl->val);
		ret = main3_vcm_set_position(main3_vcm, ctrl->val);
		if (ret < 0) {
			LOG_INF("%s I2C failure: %d\n",
				__func__, ret);
			return ret;
...
static const struct v4l2_ctrl_ops main3_vcm_vcm_ctrl_ops = {
	.s_ctrl = main3_vcm_set_ctrl,
};

static int main3_vcm_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	int ret;
...
static int main3_vcm_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct main3_vcm_device *main3_vcm = sd_to_main3_vcm_vcm(sd);
...
static long main3_vcm_ops_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	int ret = 0;
	struct main3_vcm_device *main3_vcm = sd_to_main3_vcm_vcm(sd);
...
static const struct v4l2_subdev_internal_ops main3_vcm_int_ops = {
	.open = main3_vcm_open,
	.close = main3_vcm_close,
};

static const struct v4l2_subdev_core_ops main3_vcm_ops_core = {
	.ioctl = main3_vcm_ops_core_ioctl,
};

static const struct v4l2_subdev_ops main3_vcm_ops = {
	.core = &main3_vcm_ops_core,
};

static void main3_vcm_subdev_cleanup(struct main3_vcm_device *main3_vcm)
{
	v4l2_async_unregister_subdev(&main3_vcm->sd);
	v4l2_ctrl_handler_free(&main3_vcm->ctrls);
#if IS_ENABLED(CONFIG_MEDIA_CONTROLLER)
	media_entity_cleanup(&main3_vcm->sd.entity);
#endif
}
...
static int main3_vcm_init_controls(struct main3_vcm_device *main3_vcm)
{
	...
	main3_vcm->sd.ctrl_handler = hdl;

	return 0;
}

static int main3_vcm_probe(struct i2c_client *client)
{
	struct device *dev = &client->dev;
	struct main3_vcm_device *main3_vcm;
	int ret;
	int ldo_num;
	int i;
	...
	main3_vcm = devm_kzalloc(dev, sizeof(*main3_vcm), GFP_KERNEL);
	if (!main3_vcm)
		return -ENOMEM;
	...
	if (ldo_num > REGULATOR_MAXSIZE)
		ldo_num = REGULATOR_MAXSIZE;
	for (i = 0; i < ldo_num; i++) {
		main3_vcm->ldo[i] = devm_regulator_get(dev, ldo_names[i]);
		if (IS_ERR(main3_vcm->ldo[i])) {
	...
	if (IS_ERR(main3_vcm->vcamaf_pinctrl)) {
		ret = PTR_ERR(main3_vcm->vcamaf_pinctrl);
		main3_vcm->vcamaf_pinctrl = NULL;
		LOG_INF("cannot get pinctrl\n");
	} else {
		main3_vcm->vcamaf_on = pinctrl_lookup_state(
			main3_vcm->vcamaf_pinctrl, "vcamaf_on");

		if (IS_ERR(main3_vcm->vcamaf_on)) {
			ret = PTR_ERR(main3_vcm->vcamaf_on);
			main3_vcm->vcamaf_on = NULL;
			LOG_INF("cannot get vcamaf_on pinctrl\n");
		}

		main3_vcm->vcamaf_off = pinctrl_lookup_state(
			main3_vcm->vcamaf_pinctrl, "vcamaf_off");

		if (IS_ERR(main3_vcm->vcamaf_off)) {
	...
	main3_vcm->sd.internal_ops = &main3_vcm_int_ops;

	ret = main3_vcm_init_controls(main3_vcm);
	if (ret)
		goto err_cleanup;

#if IS_ENABLED(CONFIG_MEDIA_CONTROLLER)
	ret = media_entity_pads_init(&main3_vcm->sd.entity, 0, NULL);
	if (ret < 0)
		goto err_cleanup;

	main3_vcm->sd.entity.function = MEDIA_ENT_F_LENS;
#endif

	ret = v4l2_async_register_subdev(&main3_vcm->sd);
	if (ret < 0)
		goto err_cleanup;
...
err_cleanup:
	main3_vcm_subdev_cleanup(main3_vcm);
	return ret;
}

static void main3_vcm_remove(struct i2c_client *client)
{
...
static const struct i2c_device_id main3_vcm_id_table[] = {
	{ MAIN3_VCM_NAME, 0 },
	{ },
};
MODULE_DEVICE_TABLE(i2c, main3_vcm_id_table);

static const struct of_device_id main3_vcm_of_table[] = {
	{ .compatible = "mediatek,main3_vcm" },
	{ },
};
MODULE_DEVICE_TABLE(of, main3_vcm_of_table);

static struct i2c_driver main3_vcm_i2c_driver = {
	.driver = {
		.name = MAIN3_VCM_NAME,
		.of_match_table = main3_vcm_of_table,
	},
	.probe_new  = main3_vcm_probe,
	.remove = main3_vcm_remove,
	.id_table = main3_vcm_id_table,
};

module_i2c_driver(main3_vcm_i2c_driver);

MODULE_AUTHOR("Po-Hao Huang <Po-Hao.Huang@mediatek.com>");
MODULE_DESCRIPTION("MAIN3_VCM VCM driver");
MODULE_LICENSE("GPL");
