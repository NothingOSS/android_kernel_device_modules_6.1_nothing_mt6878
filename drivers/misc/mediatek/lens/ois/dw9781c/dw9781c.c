...
static void readhall(u16 *gyro_x, u16 *gyro_y, s16 *target_x, s16 *target_y, s16 *len_x, s16 *len_y)
{
	...
	// get x gyro
	ret = ois_i2c_rd_u16(m_client, DW9781C_REG_OIS_GYROX, gyro_x);
	if (ret < 0) {
	...
	// get y gyro
	ret = ois_i2c_rd_u16(m_client, DW9781C_REG_OIS_GYROY, gyro_y);
	if (ret < 0) {
	...
	// get x target
	ret = ois_i2c_rd_s16(m_client, DW9781C_REG_OIS_TARGETX, target_x);
	if (ret < 0) {
	...
	// get y target
	ret = ois_i2c_rd_s16(m_client, DW9781C_REG_OIS_TARGETY, target_y);
	if (ret < 0) {
...
static int dw9781c_init(struct dw9781c_device *dw9781c)
{
	...
	ret = ois_i2c_wr_u16(client, DW9781C_REG_IMU_SELECT, ICM42631);
	I2C_OPERATION_CHECK(ret);
	mdelay(1);
