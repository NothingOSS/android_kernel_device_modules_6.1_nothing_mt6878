...
//The default chip for NFC driver settings is UNMATCH
static chip_t chip_type = UNMATCH;
...
#endif
{
    ...
    case NXP_SN220:
        TMS_INFO("NXP SN220 driver start probe\n");
        ret = nfc_i2c_dev_probe(client, id);
...
//add nxp nfc  nxp_chip_identification
static int nxp_chip_identification(struct match_info info, uint8_t *buf)
{
    int ret, i;
    int cmp_byte = 0;

    if (info.pattern == NXP_FW) {
        cmp_byte = info.sum - TMS_FW_CMP_BYTE - info.check_sum;
        TMS_INFO("nxp_cmp_byte = %d\n", cmp_byte);
    } else if (info.pattern == TMS_BL) {
        cmp_byte = info.sum - TMS_BL_CMP_BYTE - info.check_sum;
    } else {
        TMS_ERR("Chip identified fail\n");
    ...
    TMS_INFO("Compare major version is %02x\n", buf[cmp_byte]);
    for (i = 0; i < info.ver_num; i++) {
        if ((buf[cmp_byte] & NXP_VERSION_MASK) == info.major_ver[i]) {
            ret = SUCCESS;
            TMS_INFO("Version matched\n");
            break;
        }
        ret = -ERROR;
    }
                               ...
                               uint8_t *buf)
{
    ...
    do {
        ...
        if (gpio_get_value(gdata->hw_res.irq_gpio)) {
            ...
            if (TMS_FW == info->pattern || NXP_FW == info->pattern) {
                if (1 == read_len) {
                    info->check_sum = 1;
                } else {
                    info->check_sum = 0;
...
//add nxp nfc chip info
static int nxp_check_chip_info(struct guide_dev *gdata, struct match_info info)
{
    int ret;
    int retry;
    uint8_t read_buf[MAX_CMD_LEN];
    ...
    if (info.pattern == NXP_FW) {
        guidev_hw_reset();
        if (!gdata->tms->feature.dl_support) {
            TMS_INFO("NXP chip jump to FW\n");
            nfc_jump_fw(gdata->client, gdata->hw_res.irq_gpio);
        }
        TMS_INFO("NXP chip FW match check...\n");
    }  else {
        TMS_ERR("NXP chip info error\n");
        return -ERROR;
    }
    ...
    ret = nxp_chip_identification(info, read_buf);
    return ret;
}
...
static int guidev_chip_match(struct guide_dev *gdata)
{
    ...
    for (count = 0; count < ARRAY_SIZE(info_list); count++) {
        TMS_INFO("Check chip info %d times\n", count + 1);
        if (info_list[count].type == TMS_THN31) {
             ret = tms_check_chip_info(gdata, info_list[count]);
        } else if (info_list[count].type == NXP_SN220) {
            ret = nxp_check_chip_info(gdata, info_list[count]);
...
#endif
{
    ...
    case NXP_SN220:
        TMS_INFO("NXP_SN220 nfc start removed\n");
...
static int guide_suspend(struct device *device)
{
    ...
    case NXP_SN220:
        TMS_INFO("NXP_SN220 nfc start suspend\n");
...
static int guide_resume(struct device *device)
{
    ...
    case NXP_SN220:
        TMS_INFO("NXP_SN220 nfc start resume\n");
