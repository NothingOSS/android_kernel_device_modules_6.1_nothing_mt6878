...
static int mt6989_apu_top_on(struct device *dev)
{
	int ret = 0;
...
static int mt6989_apu_top_off(struct device *dev)
{
	int ret = 0, val = 0;
...
static int mt6989_apu_top_pb(struct platform_device *pdev)
{
	int ret = 0, val = 0;
...
static int mt6989_apu_top_rm(struct platform_device *pdev)
{
	int idx;
...
static int mt6989_apu_top_suspend(struct device *dev)
{
	return 0;
}

static int mt6989_apu_top_resume(struct device *dev)
{
	return 0;
}

static int mt6989_apu_top_func(struct platform_device *pdev,
		enum aputop_func_id func_id, struct aputop_func_param *aputop)
{
	...
	case APUTOP_FUNC_OPP_LIMIT_HAL:
		mt6989_aputop_opp_limit(aputop, OPP_LIMIT_HAL);
		break;
	case APUTOP_FUNC_OPP_LIMIT_DBG:
		mt6989_aputop_opp_limit(aputop, OPP_LIMIT_DEBUG);
		break;
...
const struct apupwr_plat_data mt6989_plat_data = {
	.plat_name = "mt6989_apupwr",
	.plat_aputop_on = mt6989_apu_top_on,
	.plat_aputop_off = mt6989_apu_top_off,
	.plat_aputop_pb = mt6989_apu_top_pb,
	.plat_aputop_rm = mt6989_apu_top_rm,
	.plat_aputop_suspend = mt6989_apu_top_suspend,
	.plat_aputop_resume = mt6989_apu_top_resume,
	.plat_aputop_func = mt6989_apu_top_func,
#if IS_ENABLED(CONFIG_DEBUG_FS)
	.plat_aputop_dbg_open = mt6989_apu_top_dbg_open,
	.plat_aputop_dbg_write = mt6989_apu_top_dbg_write,
#endif
	.plat_rpmsg_callback = mt6989_apu_top_rpmsg_cb,
	.bypass_pwr_on = 0,
	.bypass_pwr_off = 0,
};
