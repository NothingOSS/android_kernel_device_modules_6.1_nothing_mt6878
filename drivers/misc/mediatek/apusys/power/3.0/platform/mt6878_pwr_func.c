...
void mt6878_aputop_opp_limit(struct aputop_func_param *aputop,
		enum apu_opp_limit_type type)
{
...
static int aputop_show_curr_status(struct seq_file *s, void *unused)
{
	...
	for (i = 0 ; i < CLUSTER_NUM ; i++) {
		mt6878_apu_dump_rpc_status(i, &cluster_dump[i]);
	...
	// for RCX
	mt6878_apu_dump_rpc_status(RCX, &cluster_dump[CLUSTER_NUM]);
	seq_printf(s,
...
int mt6878_apu_top_dbg_open(struct inode *inode, struct file *file)
{
	pr_info("%s ++\n", __func__);
...
#define MAX_ARG 4
ssize_t mt6878_apu_top_dbg_write(
...
int mt6878_apu_top_rpmsg_cb(int cmd, void *data, int len, void *priv, u32 src)
{
	int ret = 0;
...
int mt6878_drv_cfg_remote_sync(struct aputop_func_param *aputop)
{
...
int mt6878_init_remote_data_sync(void __iomem *reg_base)
{
	int i;
	...
	for (i = 0 ; i < CLUSTER_NUM ; i++) {
		...
		reg_offset = opp_limit_tbl[i].opp_lmt_reg;
#if LOCAL_DBG
		pr_info("%s spare_reg_base:0x%08llx, offset:0x%08x\n",
				__func__, (uint64_t)spare_reg_base, reg_offset);
#endif
