...
static int mt6878_rproc_init(struct mtk_apu *apu)
{
	return 0;
}

static int mt6878_rproc_exit(struct mtk_apu *apu)
{
	return 0;
}
...
static void mt6878_rv_boot_non_secure(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
...
static int mt6878_rproc_start(struct mtk_apu *apu)
{
	int ns = 1; /* Non Secure */
	int domain = 0;
...
static int mt6878_rproc_stop(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
...
static int mt6878_apu_power_init(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
...
static int mt6878_apu_power_on(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
	int ret, timeout, i;
...
static int mt6878_apu_power_off(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
	int ret, timeout, i;
...
static int mt6878_ipi_send_post(struct mtk_apu *apu)
{
	cancel_delayed_work_sync(&timeout_work);
...
static int mt6878_irq_affin_init(struct mtk_apu *apu)
{
	int i;
...
static int mt6878_irq_affin_set(struct mtk_apu *apu)
{
...
static int mt6878_irq_affin_unset(struct mtk_apu *apu)
{
	irq_set_affinity_hint(apu->mbox0_irq_number, &normal_cpus);
...
static int mt6878_check_apu_exp_irq(struct mtk_apu *apu, char *ce_module)
{
	return 1;
}


static int mt6878_apu_memmap_init(struct mtk_apu *apu)
{
...
static void mt6878_apu_memmap_remove(struct mtk_apu *apu)
{
}

static void mt6878_rv_cg_gating(struct mtk_apu *apu)
{
	iowrite32(0x0, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6878_rv_cg_ungating(struct mtk_apu *apu)
{
	iowrite32(0x1, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6878_rv_cachedump(struct mtk_apu *apu)
{
	int offset;
	unsigned long flags;
...
const struct mtk_apu_platdata mt6878_platdata = {
	.flags		= F_PRELOAD_FIRMWARE | F_AUTO_BOOT | F_DEBUG_LOG_ON |
				...
				F_APUSYS_RV_TAG_SUPPORT | F_EXCEPTION_KE | F_APU_IPI_UT_SUPPORT |
				F_TCM_WA,
	.ops		= {
		.init	= mt6878_rproc_init,
		.exit	= mt6878_rproc_exit,
		.start	= mt6878_rproc_start,
		.stop	= mt6878_rproc_stop,
		.apu_memmap_init = mt6878_apu_memmap_init,
		.apu_memmap_remove = mt6878_apu_memmap_remove,
		.cg_gating = mt6878_rv_cg_gating,
		.cg_ungating = mt6878_rv_cg_ungating,
		.rv_cachedump = mt6878_rv_cachedump,
		.power_init = mt6878_apu_power_init,
		.power_on = mt6878_apu_power_on,
		.power_off = mt6878_apu_power_off,
		.ipi_send_post = mt6878_ipi_send_post,
		.irq_affin_init = mt6878_irq_affin_init,
		.irq_affin_set = mt6878_irq_affin_set,
		.irq_affin_unset = mt6878_irq_affin_unset,
		.check_apu_exp_irq = mt6878_check_apu_exp_irq,
	},
};
