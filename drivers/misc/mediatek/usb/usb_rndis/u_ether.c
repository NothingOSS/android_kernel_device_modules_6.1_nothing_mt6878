...
#define DBG(d, fmt, args...) \
	pr_debug("%s: " fmt, (d)->net->name, ## args)
#else
...
#define ERROR(d, fmt, args...) \
	pr_err("%s: " fmt, (d)->net->name, ## args)
#define INFO(d, fmt, args...) \
	pr_info("%s: " fmt, (d)->net->name, ## args)
...
static int alloc_requests(struct eth_dev *dev, struct gether *link, unsigned int n)
{
	int	status;
...
void mtk_gether_set_qmult(struct net_device *net, unsigned int qmult)
{
	struct eth_dev *dev;
...
unsigned int mtk_gether_get_qmult(struct net_device *net)
{
	struct eth_dev *dev;
...
{
	rtnl_lock();
	strscpy(name, netdev_name(net), len);
	rtnl_unlock();
	return strlen(name);
}
