 ...
 *		Fixed mtk_rndis_rm_hdr length bug.
 *
...
/* Function Prototypes */
static struct rndis_resp_t *rndis_add_response(struct rndis_params *params,
					u32 length);
...
static int gen_ndis_query_resp(struct rndis_params *params, u32 OID, u8 *buf,
			       unsigned int buf_len, struct rndis_resp_t *r)
{
	int retval = -EOPNOTSUPP;
	u32 length = 4;	/* usually */
	__le32 *outbuf;
	int i, count;
	struct rndis_query_cmplt_type *resp;
	...
	if (!r)
		return -ENOMEM;
	resp = (struct rndis_query_cmplt_type *)r->buf;
...
static int gen_ndis_set_resp(struct rndis_params *params, u32 OID,
			     u8 *buf, u32 buf_len, struct rndis_resp_t *r)
{
	struct rndis_set_cmplt_type *resp;
	int i, retval = -EOPNOTSUPP;
...
{
	struct rndis_init_cmplt_type *resp;
	struct rndis_resp_t *r;

	if (!params->dev)
		return -EOPNOTSUPP;

	r = rndis_add_response(params, sizeof(struct rndis_init_cmplt_type));
	if (!r)
		return -ENOMEM;
	resp = (struct rndis_init_cmplt_type *)r->buf;
...
static int rndis_query_response(struct rndis_params *params,
				struct rndis_query_msg_type *buf)
{
	struct rndis_query_cmplt_type *resp;
	struct rndis_resp_t *r;
	...
	r = rndis_add_response(params,
		...
		sizeof(struct rndis_query_cmplt_type));
	if (!r) {
...
static int rndis_reset_response(struct rndis_params *params,
				struct rndis_reset_msg_type *buf)
{
	struct rndis_reset_cmplt_type *resp;
	struct rndis_resp_t *r;
	...
	while ((xbuf = mtk_rndis_get_next_response(params, &length)))
		mtk_rndis_free_response(params, xbuf);

	rndis_test_reset_msg_cnt++;

	r = rndis_add_response(params, sizeof(struct rndis_reset_cmplt_type));
	if (!r)
		return -ENOMEM;
	resp = (struct rndis_reset_cmplt_type *)r->buf;
...
static int rndis_keepalive_response(struct rndis_params *params,
				    struct rndis_keepalive_msg_type *buf)
{
	struct rndis_keepalive_cmplt_type *resp;
	struct rndis_resp_t *r;

	/* host "should" check only in RNDIS_DATA_INITIALIZED state */

	r = rndis_add_response(params, sizeof(struct rndis_keepalive_cmplt_type));
	if (!r) {
		RNDIS_DBG("rndis_add_response return NULL\n");
		return -ENOMEM;
	}
	resp = (struct rndis_keepalive_cmplt_type *)r->buf;
...
{
	struct rndis_indicate_status_msg_type *resp;
	struct rndis_resp_t *r;
	...
	r = rndis_add_response(params,
		sizeof(struct rndis_indicate_status_msg_type));
	if (!r)
		return -ENOMEM;
	resp = (struct rndis_indicate_status_msg_type *)r->buf;
...
EXPORT_SYMBOL_GPL(mtk_rndis_signal_connect);
...
EXPORT_SYMBOL_GPL(mtk_rndis_signal_disconnect);
...
void mtk_rndis_set_host_mac(struct rndis_params *params, const u8 *addr)
{
	params->host_mac = addr;
}
EXPORT_SYMBOL_GPL(mtk_rndis_set_host_mac);
...
int mtk_rndis_msg_parser(struct rndis_params *params, u8 *buf)
{
	...
	case RNDIS_MSG_INIT:
		pr_info("%s: RNDIS_MSG_INIT\n",
			__func__);
		params->state = RNDIS_INITIALIZED;
		return rndis_init_response(params,
					(struct rndis_init_msg_type *)buf);
	...
	case RNDIS_MSG_SET:
		return rndis_set_response(params,
					(struct  rndis_set_msg_type *)buf);
...
EXPORT_SYMBOL_GPL(mtk_rndis_msg_parser);
...
EXPORT_SYMBOL_GPL(mtk_rndis_register);

void mtk_rndis_deregister(struct rndis_params *params)
{
	int i;

	pr_debug("mtk_rndis deregister\n");
...
EXPORT_SYMBOL_GPL(mtk_rndis_set_param_dev);

int mtk_rndis_set_param_vendor(struct rndis_params *params, u32 vendorID,
			   const char *vendorDescr)
{
	pr_debug("mtk rndis set param vendor\n");
	if (!vendorDescr)
		return -1;
	if (!params)
		return -1;
...
EXPORT_SYMBOL_GPL(mtk_rndis_set_param_vendor);

int mtk_rndis_set_param_medium(struct rndis_params *params, u32 medium, u32 speed)
{
...
EXPORT_SYMBOL_GPL(mtk_rndis_set_param_medium);

void rndis_set_max_pkt_xfer(struct rndis_params *params, u8 max_pkt_per_xfer)
{
	pr_debug("rndis set max pkt xfer\n");
...
EXPORT_SYMBOL_GPL(mtk_rndis_add_hdr);
...
void mtk_rndis_free_response(struct rndis_params *params, u8 *buf)
{
	...
	list_for_each_safe(act, tmp, &(params->resp_queue)) {
		...
		r = list_entry(act, struct rndis_resp_t, list);
		if (r && r->buf == buf) {
			list_del(&r->list);
			kfree(r);
		}
...
EXPORT_SYMBOL_GPL(mtk_rndis_free_response);
...
EXPORT_SYMBOL_GPL(mtk_rndis_get_next_response);
			...
			u32 length)
{
	...
	r = kmalloc(sizeof(struct rndis_resp_t) + length, GFP_ATOMIC);
	if (!r)
		return NULL;
...
int mtk_rndis_rm_hdr(struct gether *port,
			struct sk_buff *skb,
...
EXPORT_SYMBOL_GPL(mtk_rndis_rm_hdr);
...
static int rndis_proc_show(struct seq_file *m, void *v)
{
	...
	case RNDIS_UNINITIALIZED:
		s = "RNDIS_UNINITIALIZED";
		break;
	case RNDIS_INITIALIZED:
		s = "RNDIS_INITIALIZED";
		break;
	case RNDIS_DATA_INITIALIZED:
		s = "RNDIS_DATA_INITIALIZED";
		break;
	default:
		break;
	}
	...
	seq_printf(m,
			 ...
			 "vendor ID : 0x%08X\n"
			 "vendor    : %s\n"
			 "max xfer size:%d, max pkt per xfer:%d\n"
			 "UL max xfer size rcvd:%d, UL max pkt per xfer rcvd:%d\n",
				...
				size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < count; i++) {
		...
		case 'C':
		case 'c':
			mtk_rndis_signal_connect(p);
			break;
		case 'D':
		case 'd':
			mtk_rndis_signal_disconnect(p);
			break;
		default:
			if (fl_speed)
				p->speed = speed;
