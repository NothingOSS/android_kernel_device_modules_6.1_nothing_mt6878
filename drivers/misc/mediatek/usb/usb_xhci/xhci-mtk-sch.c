...
int xhci_mtk_check_bandwidth(struct usb_hcd *hcd, struct usb_device *udev)
{
	...
	list_for_each_entry(sch_ep, &mtk->bw_ep_chk_list, endpoint) {
		...
		unsigned int ep_index = xhci_get_endpoint_index_(&ep->desc);
	...
	ret = xhci_check_bandwidth_(hcd, udev);
	if (!ret)
...
void xhci_mtk_reset_bandwidth(struct usb_hcd *hcd, struct usb_device *udev)
{
	...
	xhci_reset_bandwidth_(hcd, udev);
}
		    ...
		    struct usb_host_endpoint *ep)
{
	...
	ret = xhci_add_endpoint_(hcd, udev, ep);
	if (ret)
		return ret;
		     ...
		     struct usb_host_endpoint *ep)
{
	...
	ret = xhci_drop_endpoint_(hcd, udev, ep);
	if (ret)
		return ret;
