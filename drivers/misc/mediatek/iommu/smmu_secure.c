...
static int mtk_smmu_sec_debug_set(void *data, u64 input)
{
	...
	pr_info("%s input:0x%llx, index:%u, smmu:%u, sec:%u, aid:%u, sid:%u\n",
		__func__, input, index, smmu_type, sec, aid, sid);
	...
	case SMMU_SECURE_AID_SID_MAP:
		...
		ret = mtk_smmu_aid2sid(smmu_type, sid, aid, sec);
		break;
