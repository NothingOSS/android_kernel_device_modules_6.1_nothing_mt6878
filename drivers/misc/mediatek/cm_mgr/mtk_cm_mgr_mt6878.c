...
u32 cm_mgr_get_perfs_mt6878(int num)
{
...
static void cm_mgr_perf_platform_set_status_mt6878(int enable)
{
	...
	if (enable) {
		...
		icc_set_bw(cm_mgr_get_bw_path(), 0,
			   cm_mgr_get_perfs_mt6878(cm_mgr_dram_opp));
...
static void cm_mgr_perf_set_status_mt6878(int enable)
{
	cm_mgr_perf_platform_set_status_mt6878(enable);
}

static void check_cm_mgr_status_mt6878(unsigned int cluster, unsigned int freq,
				       unsigned int idx)
{
...
static int cm_mgr_check_dts_setting_mt6878(struct platform_device *pdev)
{
	int i = 0;
	int ret = 0;
	int temp = 0;
...
static int platform_cm_mgr_probe(struct platform_device *pdev)
{
	...
	ret = cm_mgr_check_dts_setting_mt6878(pdev);
	if (ret) {
	...
	local_hk.cm_mgr_get_perfs = cm_mgr_get_perfs_mt6878;
	local_hk.check_cm_mgr_status = check_cm_mgr_status_mt6878;
	local_hk.cm_mgr_perf_set_status = cm_mgr_perf_set_status_mt6878;
