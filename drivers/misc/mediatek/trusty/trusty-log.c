				   ...
				   unsigned long action, void *data)
{
	...
	pr_info("trusty-log panic notifier - trusty version %s",
		ise_version_str_get(s->trusty_dev));
	trusty_dump_logs(s);
	return NOTIFY_OK;
}
...
static bool trusty_supports_logging(struct device *device)
{
	...
	result = ise_std_call32(device, SMC_SC_SHARED_LOG_VERSION,
...
static int trusty_log_probe(struct platform_device *pdev)
{
	...
	s->get = 0;
	s->log = trusty_shm_alloc(TRUSTY_LOG_SIZE, GFP_KERNEL | __GFP_ZERO);
	if (!s->log) {
		result = -ENOMEM;
		goto error_alloc_log;
	}
	s->log_pages = virt_to_page(s->log);

	pa = trusty_shm_virt_to_phys(s->log);
	result = ise_std_call32(s->trusty_dev,
	...
	result = ise_call_notifier_register(s->trusty_dev,
					       &s->call_notifier);
	if (result < 0) {
		dev_err(&pdev->dev,
...
error_call_notifier:
	ise_std_call32(s->trusty_dev, SMC_SC_SHARED_LOG_RM,
			  (u32)pa, (u32)(pa >> 32), 0);
error_std_call:
