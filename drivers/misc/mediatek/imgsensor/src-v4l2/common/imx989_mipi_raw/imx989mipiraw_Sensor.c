...
static struct subdrv_mode_struct mode_struct[] = {
	...
	},
	{
		...
		.hdr_mode = HDR_NONE,
		.pclk = 2195200000, //4390400000
		.linelength = 22336, //11168
		.framelength = 3272,
		.max_framerate = 1200,
	...
	},
	{
		...
		.hdr_group = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
		.pclk = 2176000000, //4352000000
		.linelength = 14336, //7168
		.framelength = 2528,
		.max_framerate = 2400,
		.mipi_pixel_rate = 2452114286,
		.readout_length = 0,
	...
	},
	{
		...
		.hdr_group = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
		.pclk = 2169600000, //4339200000
		.linelength = 43072, //21536
		.framelength = 6712,
		.max_framerate = 300,
		.mipi_pixel_rate = 2452114286,
		.readout_length = 0,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.checksum_value = 0xAF3E324F, //mtk31709
};
...
static int imx989_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case 5:
		/* dig_gain = 0x */
		subdrv_i2c_wr_u8(ctx, ctx->s_ctx.reg_addr_dig_gain[0].addr[0], 0x00);
		subdrv_i2c_wr_u8(ctx, ctx->s_ctx.reg_addr_dig_gain[0].addr[1], 0x00);
		break;
	default:
		if (ctx->test_pattern == 5) {
			/* dig_gain = 1x */
			subdrv_i2c_wr_u8(ctx, ctx->s_ctx.reg_addr_dig_gain[0].addr[0], 0x01);
			subdrv_i2c_wr_u8(ctx, ctx->s_ctx.reg_addr_dig_gain[0].addr[1], 0x00);
		}
		if (ctx->test_pattern)
