...
static int g_seamless_switch_scenario(struct adaptor_ctx *ctx, void *arg)
{
	...
	union feature_para para;
	u32 len, count = 0;
	unsigned int ret_for_copy_to_user = 0;
	...
	ret_for_copy_to_user = copy_to_user(target->target_scenario_ids,
				 target_scenario_ids, sizeof(target_scenario_ids));
	if (ret_for_copy_to_user)
		dev_info(ctx->dev, "ret_for_copy_to_user: %d\n", ret_for_copy_to_user);
