...
static void armv8_pmu_hw_start(struct pmu_data_info *pmu, int count)
{
	int i;
	int generic = count - 1;
...
static unsigned int armv8_pmu_hw_polling(struct pmu_data_info *pmu, int count, unsigned int *pmu_v)
{
	int i, cnt = 0;
	int generic = count - 1;
	...
	for (i = 0; i < generic; i++) {
		if (pmu[i].mode == MODE_POLLING) {
			pmu_v[cnt] = armv8_pmu_read_count(i);
			cnt++;
		}
	}
	if (pmu[count - 1].mode == MODE_POLLING) {
		pmu_v[cnt] = armv8_pmu_read_count(31);
		cnt++;
	}
...
static void set_pmu_event_count(void *info)
{
	...
	arm_pmu.event_count[cpu] = ((read_sysreg(pmcr_el0) >> PMCR_N_SHIFT) & PMCR_N_MASK) + 1;
}
