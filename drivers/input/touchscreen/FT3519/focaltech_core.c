...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *v)
{
    ...
    if (ts_data && v) {
        if (event == MTK_DISP_EVENT_BLANK) {
            ...
            FTS_INFO("event=%lu, MTK_DISP_EVENT_BLANK=%d, lcm_now_state=%d\n", event, MTK_DISP_EVENT_BLANK, lcm_now_state);
            ...
            * after screen is off
            */
            FTS_INFO("event=%lu, MTK_DISP_EARLY_EVENT_BLANK=%d, lcm_now_state=%d\n", event, MTK_DISP_EARLY_EVENT_BLANK, lcm_now_state);
        ...
        FTS_INFO("ft3519 touch IC can not suspend or resume");
        return -EINVAL;
    }
...
//    ts_data->fb_notif.notifier_call = fb_notifier_callback;
    ...
    ret = mtk_disp_notifier_register("Touch_driver", &ts_data->disp_notifier);
    if(ret) {
        FTS_ERROR("register mtk_disp_notifier_register failed. ret=%d\n", ret);
...
/*
    ...
    if (active_panel) {
        ...
        if (ret) FTS_ERROR("[DRM]drm_panel_notifier_register fail: %d\n", ret);
    }

#else
    FTS_INFO("init notifier with msm_drm_register_client");
    ret = msm_drm_register_client(&ts_data->fb_notif);
    if (ret) FTS_ERROR("[DRM]msm_drm_register_client fail: %d\n", ret);
#endif //CONFIG_DRM_PANEL
*/
...
*/
    ...
    if (mtk_disp_notifier_unregister(&ts_data->disp_notifier))
        FTS_ERROR("[FB]Error occurred while unregistering disp_notifier.");
#elif IS_ENABLED(CONFIG_FB)
