...
#if FTS_SPI_PINCTRL_EN
static int fts_spi_pinctrl_init(struct fts_ts_data *ts)
{
    int ret = 0;

    FTS_FUNC_ENTER();
    ts->spi_pinctrl = devm_pinctrl_get(ts->dev);
    if (IS_ERR_OR_NULL(ts->spi_pinctrl)) {
        FTS_ERROR("Failed to get spi pinctrl, please check dts");
        ret = PTR_ERR(ts->spi_pinctrl);
        goto err_pinctrl_get;
    }

    ts->pins_spi_default = pinctrl_lookup_state(ts->spi_pinctrl, "ts_spi_mode");
    if (IS_ERR_OR_NULL(ts->pins_spi_default)) {
        FTS_ERROR("Pin state[spi] not found");
        ret = PTR_ERR(ts->pins_spi_default);
        goto err_pinctrl_lookup;
    }
...
err_pinctrl_lookup:
    if (ts->spi_pinctrl) {
        devm_pinctrl_put(ts->spi_pinctrl);
    }
err_pinctrl_get:
    ts->spi_pinctrl = NULL;
    ts->pins_spi_default = NULL;

    FTS_FUNC_EXIT();
    return ret;
}
#endif /* FTS_SPI_PINCTRL_EN */
...
#if 0
	    FTS_DEBUG("[GPIO] avdd gpio pull hight");
            ret = gpio_direction_output(ts_data->pdata->avdd_gpio, 1);
            if (ret) {
                FTS_ERROR("[GPIO]set_direction for avdd gpio hight failed");
            }
#endif
            FTS_DEBUG("[GPIO] dvdd gpio pull hight");
            ret = gpio_direction_output(ts_data->pdata->dvdd_gpio, 1);
            if (ret) {
                FTS_ERROR("[GPIO]set_direction for dvdd gpio failed");
            }
            fts_msleep(2);
            fts_set_reset(ts_data, 1);
...
#if 0
            FTS_DEBUG("[GPIO] avdd gpio pull low");
            ret = gpio_direction_output(ts_data->pdata->avdd_gpio, 0);
            if (ret) {
                FTS_ERROR("[GPIO]set_direction for avdd gpio hight failed");
            }
#endif
            FTS_DEBUG("[GPIO] dvdd gpio pull low");
            ret = gpio_direction_output(ts_data->pdata->dvdd_gpio, 0);
            if (ret) {
                FTS_ERROR("[GPIO]set_direction for dvdd gpio failed");
            }
...
#if 0
    /* request avdd gpio*/
    //GPIO interface is left on the hardware schematic for software synchronization adaptation
...
#endif
   /* request dvdd gpio */
   if (gpio_is_valid(ts_data->pdata->dvdd_gpio)) {
        ret = gpio_request(ts_data->pdata->dvdd_gpio, "fts_dvdd_gpio");
        if (ret) {
            FTS_ERROR("[GPIO]dvdd gpio request failed");
            goto err_dvdd_gpio_dir;
        }
    }
...
#if FTS_SPI_PINCTRL_EN
    fts_spi_pinctrl_init(ts_data);
    if (ts_data->spi_pinctrl && ts_data->pins_spi_default) {
        ret = pinctrl_select_state(ts_data->spi_pinctrl, ts_data->pins_spi_default);
        if (ret < 0) {
            FTS_ERROR("Set spi pins to default state failed,ret=%d", ret);
        }
    }
#endif
...
#endif
    pdata->dvdd_gpio = of_get_named_gpio_flags(np, "focaltech,dvdd-gpio",
                      0, &pdata->dvdd_gpio_flags);
    if (pdata->dvdd_gpio < 0)
        FTS_ERROR("Unable to get idvdd_gpio");
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *v)
{
    ...
    if (ts_data && v) {
        if (event == MTK_DISP_EVENT_BLANK) {
            ...
            FTS_ERROR("event=%lu, MTK_DISP_EVENT_BLANK=%d\n", event, MTK_DISP_EVENT_BLANK);
            ...
            * after screen is off
            */
            FTS_ERROR("event=%lu, MTK_DISP_EARLY_EVENT_BLANK=%d\n", event, MTK_DISP_EARLY_EVENT_BLANK);
...
#endif
#if FTS_SPI_PINCTRL_EN
    if (ts_data->spi_pinctrl) {
        devm_pinctrl_put(ts_data->spi_pinctrl);
        ts_data->spi_pinctrl = NULL;
    }
#endif
