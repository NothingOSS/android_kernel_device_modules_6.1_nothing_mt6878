...
int short_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
    ...
    show_buffer(adc_buf, byte_num / 2, 0);
test_err:
    FTS_TEST_FUNC_EXIT();
    return ret;
}
...
static int fts_test_show(struct seq_file *s, void *unused)
{
    ...
    if (strstr(panel_name_find, "rm69220_boe")) {
        snprintf(fwname, FILE_NAME_LENGTH, "%s", FTS_INI_FILE_NAME_BOE);
    } else if (strstr(panel_name_find, "rm69220_vxn")) {
        snprintf(fwname, FILE_NAME_LENGTH, "%s", FTS_INI_FILE_NAME_VXN);
    }
