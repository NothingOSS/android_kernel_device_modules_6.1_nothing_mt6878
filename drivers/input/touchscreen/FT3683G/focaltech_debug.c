...
static int fts_diff_mode_show(struct seq_file *s, void *unused)
{
    ...
    mutex_lock(&dbg->mutex);
    fts_read_reg(FTS_REG_FW_DEBUG_EN, &val);
    seq_printf(s, "FwDebug support:%d,value:0x%x\n", ts_data->fwdbg_support, ts_data->fwdbg_value);
    seq_printf(s, "Reg(0x9E)=0x%x\n", val);
    mutex_unlock(&dbg->mutex);

    return 0;
}
int fts_diff_mode_open(struct inode* inode, struct file* file)
{
    return single_open(file, fts_diff_mode_show, NULL);
}
...
int fts_fwdbg_init(struct fts_ts_data *ts_data)
{
    ...
    dbg->proc_fwdbg = proc_create_data("diff_data", 0777, proc_touchpanel, &fts_fwdbg_fops, dbg);
