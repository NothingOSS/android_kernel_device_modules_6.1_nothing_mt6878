...
#if IS_ENABLED(CONFIG_PRIZE_FP_USE_VFP)
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
...
#if IS_ENABLED(CONFIG_PRIZE_FP_USE_VFP)
	gf_debug(DEBUG_LOG, "%s, get pinctrl CONFIG_PRIZE_FP_USE_VFP success!\n", __func__);
	...
	ret = regulator_set_voltage(vdd_reg, 3300000, 3300000);
	if (ret) {
...
void gf_netlink_send(struct gf_device *gf_dev, const int command, void *data)
{
	struct nlmsghdr *nlh = NULL;
	struct sk_buff *skb = NULL;
	int ret;
	int msg_size = MAX_NL_MSG_LEN;
	...
	memcpy(msg, (char *)&command, 1);
	if (((GF_NETLINK_UI_DISAPPEAR == command) || (GF_NETLINK_UI_READY == command)) && (NULL != data)) {
		memcpy(msg + 1, (char *)fp_event, sizeof(struct fp_notify_event));
	} else if (((GF_NETLINK_FINGER_DOWN == command) || (GF_NETLINK_FINGER_UP == command)) && (NULL != data)) {
			...
			unsigned long event, void *data)
{
	...
	int retval = 0;
	struct fp_notify_event *fp_event = (struct fp_notify_event *)data;
	FUNC_ENTRY();

	gf_dev = container_of(self, struct gf_device, notifier);
	switch (event) {
		case MTK_DISP_EARLY_EVENT_BLANK:
			gf_debug(INFO_LOG, "[%s] : MTK_DISP_EARLY_EVENT_BLANK,event=%ld\n", __func__, event);
			break;
		case MTK_DISP_EVENT_BLANK:
			gf_debug(INFO_LOG, "[%s] : MTK_DISP_EVENT_BLANK,event=%ld\n", __func__, event);
			gf_debug(INFO_LOG, "[%s] : enter, blank=%d\n", __func__, blank_value);
			switch (blank_value) { //display on off message.
				case MTK_DISP_BLANK_UNBLANK://screen on
					gf_debug(INFO_LOG, "[%s] : lcd on notify\n", __func__);
					gf_netlink_send(gf_dev, GF_NETLINK_SCREEN_ON, NULL);
					break;
				case MTK_DISP_BLANK_POWERDOWN://screen off
					gf_debug(INFO_LOG, "[%s] : lcd off notify\n", __func__);
					gf_netlink_send(gf_dev, GF_NETLINK_SCREEN_OFF, NULL);
					break;
				case MTK_DISP_BLANK_AOD://AOD
					gf_debug(INFO_LOG, "[%s] : lcd aod notify\n", __func__);
					gf_netlink_send(gf_dev, GF_NETLINK_SCREEN_AOD, NULL);
					break;
				default:
			...
			break;
		case FP_NOTIFIER_EVENT_UI:
			/*both hbm_status=1 ui_status=1, then UI Ready. otherwise UI disappear.*/
			gf_debug(INFO_LOG, "[%s] :FP_NOTIFIER_EVENT_UI hbm_status=%d,ui_status=%d\n", __func__, fp_event->hbm_status, fp_event->ui_status);
			if((1 == fp_event->hbm_status) && (1 == fp_event->ui_status)) {
				gf_netlink_send(gf_dev, GF_NETLINK_UI_READY, fp_event);
				gf_debug(INFO_LOG,"[%s]:FP_NOTIFIER_EVENT_UI FP_UI_READY #^_^#", __func__);
			} else {
				gf_netlink_send(gf_dev, GF_NETLINK_UI_DISAPPEAR, fp_event);
				gf_debug(INFO_LOG,"[%s]:FP_NOTIFIER_EVENT_UI FP_UI_DISAPPEAR #@_@#", __func__);
			}
			break;
		default:
			gf_debug(INFO_LOG, "[%s] : Ignore event, event=%ld\n", __func__, event);
		break;
	}
	FUNC_EXIT();
	return retval;
...
static int nt_tp_notifier_callback(struct notifier_block *self,
			unsigned long event, void *data)
{
	struct gf_device *gf_dev = NULL;
	int retval = 0;
	struct touchpanel_coordinate *coordinate = (struct touchpanel_coordinate *)data;
	FUNC_ENTRY();
	gf_dev = container_of(self, struct gf_device, tp_notifier);
	switch (event) {
		case TOUCHPANEL_FPEVENT_UP:
			/*message send format. data[0]:up/or down; data[1]:x coordinate; data[2]:y coordinate.*/
			gf_debug(INFO_LOG, "[%s] : TOUCHPANEL_FPEVENT_UP,event=%ld,x=%d,y=%d\n", \
								__func__, event, coordinate->x, coordinate->y);
			gf_netlink_send(gf_dev, GF_NETLINK_FINGER_UP, coordinate);
			break;
		case TOUCHPANEL_FPEVENT_DOWN:
			/*message send format. data[0]:up/or down; data[1]:x coordinate; data[2]:y coordinate.*/
			gf_debug(INFO_LOG, "[%s] : TOUCHPANEL_FPEVENT_DOWN,event=%ld,x=%d,y=%d\n", \
								__func__, event, coordinate->x, coordinate->y);
			gf_netlink_send(gf_dev, GF_NETLINK_FINGER_DOWN, coordinate);
			break;
		default:
			gf_debug(INFO_LOG, "[%s] : Ignore event, event=%ld\n", __func__, event);
		break;
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	gf_netlink_send(gf_dev, GF_NETLINK_IRQ, NULL);
	gf_dev->sig_count++;
...
static int gf_probe(struct spi_device *spi)
{
	...
	status = mtk_disp_notifier_register("fingerprint", &gf_dev->notifier);
	if (status < 0) {
		gf_debug(ERR_LOG, "%s, mtk_disp_notifier_register fail: %d\n", __func__, status);
	}
	/* register touchpanel fp event notify */
	gf_dev->tp_notifier.notifier_call = nt_tp_notifier_callback;
	status = touchpanel_event_register_notifier(&gf_dev->tp_notifier);
	if (status < 0) {
		gf_debug(ERR_LOG, "%s, touchpanel_event_register_notifier fail: %d\n", __func__, status);
	}
...
static void gf_remove(struct spi_device *spi)
{
	...
	touchpanel_event_unregister_notifier(&gf_dev->tp_notifier);
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	if (mtk_disp_notifier_unregister(&gf_dev->notifier))
		gf_debug(ERR_LOG, "%s mtk_disp_notifier_unregister failure!\n", __func__);
#endif
