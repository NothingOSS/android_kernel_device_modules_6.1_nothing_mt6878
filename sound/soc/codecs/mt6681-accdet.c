...
static struct accdet_priv mt6681_accdet[] = {
	{
		.caps = 0,
	},
};
...
static void dump_register(void)
{
	...
	if (dump_reg) {
		...
		for (addr = MT6681_RG_AUDPREAMPLON_ADDR; addr <= end_addr; addr += 4) {
			idx = addr;
			pr_info("(0x%x)=0x%x (0x%x)=0x%x (0x%x)=0x%x (0x%x)=0x%x\n",
				idx, pmic_read(idx),
				idx+1, pmic_read(idx+1),
		...
		pr_info("(0x%x)=0x%x (0x%x)=0x%x (0x%x)=0x%x\n",
			MT6681_RG_ACCDET_CK_PDN_ADDR,
			pmic_read(MT6681_RG_ACCDET_CK_PDN_ADDR),
		...
		pr_info("(0x%x)=0x%x (0x%x)=0x%x (0x%x)=0x%x (0x%x)=0x%x\n",
			...
			pmic_read(MT6681_RG_INT_MASK_ACCDET_ADDR),
			MT6681_RG_INT_STATUS_ACCDET_ADDR,
			pmic_read(MT6681_RG_INT_STATUS_ACCDET_ADDR),
			MT6681_RG_AUDMICBIAS1VREF_ADDR,
			pmic_read(MT6681_RG_AUDMICBIAS1VREF_ADDR),
	...
	const char *buf, size_t count)
{
	...
	if (addr_tmp > MT6681_ACCDET_MON_FLAG_EN_ADDR)
...
int mt6681_accdet_read_audio_res(unsigned int res_value)
{
...
EXPORT_SYMBOL(mt6681_accdet_read_audio_res);
...
{
	unsigned short efuseval = 0, efuseval2 = 0;
	int ret = 0;
	int tmp_div;
	 ...
	 */
	ret = nvmem_device_read(accdet->accdet_efuse, 114, 1, &efuseval);
	ret = nvmem_device_read(accdet->accdet_efuse, 115, 1, &efuseval2);
	accdet->auxadc_offset = (((efuseval >> 4) | ((efuseval2 << 4))) & ACCDET_CALI_MASK0);
...
{
	unsigned short tmp_val = 0, tmp_val2 = 0;
	 ...
	 */
	ret = nvmem_device_read(accdet->accdet_efuse, 116, 1, &tmp_val);
	ret = nvmem_device_read(accdet->accdet_efuse, 117, 1, &tmp_val2);
	tmp_8bit = (((tmp_val >> 4) | ((tmp_val2 << 4))) & ACCDET_CALI_MASK0);
	accdet_dts.four_key.mid = tmp_8bit << 2;

	ret = nvmem_device_read(accdet->accdet_efuse, 117, 1, &tmp_val);
	ret = nvmem_device_read(accdet->accdet_efuse, 118, 1, &tmp_val2);
	tmp_8bit = (((tmp_val >> 4) | ((tmp_val2 << 4))) & ACCDET_CALI_MASK0);
	accdet_dts.four_key.voice = tmp_8bit << 2;

	ret = nvmem_device_read(accdet->accdet_efuse, 118, 1, &tmp_val);
	ret = nvmem_device_read(accdet->accdet_efuse, 119, 1, &tmp_val2);
	tmp_8bit = (((tmp_val >> 4) | ((tmp_val2 << 4))) & ACCDET_CALI_MASK0);
...
static void multi_key_detection(u32 cur_AB)
{
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_AP_GPIO_EINT)) {
		...
		irq_bit = !(pmic_read(MT6681_ACCDET_IRQ_ADDR) & ACCDET_EINT_IRQ_B2_B3);
...
static inline void clear_accdet_eint_check(u32 eintid)
{
	...
	if ((eintid & PMIC_EINT0) == PMIC_EINT0) {
		while ((pmic_read(MT6681_ACCDET_EINT0_IRQ_ADDR) & ACCDET_EINT0_IRQ_B2) &&
		       (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT)))
			;
		pmic_write_clr(MT6681_ACCDET_EINT0_IRQ_CLR_ADDR,
			       MT6681_ACCDET_EINT0_IRQ_CLR_SHIFT);
		/* Unmask accdet and clear status */
		pmic_write_set(MT6681_AUD_TOP_INT_MASK_CON0_CLR,
	...
	if ((eintid & PMIC_EINT1) == PMIC_EINT1) {
		while ((pmic_read(MT6681_ACCDET_EINT1_IRQ_ADDR) & ACCDET_EINT1_IRQ_B3) &&
		       (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT)))
			;
		pmic_write_clr(MT6681_ACCDET_IRQ_ADDR,
			       MT6681_ACCDET_EINT1_IRQ_CLR_SHIFT);
		/* Unmask accdet and clear status */
		pmic_write_set(MT6681_AUD_TOP_INT_MASK_CON0_CLR,
			       MT6681_RG_INT_MASK_ACCDET_EINT1_SHIFT);
		pmic_write_set(MT6681_RG_INT_STATUS_ACCDET_ADDR,
...
static u32 adjust_eint_analog_setting(void)
{
	...
	if (accdet_dts.eint_detect_mode == 0x4) {
		if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
			...
			pmic_write_set(MT6681_RG_EINT0CONFIGACCDET_ADDR,
				       MT6681_RG_EINT0CONFIGACCDET_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
			/* enable RG_EINT1CONFIGACCDET */
			pmic_write_set(MT6681_RG_EINT1CONFIGACCDET_ADDR,
				       MT6681_RG_EINT1CONFIGACCDET_SHIFT);
		...
		if ((accdet_dts.eint_use_ext_res == 0x3) ||
			...
			pmic_write_set(MT6681_RG_EINT0HIRENB_ADDR, MT6681_RG_EINT0HIRENB_SHIFT);
		}
	}
	return 0;
}
...
static u32 adjust_eint_digital_setting(void)
{
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT0)) {
		/* disable inverter */
		pmic_write_clr(MT6681_ACCDET_EINT0_INVERTER_SW_EN_ADDR,
			       MT6681_ACCDET_EINT0_INVERTER_SW_EN_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
		/* disable inverter */
		pmic_write_clr(MT6681_ACCDET_EINT1_INVERTER_SW_EN_ADDR,
			       MT6681_ACCDET_EINT1_INVERTER_SW_EN_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
		/* disable inverter */
		pmic_write_clr(MT6681_ACCDET_EINT0_INVERTER_SW_EN_ADDR,
			       MT6681_ACCDET_EINT0_INVERTER_SW_EN_SHIFT);
	...
	if ((accdet_dts.eint_detect_mode == 0x1) ||
		...
		pmic_write_clr(MT6681_ACCDET_EINT0_CEN_STABLE_ADDR,
			       MT6681_ACCDET_EINT0_CEN_STABLE_SHIFT);
		ret = get_moisture_sw_auxadc_check();
		/* disable mtest en */
		pmic_write_clr(MT6681_RG_MTEST_EN_ADDR, MT6681_RG_MTEST_EN_SHIFT);
		pmic_write_clr(MT6681_AUDACCDETAUXADCSWCTRL_SEL_ADDR,
			       MT6681_AUDACCDETAUXADCSWCTRL_SEL_SHIFT);
		return ret;
	}
	if (accdet_dts.eint_detect_mode == 0x3) {
		pmic_write_clr(MT6681_ACCDET_EINT0_CEN_STABLE_ADDR,
			       MT6681_ACCDET_EINT0_CEN_STABLE_SHIFT);
		pmic_write_set(MT6681_ACCDET_EINT_CTURBO_SEL_ADDR,
			       MT6681_ACCDET_EINT_CTURBO_SEL_SHIFT);
		pmic_write_set(MT6681_ACCDET_EINT0_CTURBO_SW_ADDR,
			       MT6681_ACCDET_EINT0_CTURBO_SW_SHIFT);
		pr_info("auxadc T1\r");
		ret = get_moisture_sw_auxadc_check();
		if ((ret == M_WATER_IN) || (ret == M_HP_PLUG_IN)) {
			pmic_write_clr(MT6681_ACCDET_EINT_CTURBO_SEL_ADDR,
				       MT6681_ACCDET_EINT_CTURBO_SEL_SHIFT);
			pmic_write_clr(MT6681_ACCDET_EINT0_CTURBO_SW_ADDR,
				       MT6681_ACCDET_EINT0_CTURBO_SW_SHIFT);
		}
		return ret;
	}
	...
	if (accdet_dts.eint_detect_mode == 0x4) {
		if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
			...
			/* set DA stable signal */
			pmic_write_clr(MT6681_ACCDET_EINT1_CEN_STABLE_ADDR,
				       MT6681_ACCDET_EINT1_CEN_STABLE_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
			/* set DA stable signal */
			pmic_write_clr(MT6681_ACCDET_EINT0_CEN_STABLE_ADDR,
				       MT6681_ACCDET_EINT0_CEN_STABLE_SHIFT);
...
static u32 adjust_moisture_digital_setting(u32 eintID)
{
	if ((accdet_dts.moisture_detect_mode == 0x1) ||
		...
		if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT0)) {
			...
			pmic_write_set(MT6681_ACCDET_EINT0_M_SW_EN_ADDR,
				       MT6681_ACCDET_EINT0_M_SW_EN_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
			/* wk1, enable moisture detection */
			pmic_write_set(MT6681_ACCDET_EINT1_M_SW_EN_ADDR,
				       MT6681_ACCDET_EINT1_M_SW_EN_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
			/* wk1, enable moisture detection */
			pmic_write_set(MT6681_ACCDET_EINT0_M_SW_EN_ADDR,
				       MT6681_ACCDET_EINT0_M_SW_EN_SHIFT);
		...
		/* new */
		pmic_write_set(MT6681_ACCDET_EINT0_CMPMEN_SW_ADDR,
			       MT6681_ACCDET_EINT0_CMPMEN_SW_SHIFT);
		pr_info("%s [M_Check_Flow] (0x%x)=0x%x\n", __func__,
			MT6681_ACCDET_EINT0_CMPMEN_SW_ADDR,
			pmic_read(MT6681_ACCDET_EINT0_CMPMEN_SW_ADDR));
	}
	return 0;
}
...
static u32 adjust_moisture_analog_setting(u32 eintID)
{
	if (accdet_dts.moisture_detect_mode == 0x1) {
		...
		pmic_write_mset(MT6681_RG_EINTCOMPVTH_ADDR,
				MT6681_RG_EINTCOMPVTH_SHIFT,
				MT6681_RG_EINTCOMPVTH_MASK,
		...
		pmic_write_mset(MT6681_RG_EINTCOMPVTH_ADDR,
				...
				accdet_dts.moisture_comp_vth);
		/* Enable mtest en */
		pmic_write_set(MT6681_RG_MTEST_EN_ADDR, MT6681_RG_MTEST_EN_SHIFT);
		/* select PAD_HP_EINT for moisture detection */
		pmic_write_clr(MT6681_RG_MTEST_SEL_ADDR, MT6681_RG_MTEST_SEL_SHIFT);
	...
	if (moistureID == M_PLUG_IN) {
		if (accdet->thing_in_flag == true) {
			...
			if (accdet_dts.moisture_detect_mode != 0x5) {
				/* wk2 */
				pmic_write_set(MT6681_RG_EINT0HIRENB_ADDR,
					       MT6681_RG_EINT0HIRENB_SHIFT);
			}
...
static void recover_eint_analog_setting(void)
{
	...
	if (accdet_dts.eint_detect_mode == 0x4) {
		if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
			...
			pmic_write_clr(MT6681_RG_EINT0CONFIGACCDET_ADDR,
				       MT6681_RG_EINT0CONFIGACCDET_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
			/* disable RG_EINT1CONFIGACCDET */
			pmic_write_clr(MT6681_RG_EINT1CONFIGACCDET_ADDR,
				       MT6681_RG_EINT1CONFIGACCDET_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
			/* disable RG_EINT0CONFIGACCDET */
			pmic_write_clr(MT6681_RG_EINT0CONFIGACCDET_ADDR,
		...
		pmic_write_clr(MT6681_RG_EINT0HIRENB_ADDR,
			       MT6681_RG_EINT0HIRENB_SHIFT);
	}
}
...
static void recover_eint_digital_setting(void)
{
	pr_info("[M_Check_Flow] OUT.3 cur_eint:%d (0x%x)=0x%x\n"
		, accdet->cur_eint_state, MT6681_ACCDET_EINT0_CMPMEN_SW_ADDR
		, pmic_read(MT6681_ACCDET_EINT0_CMPMEN_SW_ADDR));
	if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
		/* wk1, disable moisture detection */
		pmic_write_clr(MT6681_ACCDET_EINT0_M_SW_EN_ADDR,
			       MT6681_ACCDET_EINT0_M_SW_EN_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
		/* wk1, disable moisture detection */
		pmic_write_clr(MT6681_ACCDET_EINT1_M_SW_EN_ADDR,
			       MT6681_ACCDET_EINT1_M_SW_EN_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
		/* wk1, disable moisture detection */
		pmic_write_clr(MT6681_ACCDET_EINT0_M_SW_EN_ADDR,
	...
	if (accdet_dts.eint_detect_mode == 0x4) {
		...
		if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
			/* enable eint0cen */
			pmic_write_set(MT6681_ACCDET_EINT0_CEN_STABLE_ADDR,
				       MT6681_ACCDET_EINT0_CEN_STABLE_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
			/* enable eint1cen */
			pmic_write_set(MT6681_ACCDET_EINT1_CEN_STABLE_ADDR,
				       MT6681_ACCDET_EINT1_CEN_STABLE_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
			/* enable eint0cen */
			pmic_write_set(MT6681_ACCDET_EINT0_CEN_STABLE_ADDR,
	...
	if (accdet_dts.eint_detect_mode != 0x1) {
		if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
			/* enable inverter */
			pmic_write_set(MT6681_ACCDET_EINT0_INVERTER_SW_EN_ADDR,
				       MT6681_ACCDET_EINT0_INVERTER_SW_EN_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
			/* enable inverter */
			pmic_write_set(MT6681_ACCDET_EINT1_INVERTER_SW_EN_ADDR,
				       MT6681_ACCDET_EINT1_INVERTER_SW_EN_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
			/* enable inverter */
			pmic_write_set(MT6681_ACCDET_EINT0_INVERTER_SW_EN_ADDR,
...
static void recover_moisture_analog_setting(void)
{
	if (accdet_dts.moisture_detect_mode == 0x1) {
		...
		pmic_write_mset(MT6681_RG_EINTCOMPVTH_ADDR,
				MT6681_RG_EINTCOMPVTH_SHIFT,
				MT6681_RG_EINTCOMPVTH_MASK, 0x2);
	} else if (accdet_dts.moisture_detect_mode == 0x2) {
...
static u32 get_triggered_eint(void)
{
	...
	u32 irq_status = pmic_read(MT6681_ACCDET_EINT0_IRQ_ADDR);
...
{
	/* enable ACCDET unit */
	pmic_write_set(MT6681_ACCDET_SW_EN_ADDR, MT6681_ACCDET_SW_EN_SHIFT);
}
...
static void dis_micbias_work_callback(struct work_struct *work)
{
	 ...
	 */
	eintID = pmic_read_mbit(MT6681_ACCDET_EINT0_MEM_IN_ADDR,
				MT6681_ACCDET_EINT0_MEM_IN_SHIFT,
				MT6681_ACCDET_EINT0_MEM_IN_MASK);
...
static void eint_work_callback(struct work_struct *work)
{
	if (accdet->cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		/* disable accdet_sw_en=0
		 */
		pmic_write_clr(MT6681_ACCDET_SW_EN_ADDR,
...
void accdet_set_debounce(int state, unsigned int debounce)
{
	...
	case accdet_state000:
		...
		pmic_write_word(MT6681_ACCDET_DEBOUNCE0_L_ADDR, debounce);
		break;
	case accdet_state001:
		pmic_write_word(MT6681_ACCDET_DEBOUNCE1_L_ADDR, debounce);
		break;
	case accdet_state010:
		pmic_write_word(MT6681_ACCDET_DEBOUNCE2_L_ADDR, debounce);
		break;
	case accdet_state011:
		pmic_write_word(MT6681_ACCDET_DEBOUNCE3_L_ADDR, debounce);
		break;
	case accdet_auxadc:
		/* set auxadc debounce:0x42(2ms) */
		pmic_write_word(MT6681_ACCDET_CONNECT_AUXADC_TIME_DIG_L_ADDR, debounce);
		break;
	case eint_state000:
		pmic_write_mset(MT6681_ACCDET_EINT_DEBOUNCE0_ADDR,
				MT6681_ACCDET_EINT_DEBOUNCE0_SHIFT,
				MT6681_ACCDET_EINT_DEBOUNCE0_MASK,
				debounce);
		break;
	case eint_state001:
		pmic_write_mset(MT6681_ACCDET_EINT_DEBOUNCE1_ADDR,
				MT6681_ACCDET_EINT_DEBOUNCE1_SHIFT,
				MT6681_ACCDET_EINT_DEBOUNCE1_MASK,
				debounce);
		break;
	case eint_state010:
		pmic_write_mset(MT6681_ACCDET_EINT_DEBOUNCE2_ADDR,
				MT6681_ACCDET_EINT_DEBOUNCE2_SHIFT,
				MT6681_ACCDET_EINT_DEBOUNCE2_MASK,
				debounce);
		break;
	case eint_state011:
		pmic_write_mset(MT6681_ACCDET_EINT_DEBOUNCE3_ADDR,
				MT6681_ACCDET_EINT_DEBOUNCE3_SHIFT,
				MT6681_ACCDET_EINT_DEBOUNCE3_MASK,
				debounce);
		break;
	case eint_inverter_state000:
		pmic_write(MT6681_ACCDET_EINT_INVERTER_DEBOUNCE_ADDR, debounce);
		break;
	default:
...
static u32 config_moisture_detect_1_0(void)
{
	...
	pmic_write_clr(MT6681_RG_ACCDET2AUXSWEN_ADDR,
		       MT6681_RG_ACCDET2AUXSWEN_SHIFT);
	pmic_write_set(MT6681_AUDACCDETAUXADCSWCTRL_SEL_ADDR,
		       MT6681_AUDACCDETAUXADCSWCTRL_SEL_SHIFT);
	pmic_write_clr(MT6681_AUDACCDETAUXADCSWCTRL_SW_ADDR,
		       MT6681_AUDACCDETAUXADCSWCTRL_SW_SHIFT);
...
static u32 config_moisture_detect_2_1(void)
{
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT0)) {
		...
		pmic_write_set(MT6681_RG_EINT0CTURBO_ADDR, MT6681_RG_EINT0CTURBO_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
		pmic_write_set(MT6681_RG_EINT1CTURBO_ADDR, MT6681_RG_EINT1CTURBO_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
		pmic_write_set(MT6681_RG_EINT0CTURBO_ADDR, MT6681_RG_EINT0CTURBO_SHIFT);
		pmic_write_set(MT6681_RG_EINT1CTURBO_ADDR, MT6681_RG_EINT1CTURBO_SHIFT);
	}
	 ...
	 * golden setting
	 * pmic_write_mset(MT6681_RG_ACCDETSPARE_ADDR, 0x3, 0x1F, 0xA);
	 */
	...
	/* set moisture reference voltage MVTH */
	pmic_write_mset(MT6681_RG_ACCDETSPARE_ADDR, 0x3, 0x1F, eintvth);

	ret = nvmem_device_read(accdet->accdet_efuse, 124, 1, &efuseval);
	vref_mvth2 = (int)((efuseval >> 4) & ACCDET_CALI_MASK0);
	...
	pmic_write_mset(MT6681_RG_MVTH2EN_ADDR, MT6681_RG_MVTH2EN_SHIFT,
			0x1F, vref_mvth2);

	ret = nvmem_device_read(accdet->accdet_efuse, 125, 1, &efuseval);
...
static u32 config_moisture_detect_2_1_1(void)
{
	u32 efuseval = 0, efuseval2 = 0, vref_1v = 0, ret = 0;
...
static void accdet_irq_handle(void)
{
	...
	if ((irq_status & ACCDET_IRQ_B0) && (eintID == 0)) {
		...
		/* check EINT0 status */
		accdet->eint_id = pmic_read_mbit(MT6681_ACCDET_EINT0_MEM_IN_ADDR,
						 MT6681_ACCDET_EINT0_MEM_IN_SHIFT,
						 MT6681_ACCDET_EINT0_MEM_IN_MASK);
		if (get_moisture_det_en() == 0x1) {
...
static void config_digital_moisture_init_by_mode(void)
{
	...
	pmic_write(MT6681_ACCDET_EINT_CMPMEN_PWM_THRESH_ADDR,
		   (accdet_dts.pwm_deb.eint_pwm_width << 4 |
		    accdet_dts.pwm_deb.eint_pwm_thresh));

	/* DA signal stable */
	if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
		pmic_write_word(MT6681_ACCDET_DA_STABLE_ADDR, ACCDET_EINT0_STABLE_VAL);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
		pmic_write_word(MT6681_ACCDET_DA_STABLE_ADDR, ACCDET_EINT1_STABLE_VAL);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
		pmic_write_word(MT6681_ACCDET_DA_STABLE_ADDR, ACCDET_EINT0_STABLE_VAL);
		pmic_write_word(MT6681_ACCDET_DA_STABLE_ADDR, ACCDET_EINT1_STABLE_VAL);
	}
	 ...
	 * MT6681_ACCDET_HWMODE_EN_ADDR[2:1]
	 */
	if (accdet_dts.moisture_detect_mode == 0x1) {
		/* disable moisture detection function */
		pmic_write_clr(MT6681_ACCDET_EINT_M_DETECT_EN_ADDR,
			MT6681_ACCDET_EINT_M_DETECT_EN_SHIFT);
		/* wk1, disable hwmode */
		pmic_write_word(MT6681_ACCDET_HWMODE_EN_ADDR, 0x0);
		/* disable inverter detection */
		if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
			pmic_write_set(MT6681_ACCDET_EINT0_SW_EN_ADDR,
				       MT6681_ACCDET_EINT0_SW_EN_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
			pmic_write_set(MT6681_ACCDET_EINT1_SW_EN_ADDR,
				       MT6681_ACCDET_EINT1_SW_EN_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
			pmic_write_set(MT6681_ACCDET_EINT0_SW_EN_ADDR,
				       MT6681_ACCDET_EINT0_SW_EN_SHIFT);
			pmic_write_set(MT6681_ACCDET_EINT1_SW_EN_ADDR,
		...
		pmic_write_clr(MT6681_ACCDET_EINT_M_DETECT_EN_ADDR,
			MT6681_ACCDET_EINT_M_DETECT_EN_SHIFT);
		/* wk1, disable hwmode */
		pmic_write_word(MT6681_ACCDET_HWMODE_EN_ADDR, 0x100);
	} else if (accdet_dts.moisture_detect_mode == 0x3) {
		/* disable moisture detection function */
		pmic_write_clr(MT6681_ACCDET_EINT_M_DETECT_EN_ADDR,
			MT6681_ACCDET_EINT_M_DETECT_EN_SHIFT);
		pmic_write_set(MT6681_ACCDET_EINT_CMPMOUT_SEL_ADDR,
			MT6681_ACCDET_EINT_CMPMOUT_SEL_SHIFT);
		pmic_write_set(MT6681_ACCDET_EINT_CMPMEN_SEL_ADDR,
			MT6681_ACCDET_EINT_CMPMEN_SEL_SHIFT);
		/* wk1, disable hwmode */
		pmic_write_word(MT6681_ACCDET_HWMODE_EN_ADDR, 0x100);
	} else if (accdet_dts.moisture_detect_mode == 0x4) {
		/* enable moisture detection function */
		pmic_write_set(MT6681_ACCDET_EINT_M_DETECT_EN_ADDR,
			MT6681_ACCDET_EINT_M_DETECT_EN_SHIFT);
		/* new MT6681 */
		pmic_write_set(MT6681_ACCDET_EINT0_CMPMEN_STABLE_ADDR,
			MT6681_ACCDET_EINT0_CMPMEN_STABLE_SHIFT);
		pmic_write_set(MT6681_ACCDET_EINT_CMPMEN_SEL_ADDR,
			MT6681_ACCDET_EINT_CMPMEN_SEL_SHIFT);
		/* wk1, enable hwmode */
		pmic_write_word(MT6681_ACCDET_HWMODE_EN_ADDR, 0x100);
		...
		/* wk1, enable hwmode */
		pmic_write_word(MT6681_ACCDET_HWMODE_EN_ADDR, 0x500);
		pmic_write_set(MT6681_RG_HPLOUTPUTSTBENH_VAUDP18_ADDR,
			MT6681_RG_HPLOUTPUTSTBENH_VAUDP18_SHIFT);
		pmic_write_set(MT6681_RG_HPROUTPUTSTBENH_VAUDP18_ADDR,
			MT6681_RG_HPROUTPUTSTBENH_VAUDP18_SHIFT);
	} else {
		/* wk1, disable hwmode */
		pmic_write_word(MT6681_ACCDET_HWMODE_EN_ADDR, 0x100);
	}
	...
	/* enable PWM */
	pmic_write_word(MT6681_ACCDET_CMP_PWM_EN_ADDR, 0x67);
...
{
	if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT0)) {
		pmic_write_set(MT6681_RG_EINT0EN_ADDR, MT6681_RG_EINT0EN_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
		pmic_write_set(MT6681_RG_EINT1EN_ADDR, MT6681_RG_EINT1EN_SHIFT);
	} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
		pmic_write_set(MT6681_RG_EINT0EN_ADDR, MT6681_RG_EINT0EN_SHIFT);
		pmic_write_set(MT6681_RG_EINT1EN_ADDR, MT6681_RG_EINT1EN_SHIFT);
	}
	...
	if (accdet_dts.eint_detect_mode != 0x1) {
		...
		pmic_write_mset(MT6681_RG_ACCDETSPARE_ADDR,
				MT6681_RG_ACCDETSPARE_SHIFT,
				0x3, 0x3);
	}
...
static void accdet_init_once(void)
{
	...
	pmic_write_set(MT6681_RG_ACCDET_RST_ADDR, MT6681_RG_ACCDET_RST_SHIFT);
	pmic_write_clr(MT6681_RG_ACCDET_RST_ADDR, MT6681_RG_ACCDET_RST_SHIFT);

	/* clear high micbias1 voltage setting */
	pmic_write_mclr(MT6681_RG_AUDMICBIAS1HVEN_ADDR,
			MT6681_RG_AUDMICBIAS1HVEN_SHIFT, 0x3);
	/* clear micbias1 voltage */
	pmic_write_mclr(MT6681_RG_AUDMICBIAS1VREF_ADDR,
			MT6681_RG_AUDMICBIAS1VREF_SHIFT, 0x7);
	/* open top accdet interrupt */
	pmic_write_set(MT6681_RG_INT_EN_ACCDET_ADDR,
		       MT6681_RG_INT_EN_ACCDET_SHIFT);

	/* init pwm frequency, duty & rise/falling delay */
	pmic_write_word(MT6681_ACCDET_PWM_WIDTH_L_ADDR,
			REGISTER_VAL(cust_pwm_deb->pwm_width));
	pmic_write_word(MT6681_ACCDET_PWM_THRESH_L_ADDR,
			REGISTER_VAL(cust_pwm_deb->pwm_thresh));
	pmic_write_word(MT6681_ACCDET_RISE_DELAY_L_ADDR,
	...
	if (accdet_dts.mic_vol <= 7) {
		/* micbias1 <= 2.7V */
		reg = pmic_read(MT6681_RG_AUDMICBIAS1VREF_ADDR);
		pmic_write(MT6681_RG_AUDMICBIAS1VREF_ADDR,
			reg | (accdet_dts.mic_vol<<MT6681_RG_AUDMICBIAS1VREF_SHIFT)
			| RG_AUD_MICBIAS1_LOWP_EN);
	} else if (accdet_dts.mic_vol == 8) {
		/* micbias1 = 2.8v */
		reg = pmic_read(MT6681_RG_AUDMICBIAS1LOWPEN_ADDR);
		pmic_write(MT6681_RG_AUDMICBIAS1LOWPEN_ADDR,
			reg | RG_AUD_MICBIAS1_LOWP_EN);
		reg = pmic_read(MT6681_RG_AUDMICBIAS1HVEN_ADDR);
		pmic_write(MT6681_RG_AUDMICBIAS1HVEN_ADDR,
			reg | (3<<MT6681_RG_AUDMICBIAS1HVEN_SHIFT));
		...
		reg = pmic_read(MT6681_RG_AUDMICBIAS1HVEN_ADDR);
		pmic_write(MT6681_RG_AUDMICBIAS1HVEN_ADDR,
			reg | (1<<MT6681_RG_AUDMICBIAS1HVEN_SHIFT));
	}
	/* mic mode setting */
	reg = pmic_read(MT6681_RG_AUDACCDETMICBIAS0PULLLOW_ADDR);
	if (accdet_dts.mic_mode == HEADSET_MODE_1) {
		/* ACC mode*/
		pmic_write(MT6681_RG_AUDACCDETMICBIAS0PULLLOW_ADDR,
			reg | RG_ACCDET_MODE_ANA11_MODE1);
		/* enable analog fast discharge */
		pmic_write_set(MT6681_RG_ANALOGFDEN_ADDR,
			MT6681_RG_ANALOGFDEN_SHIFT);
		pmic_write_mset(MT6681_RG_ACCDET_PL_ESDMOS_ADDR,
				MT6681_RG_ACCDET_PL_ESDMOS_SHIFT, 0x3, 0x3);
	} else if (accdet_dts.mic_mode == HEADSET_MODE_2) {
		/* DCC mode Low cost mode without internal bias*/
		pmic_write(MT6681_RG_AUDACCDETMICBIAS0PULLLOW_ADDR,
			reg | RG_ACCDET_MODE_ANA11_MODE2);
		/* enable analog fast discharge */
		pmic_write_mset(MT6681_RG_ANALOGFDEN_ADDR,
			MT6681_RG_ANALOGFDEN_SHIFT, 0x3, 0x3);
		...
		pmic_write_set(MT6681_RG_AUDMICBIAS1DCSW1PEN_ADDR,
				MT6681_RG_AUDMICBIAS1DCSW1PEN_SHIFT);
		/* enable analog fast discharge */
		pmic_write_mset(MT6681_RG_ANALOGFDEN_ADDR,
			MT6681_RG_ANALOGFDEN_SHIFT, 0x3, 0x3);
	}
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT_IRQ)) {
		...
		if (HAS_CAP(accdet->data->caps,	ACCDET_PMIC_EINT0)) {
			...
			pmic_write_set(MT6681_RG_AUD_INT_CON0_SET_ADDR,
				       MT6681_RG_INT_EN_ACCDET_EINT0_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT1)) {
			/* open top interrupt eint1 */
			pmic_write_set(MT6681_RG_AUD_INT_CON0_SET_ADDR,
				       MT6681_RG_INT_EN_ACCDET_EINT1_SHIFT);
		} else if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_BI_EINT)) {
			/* open top interrupt eint0 & eint1 */
			pmic_write_mset(MT6681_RG_AUD_INT_CON0_SET_ADDR,
					MT6681_RG_INT_EN_ACCDET_EINT0_SHIFT,
					MT6681_RG_INT_EN_ACCDET_EINT0_MASK, 0x3);
		}
	}
...
static inline void accdet_init(void)
{
	...
	pmic_write_set(MT6681_ACCDET_SEQ_INIT_ADDR, MT6681_ACCDET_SEQ_INIT_SHIFT);
	usleep_range(2000, 3000);
	pmic_write_clr(MT6681_ACCDET_SEQ_INIT_ADDR, MT6681_ACCDET_SEQ_INIT_SHIFT);
...
void mt6681_accdet_modify_vref_volt(void)
{
}
EXPORT_SYMBOL(mt6681_accdet_modify_vref_volt);
...
int mt6681_accdet_init(struct snd_soc_component *component, struct snd_soc_card *card)
{
	int ret;
...
EXPORT_SYMBOL_GPL(mt6681_accdet_init);

static void ipi_work_callback(struct work_struct *work)
{
	if (accdet_dts.accdet_irq_gpio_enable == 0x0)
		accdet_irq_handle();
}
...
/* Module information */
MODULE_DESCRIPTION("MT6681 ALSA SoC accdet driver");
MODULE_AUTHOR("Shang-ming.huang <shang-ming.huang@mediatek.com>");
MODULE_LICENSE("GPL");
