...
enum tfa_error tfa_dev_start(struct tfa_device *tfa, int next_profile, int vstep)
{
    ...
    unsigned short old_profile = -1, old_vstep = -1;
	...
	if (tfa->bus != 0) { /* non i2c  */
		...
		if (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, next_profile), ".standby") != NULL) {
		    ...
		    /* 9894 4PA project support channel swop */
            tfa9894_init_chan_sel(tfa, next_profile);

            if ( (tfa->rev & 0xff) == 0x96 ) {
                old_profile = (unsigned short)tfa_dev_get_swprof(tfa);
        		old_vstep = (unsigned short)tfa_dev_get_swvstep(tfa);
            }
            ...
            if ( (tfa->rev & 0xff) == 0x96 ) { // 9896's mtp reg will set zero after wakeup, need reset here.
                tfa_dev_set_swprof(tfa, old_profile);
        		tfa_dev_set_swvstep(tfa, old_vstep);
            }
		}
	}
	active_profile = tfa_dev_get_swprof(tfa);
...
enum tfa_error tfa_dev_stop(struct tfa_device *tfa)
{
	...
	} else if ((tfa->rev & 0xff) == 0x96) {
        temps = tfa_get_bf(tfa, TFA9896_BF_TEMPS);
        pr_info("TEMPS = %d, tries=%d\n", temps, tries);
        while((temps != 0x100) && (tries < 100)) {
            pr_err("Waiting for state machine to powerdown, TEMPS = %d, tries=%d\n", temps, tries);
            msleep_interruptible(10);
            temps = tfa_get_bf(tfa, TFA9896_BF_TEMPS);
            tries++;
        }
        pr_err("exit, TEMPS = %d, tries=%d\n", temps, tries);
    }
	return err;
}
