	...
	struct device_attribute *attr, char *buf)
{
	...
	if (is_single_cali) {
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
				len += snprintf(buf+len, PAGE_SIZE-len, "%d", cali_re[i]);
		}
		len += snprintf(buf+len, PAGE_SIZE-len, "\n");
	}
...
static ssize_t re_show_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	...
	ret = kstrtoint(buf, 10, &cali_re);
	aw_dev_info(dev, "re_show_store buf = %s, cali_re = %d", buf, cali_re);
