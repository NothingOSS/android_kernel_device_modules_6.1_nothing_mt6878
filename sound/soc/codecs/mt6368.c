...
static void zcd_enable(struct mt6368_priv *priv, bool enable, int device)
{
	if (enable) {
		switch (device) {
		case DEVICE_RCV:
			regmap_update_bits(priv->regmap,
					   MT6368_AUDDEC_ANA_CON22,
					   RG_AUDZCDMUXSEL_VAUDP32_MASK_SFT,
					   0x2 << RG_AUDZCDMUXSEL_VAUDP32_SFT);
			break;
		case DEVICE_LO:
			regmap_update_bits(priv->regmap,
					   MT6368_AUDDEC_ANA_CON22,
					   RG_AUDZCDMUXSEL_VAUDP32_MASK_SFT,
					   0x0 << RG_AUDZCDMUXSEL_VAUDP32_SFT);
			break;
		case DEVICE_HP:
		default:
			regmap_update_bits(priv->regmap,
					   MT6368_AUDDEC_ANA_CON22,
					   RG_AUDZCDMUXSEL_VAUDP32_MASK_SFT,
					   0x1 << RG_AUDZCDMUXSEL_VAUDP32_SFT);
			break;
		}
		/* Enable ZCD, for minimize pop noise */
		/* timeout, 1 = 5ms, 0 = 30ms */
		regmap_update_bits(priv->regmap, MT6368_ZCD_CON0,
				   RG_AUDZCDTIMEOUTMODESEL_MASK_SFT,
				   0x0 << RG_AUDZCDTIMEOUTMODESEL_SFT);
		regmap_update_bits(priv->regmap, MT6368_ZCD_CON0,
				   RG_AUDZCDGAINSTEPSIZE_MASK_SFT,
				   0x0 << RG_AUDZCDGAINSTEPSIZE_SFT);
		regmap_update_bits(priv->regmap, MT6368_ZCD_CON0,
				   RG_AUDZCDGAINSTEPTIME_MASK_SFT,
				   0x5 << RG_AUDZCDGAINSTEPTIME_SFT);
		regmap_update_bits(priv->regmap, MT6368_ZCD_CON0,
				   RG_AUDZCDENABLE_MASK_SFT,
				   0x1 << RG_AUDZCDENABLE_SFT);
	} else {
		regmap_update_bits(priv->regmap, MT6368_AUDDEC_ANA_CON22,
				   RG_AUDZCDMUXSEL_VAUDP32_MASK_SFT,
				   0x4 << RG_AUDZCDMUXSEL_VAUDP32_SFT);
		zcd_disable(priv);
	}
}
				...
				int from, int to)
{
	...
	if (!is_valid_hp_pga_idx(from) || !is_valid_hp_pga_idx(to)) {
		dev_info(priv->dev, "%s(), volume index is not valid, from %d, to %d\n",
			 __func__, from, to);
		return;
	}
			  ...
			  int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		usleep_range(500, 520);
		regmap_read(priv->regmap, MT6368_AUDENC_ANA_CON21, &rc_l);
		dev_dbg(priv->dev, "%s(), after reset: rc_l = 0x%x\n",
			__func__, rc_l);
			  ...
			  int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		usleep_range(500, 520);
		regmap_read(priv->regmap, MT6368_AUDENC_ANA_CON22, &rc_r);
		dev_dbg(priv->dev, "%s(), after reset: rc_r = 0x%x\n",
			__func__, rc_r);
			       ...
			       const unsigned int detect_times)
{
	...
	if (dc_input < dc_init) {
		dev_info(priv->dev, "%s(), Wrong[%d] : dc_input(%d) < dc_init(%d)\n",
...
static int detect_impedance(struct mt6368_priv *priv)
{
	...
	if (priv->ops.adda_dl_gain_control)
		...
		dev_info(priv->dev, "%s(), adda_dl_gain_control ops not ready\n",
			 __func__);
		return 0;
	}
	...
	if (PARALLEL_OHM != 0) {
		if (impedance < PARALLEL_OHM) {
			...
			dev_info(priv->dev, "%s(), PARALLEL_OHM %d <= impedance %d\n",
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (priv->dev_counter[DEVICE_HP] <= 0 ||
		...
		dev_info(priv->dev, "%s(), counter %d <= 0 || mux_select[MUX_HP_L] %d != HP_MUX_HP_IMPEDANCE\n",
			 __func__,
