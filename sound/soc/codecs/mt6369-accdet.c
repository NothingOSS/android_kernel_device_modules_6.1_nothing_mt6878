...
static struct accdet_priv mt6369_accdet[] = {
	{
		.caps = 0,
	},
};

static const struct of_device_id mt6369_accdet_of_match[] = {
	{
...
MODULE_DEVICE_TABLE(of, mt6369_accdet_of_match);
...
static void dump_register(void)
{
	...
	if (dump_reg) {
		...
		pr_notice("(0x%x)=0x%x\n", RG_HPLOUTPUTSTBENH_VAUDP15_ADDR,
			accdet_read(RG_HPLOUTPUTSTBENH_VAUDP15_ADDR));
...
int mt6369_accdet_read_audio_res(unsigned int res_value)
{
...
EXPORT_SYMBOL_GPL(mt6369_accdet_read_audio_res);
...
static void config_digital_init_by_mode(void)
{
	...
	if (accdet_dts.moisture_detect_mode == 0x1) {
		...
		accdet_update_bit(RG_HPROUTPUTSTBENH_VAUDP15_ADDR,
...
static void accdet_init_once(void)
{
	...
	/* NOTE: mt6369 doesn't have HV setting */
...
void mt6369_accdet_late_init(unsigned long data)
{
...
EXPORT_SYMBOL_GPL(mt6369_accdet_late_init);

void mt6369_accdet_modify_vref_volt(void)
{
}
...
int mt6369_accdet_init(struct snd_soc_component *component,
...
static struct platform_driver mt6369_accdet_driver = {
	...
	.driver = {
		...
		.of_match_table = of_match_ptr(mt6369_accdet_of_match),
	},
};

module_platform_driver(mt6369_accdet_driver)

/* Module information */
MODULE_DESCRIPTION("MT6369 ALSA SoC accdet driver");
