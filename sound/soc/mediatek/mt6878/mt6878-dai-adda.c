	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_MTKAIF0_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6878_DAI_ADDA);
		}
		break;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6878_DAI_ADDA_CH34);
		}
...
static bool is_adda_mtkaif_need_phase_delay(struct mt6878_afe_private *afe_priv)
{
	if (mt6878_afe_gpio_is_prepared(MT6878_AFE_GPIO_DAT_MISO0_ON) &&
	    afe_priv->mtkaif_chosen_phase[0] < 0) {
		AUDIO_AEE("adda mtkaif miso0 calib fail");
		return false;
	}

	if (mt6878_afe_gpio_is_prepared(MT6878_AFE_GPIO_DAT_MISO1_ON) &&
...
/* mtkaif dmic */
static const char *const mt6878_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6878_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6878_adda_off_on_str),
...
static int mt6878_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_adda6_only_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_adda6_only_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_adda_dl_max_vol_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_adda_dl_max_vol_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_vow_enable_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_vow_enable_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6878_adda_enum[0],
		     mt6878_adda_dmic_get, mt6878_adda_dmic_set),
	SOC_ENUM_EXT("MTKAIF_ADDA6_ONLY", mt6878_adda_enum[0],
		     mt6878_adda6_only_get, mt6878_adda6_only_set),
	SOC_SINGLE_EXT("ADDA_DL_MAX_VOL",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6878_adda_dl_max_vol_get,
		       mt6878_adda_dl_max_vol_set),
...
int mt6878_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6878_dai_set_priv(afe, MT6878_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6878_dai_set_priv(afe, MT6878_DAI_ADDA_CH34,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;
	/* get ap mic type */
	...
	afe_priv->dai_priv[MT6878_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6878_DAI_ADDA];
	afe_priv->dai_priv[MT6878_DAI_AP_DMIC_CH34] =
		afe_priv->dai_priv[MT6878_DAI_ADDA_CH34];
