			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(kcontrol->id.name, "I2SIN1_LPBK")) {
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	struct mtk_afe_i2s_priv *i2sin4_priv = afe_priv->dai_priv[MT6878_DAI_I2S_IN4];
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	struct mtk_afe_i2s_priv *i2sin4_priv = afe_priv->dai_priv[MT6878_DAI_I2S_IN4];
	struct mtk_afe_i2s_priv *i2sout4_priv = afe_priv->dai_priv[MT6878_DAI_I2S_OUT4];
	unsigned int value = 0;
...
static int get_i2s_id_by_name(struct mtk_base_afe *afe,
			      const char *name)
{
	if (strncmp(name, "I2SIN1", 6) == 0)
		return MT6878_DAI_I2S_IN1;
	else if (strncmp(name, "I2SIN2", 6) == 0)
		return MT6878_DAI_I2S_IN2;
	else if (strncmp(name, "I2SIN4", 6) == 0)
		return MT6878_DAI_I2S_IN4;
	else if (strncmp(name, "I2SOUT1", 7) == 0)
		return MT6878_DAI_I2S_OUT1;
	else if (strncmp(name, "I2SOUT2", 7) == 0)
		return MT6878_DAI_I2S_OUT2;
	else if (strncmp(name, "I2SOUT4", 7) == 0)
		return MT6878_DAI_I2S_OUT4;
	else
		return -EINVAL;
}
...
static int mtk_is_i2s_low_power(int i2s_num)
{
	...
	i2s_bit_shift = i2s_num - MT6878_DAI_I2S_IN0;
	if (i2s_bit_shift < 0 || i2s_bit_shift > MT6878_DAI_I2S_MAX_NUM) {
...
/* low jitter control */
static const char *const mt6878_i2s_hd_str[] = {
	"Normal", "Low_Jitter"
};

static const struct soc_enum mt6878_i2s_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6878_i2s_hd_str),
			    mt6878_i2s_hd_str),
};

static int mt6878_i2s_hd_get(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6878_i2s_hd_set(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_dai_i2s_controls[] = {
	SOC_ENUM_EXT(MTK_AFE_I2SIN1_KCONTROL_NAME, mt6878_i2s_enum[0],
		     mt6878_i2s_hd_get, mt6878_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SIN2_KCONTROL_NAME, mt6878_i2s_enum[0],
		     mt6878_i2s_hd_get, mt6878_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SIN4_KCONTROL_NAME, mt6878_i2s_enum[0],
		     mt6878_i2s_hd_get, mt6878_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SOUT1_KCONTROL_NAME, mt6878_i2s_enum[0],
		     mt6878_i2s_hd_get, mt6878_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SOUT2_KCONTROL_NAME, mt6878_i2s_enum[0],
		     mt6878_i2s_hd_get, mt6878_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SOUT4_KCONTROL_NAME, mt6878_i2s_enum[0],
		     mt6878_i2s_hd_get, mt6878_i2s_hd_set),
			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6878_afe_gpio_request(afe, true, i2s_priv->id, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6878_afe_gpio_request(afe, false, i2s_priv->id, 0);
		break;
	default:
		break;
	}
			  ...
			  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (strcmp(w->name, APLL1_W_NAME) == 0)
			mt6878_apll1_enable(afe);
		else
			mt6878_apll2_enable(afe);
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (strcmp(w->name, APLL1_W_NAME) == 0)
			mt6878_apll1_disable(afe);
		else
			mt6878_apll2_disable(afe);
		break;
	default:
		break;
	}
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6878_mck_enable(afe, i2s_priv->mclk_id, i2s_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		i2s_priv->mclk_rate = 0;
		mt6878_mck_disable(afe, i2s_priv->mclk_id);
		break;
	default:
		break;
	}
				    ...
				    struct snd_soc_dapm_widget *sink)
{
	...
	/* which apll */
	cur_apll = mt6878_get_apll_by_name(afe, source->name);

	/* choose APLL from i2s rate */
	i2s_need_apll = mt6878_get_apll_by_rate(afe, i2s_priv->rate);
		...
		struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6878_rate_transform(afe->dev,
				rate, dai->id);
				  ...
				  int clk_id, unsigned int freq, int dir)
{
	...
	apll = mt6878_get_apll_by_rate(afe, freq);
	apll_rate = mt6878_get_apll_rate(afe, apll);
...
static const struct mtk_afe_i2s_priv mt6878_i2s_priv[DAI_I2S_NUM] = {
	[DAI_I2SIN1] = {
		.id = MT6878_DAI_I2S_IN1,
	...
	[DAI_I2SOUT1] = {
		...
		.share_i2s_id = MT6878_DAI_I2S_IN1,
	},
	[DAI_I2SOUT2] = {
		.id = MT6878_DAI_I2S_OUT2,
		.mclk_id = MT6878_I2SIN1_MCK,
...
int mt6878_dai_i2s_get_share(struct mtk_base_afe *afe)
{
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	...
	for (i = 0; i < DAI_I2S_NUM; i++) {
		i2s_priv = afe_priv->dai_priv[mt6878_i2s_priv[i].id];
		property_name = mt6878_i2s_priv[i].share_property_name;
...
int mt6878_dai_i2s_set_priv(struct mtk_base_afe *afe)
{
	int i;
	int ret;

	for (i = 0; i < DAI_I2S_NUM; i++) {
		ret = mt6878_dai_set_priv(afe, mt6878_i2s_priv[i].id,
...
int mt6878_dai_i2s_register(struct mtk_base_afe *afe)
{
	...
	ret = mt6878_dai_i2s_set_priv(afe);
	if (ret)
		return ret;

	/* parse share i2s */
	ret = mt6878_dai_i2s_get_share(afe);
	if (ret)
		return ret;
