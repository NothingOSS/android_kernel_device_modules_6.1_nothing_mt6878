...
/*
 *  Mediatek ALSA SoC AFE platform driver for 6878
 *
...
static ssize_t mt6878_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe);

static const struct snd_pcm_hardware mt6878_afe_hardware = {
...
static int mt6878_fe_startup(struct snd_pcm_substream *substream,
...
void mt6878_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6878_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8")) {
				if (mt6878_is_need_enable_cm(afe, CM0))
					mt6878_enable_cm(afe, CM0, 1);
			}
			if (!strcmp(memif->data->name, "VUL9")) {
				if (mt6878_is_need_enable_cm(afe, CM1))
					mt6878_enable_cm(afe, CM1, 1);
			}
		}
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8"))
				mt6878_enable_cm(afe, CM0, 0);
			if (!strcmp(memif->data->name, "VUL9"))
				mt6878_enable_cm(afe, CM1, 0);
		}
...
static int mt6878_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	rate_reg = mt6878_rate_transform(afe->dev, rate, id);

	switch (id) {
	case MT6878_MEMIF_VUL8:
	case MT6878_MEMIF_VUL_CM0:
		cm = 0x0;
		break;
	case MT6878_MEMIF_VUL9:
	case MT6878_MEMIF_VUL_CM1:
		cm = 0x1;
		break;
	default:
		cm = 0x0;
	...
	mt6878_set_cm_rate(cm, rate_reg);

	return rate_reg;
}

static int mt6878_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6878_rate_transform(afe->dev, rate, dai_id);
}

static int mt6878_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	if (!component)
		return -EINVAL;
	afe = snd_soc_component_get_drvdata(component);
	return mt6878_general_rate_transform(afe->dev, rate);
}

int mt6878_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6878_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6878_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6878_memif_dai_ops = {
	.startup        = mt6878_fe_startup,
	.shutdown       = mt6878_fe_shutdown,
	.hw_params      = mtk_afe_fe_hw_params,
	.hw_free        = mtk_afe_fe_hw_free,
	.prepare        = mtk_afe_fe_prepare,
	.trigger        = mt6878_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6878_memif_dai_driver[] = {
...
/* kcontrol */
static int mt6878_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6878_PRIMARY_MEMIF];
	return 0;
}

static int mt6878_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6878_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6878_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6878_RECORD_MEMIF];
	return 0;
}

static int mt6878_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6878_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6878_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6878_DEEP_MEMIF];
	return 0;
}

static int mt6878_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6878_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6878_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6878_VOIP_MEMIF];
	return 0;
}

static int mt6878_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6878_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6878_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6878_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6878_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6878_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6878_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6878_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6878_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6878_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6878_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6878_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

static int mt6878_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_vow_barge_in_irq_id_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6878_BARGE_IN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6878_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_mmap_dl_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6878_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6878_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6878_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6878_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6878_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6878_mmap_ion_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6878_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6878_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static const struct snd_kcontrol_new mt6878_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6878_adsp_ref_mem_get,
		       mt6878_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6878_mmap_dl_scene_get, mt6878_mmap_dl_scene_set),
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6878_mmap_ul_scene_get, mt6878_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6878_dl_mmap_fd_get,
		       mt6878_dl_mmap_fd_set),
	SOC_SINGLE_EXT("aaudio_ul_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6878_ul_mmap_fd_get,
		       mt6878_ul_mmap_fd_set),
};
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		mt6878_enable_cm_bypass(afe, CM0, 0x0);
		mt6878_set_cm(afe, CM0, 0x7, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt6878_enable_cm_bypass(afe, CM0, 0x1);
		break;
	default:
		break;
	}
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		channels = mtk_get_channel_value();
		mt6878_enable_cm_bypass(afe, CM1, 0x0);
		mt6878_set_cm(afe, CM1, 0x7, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt6878_enable_cm_bypass(afe, CM1, 0x1);
		break;
	default:
		break;
	}
...
static const struct snd_soc_dapm_widget mt6878_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6878_memif_routes[] = {
	{"UL0", NULL, "UL0_CH1"},
	{"UL0", NULL, "UL0_CH2"},
	/* Normal record */
...
static const struct mtk_base_memif_data memif_data[MT6878_MEMIF_NUM] = {
	[MT6878_MEMIF_DL0] = {
		.name = "DL0",
		.id = MT6878_MEMIF_DL0,
	...
	[MT6878_MEMIF_DL_24CH] = {
		...
		.hd_shift = DL_24CH_HD_MODE_MASK_SFT,
...
static const struct mtk_base_irq_data irq_data[MT6878_IRQ_NUM] = {
	[MT6878_IRQ_0] = {
		.id = MT6878_IRQ_0,
...
static const int memif_irq_usage[MT6878_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6878_MEMIF_DL0] = MT6878_IRQ_0,
	[MT6878_MEMIF_DL1] = MT6878_IRQ_1,
	[MT6878_MEMIF_DL2] = MT6878_IRQ_2,
	[MT6878_MEMIF_DL3] = MT6878_IRQ_3,
	[MT6878_MEMIF_DL4] = MT6878_IRQ_4,
	[MT6878_MEMIF_DL5] = MT6878_IRQ_5,
	[MT6878_MEMIF_DL6] = MT6878_IRQ_6,
	[MT6878_MEMIF_DL7] = MT6878_IRQ_7,
	[MT6878_MEMIF_DL8] = MT6878_IRQ_8,
	[MT6878_MEMIF_DL23] = MT6878_IRQ_9,
	[MT6878_MEMIF_DL24] = MT6878_IRQ_10,
	[MT6878_MEMIF_DL25] = MT6878_IRQ_11,
	[MT6878_MEMIF_DL_24CH] = MT6878_IRQ_12,
	[MT6878_MEMIF_VUL0] = MT6878_IRQ_13,
	[MT6878_MEMIF_VUL1] = MT6878_IRQ_14,
	[MT6878_MEMIF_VUL2] = MT6878_IRQ_15,
	[MT6878_MEMIF_VUL3] = MT6878_IRQ_16,
	[MT6878_MEMIF_VUL4] = MT6878_IRQ_17,
	[MT6878_MEMIF_VUL5] = MT6878_IRQ_18,
	[MT6878_MEMIF_VUL6] = MT6878_IRQ_19,
	[MT6878_MEMIF_VUL7] = MT6878_IRQ_20,
	[MT6878_MEMIF_VUL8] = MT6878_IRQ_21,
	[MT6878_MEMIF_VUL9] = MT6878_IRQ_22,
	[MT6878_MEMIF_VUL10] = MT6878_IRQ_23,
	[MT6878_MEMIF_VUL24] = MT6878_IRQ_24,
	[MT6878_MEMIF_VUL25] = MT6878_IRQ_25,
	[MT6878_MEMIF_VUL_CM0] = MT6878_IRQ_26,
	[MT6878_MEMIF_VUL_CM1] = MT6878_IRQ_0,
	[MT6878_MEMIF_ETDM_IN1] = MT6878_IRQ_0,
	[MT6878_MEMIF_ETDM_IN2] = MT6878_IRQ_0,
	[MT6878_MEMIF_ETDM_IN4] = MT6878_IRQ_0,
};

static bool mt6878_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6878_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6878_is_volatile_reg,
...
static irqreturn_t mt6878_afe_irq_handler(int irq_id, void *dev)
{
	...
	if (t2 > timeout_limit) {
		dev_info(afe->dev, "%s(), mcu_en 0x%x, timeout %llu, limit %llu, ret %d\n",
			__func__, mcu_en,
...
static int mt6878_afe_runtime_suspend(struct device *dev)
{
...
skip_regmap:
	mt6878_afe_sram_release(afe);
	mt6878_afe_disable_clock(afe);
	return 0;
}

static int mt6878_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	int ret;
	...
	ret = mt6878_afe_enable_clock(afe);
	if (ret)
		return ret;
	mt6878_afe_sram_request(afe);
...
static int mt6878_afe_pcm_copy(struct snd_pcm_substream *substream,
			       ...
			       mtk_sp_copy_f sp_copy)
{
	...
	if (!component)
		return -EINVAL;
	afe = snd_soc_component_get_drvdata(component);
	mt6878_set_audio_int_bus_parent(afe, CLK_TOP_MAINPLL_D4_D4);
...
static int mt6878_set_memif_sram_mode(struct device *dev,
...
static int mt6878_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6878_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6878_sram_ops = {
	.set_sram_mode = mt6878_set_sram_mode,
};
				    ...
				    char *buf, loff_t offset, size_t size)
{
	...
	read_size = mt6878_debug_read_reg(buffer, AFE_SYS_DEBUG_SIZE, afe);
...
static int mt6878_afe_component_probe(struct snd_soc_component *component)
{
	struct mtk_base_afe *afe = NULL;
	...
	if (component) {
		...
		mt6878_add_misc_control(component);
...
static const struct snd_soc_component_driver mt6878_afe_component = {
	.name = AFE_PCM_NAME,
	.probe = mt6878_afe_component_probe,
...
static ssize_t mt6878_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe)
{
	...
	regmap_read(afe->regmap, AFE_ADDA_ULSRC_PHASE_CON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x]	 AFE_ADDA_ULSRC_PHASE_CON0 = 0x%x\n",
		       AFE_ADDA_ULSRC_PHASE_CON0, value);
	regmap_read(afe->regmap, AFE_ADDA_ULSRC_PHASE_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x]	 AFE_ADDA_ULSRC_PHASE_CON1 = 0x%x\n",
		       AFE_ADDA_ULSRC_PHASE_CON1, value);
	regmap_read(afe->regmap, AFE_ADDA_ULSRC_PHASE_CON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x]	 AFE_ADDA_ULSRC_PHASE_CON2 = 0x%x\n",
		       AFE_ADDA_ULSRC_PHASE_CON2, value);
	regmap_read(afe->regmap, AFE_ADDA_ULSRC_PHASE_CON3, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x]	 AFE_ADDA_ULSRC_PHASE_CON3 = 0x%x\n",
		       AFE_ADDA_ULSRC_PHASE_CON3, value);
...
static ssize_t mt6878_debugfs_read(struct file *file, char __user *buf,
				   size_t count, loff_t *pos)
{
	...
	n = mt6878_debug_read_reg(buffer, size, afe);
...
static const struct mtk_afe_debug_cmd mt6878_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6878_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6878_debugfs_read,
};
#endif

static int mt6878_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6878_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6878_memif_dai_driver);

	dai->controls = mt6878_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6878_pcm_kcontrols);
	dai->dapm_widgets = mt6878_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6878_memif_widgets);
	dai->dapm_routes = mt6878_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6878_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6878_dai_adda_register,
	mt6878_dai_i2s_register,
	mt6878_dai_hw_gain_register,
	mt6878_dai_src_register,
	mt6878_dai_pcm_register,
	mt6878_dai_hostless_register,
	mt6878_dai_memif_register,
};

static int mt6878_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
	unsigned int tmp_reg = 0;
#if !defined(CONFIG_FPGA_EARLY_PORTING) || defined(FORCE_FPGA_ENABLE_IRQ)
	...
	struct mt6878_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6878_sram_ops);
	if (ret)
		return ret;
	...
	afe->memif_size = MT6878_MEMIF_NUM;
	...
	afe->memif[MT6878_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;
	afe->memif[MT6878_FAST_MEMIF].fast_palyback = 1;

	mutex_init(&afe->irq_alloc_lock);       /* needed when dynamic irq */

	/* init irq */
	afe->irqs_size = MT6878_IRQ_NUM;
	...
	ret = devm_request_irq(dev, irq_id, mt6878_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6878_afe_hardware;
	afe->memif_fs = mt6878_memif_fs;
	afe->irq_fs = mt6878_irq_fs;
	afe->get_dai_fs = mt6878_get_dai_fs;
	afe->get_memif_pbuf_size = mt6878_get_memif_pbuf_size;

	afe->runtime_resume = mt6878_afe_runtime_resume;
	afe->runtime_suspend = mt6878_afe_runtime_suspend;

	afe->request_dram_resource = mt6878_afe_dram_request;
	afe->release_dram_resource = mt6878_afe_dram_release;

	afe->copy = mt6878_afe_pcm_copy;
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* debugfs */
	afe->debug_cmds = mt6878_debug_cmds;
	afe->debugfs = debugfs_create_file("mtksocaudio",
					   S_IFREG | 0444, NULL,
					   afe, &mt6878_debugfs_ops);
#endif
#if !defined(SKIP_SB_USB_OFFLOAD)
...
static int mt6878_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6878_afe_runtime_suspend(&pdev->dev);
#if !defined(SKIP_SB_USB_OFFLOAD)
...
static const struct of_device_id mt6878_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6878-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6878_afe_pcm_dt_match);

static const struct dev_pm_ops mt6878_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6878_afe_runtime_suspend,
			   mt6878_afe_runtime_resume, NULL)
};

static struct platform_driver mt6878_afe_pcm_driver = {
	.driver = {
		.name = "mt6878-audio",
		.of_match_table = mt6878_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		.pm = &mt6878_afe_pm_ops,
#endif
	},
	.probe = mt6878_afe_pcm_dev_probe,
	.remove = mt6878_afe_pcm_dev_remove,
};

module_platform_driver(mt6878_afe_pcm_driver);

MODULE_DESCRIPTION("Mediatek ALSA SoC AFE platform driver for 6878");
