...
/*
 *  mt6985-mt6338.c  --  mt6985 mt6338 ALSA SoC machine driver
 *
...
#include "../../codecs/mt6338.h"
#if IS_ENABLED(CONFIG_SND_SOC_MT6338_ACCDET)
#include "../../codecs/mt6338-accdet.h"
#endif
#include "../common/mtk-sp-spk-amp.h"
/*
 * if need additional control for the ext spk amp that is connected
 * after Lineout Buffer / HP Buffer on the codec, put the control in
 * mt6985_mt6338_spk_amp_event()
 */
#define EXT_SPK_AMP_W_NAME "Ext_Speaker_Amp"

static struct snd_soc_card mt6985_mt6338_soc_card;

struct mt6985_compress_info compr_info;

static const char *const mt6985_spk_type_str[] = {MTK_SPK_NOT_SMARTPA_STR,
...
static const struct soc_enum mt6985_spk_type_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6985_spk_type_str),
			    mt6985_spk_type_str),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6985_spk_i2s_type_str),
			    mt6985_spk_i2s_type_str),
};

static int mt6985_spk_type_get(struct snd_kcontrol *kcontrol,
...
static int mt6985_spk_i2s_out_type_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6985_spk_i2s_in_type_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6985_compress_info_set(struct snd_kcontrol *kcontrol,
				    const unsigned int __user *data,
				    unsigned int size)
{
	if (copy_from_user(&compr_info,
			   data,
			   sizeof(struct mt6985_compress_info))) {
...
static int mt6985_compress_info_get(struct snd_kcontrol *kcontrol,
				    unsigned int __user *data, unsigned int size)
{
	struct snd_soc_card *card = &mt6985_mt6338_soc_card;
	...
	if (copy_to_user(data, &compr_info, sizeof(struct mt6985_compress_info))) {
...
static int mt6985_mt6338_spk_amp_event(struct snd_soc_dapm_widget *w,
...
static const struct snd_soc_dapm_widget mt6985_mt6338_widgets[] = {
	SND_SOC_DAPM_SPK(EXT_SPK_AMP_W_NAME, mt6985_mt6338_spk_amp_event),
};

static const struct snd_soc_dapm_route mt6985_mt6338_routes[] = {
...
static const struct snd_kcontrol_new mt6985_mt6338_controls[] = {
	SOC_DAPM_PIN_SWITCH(EXT_SPK_AMP_W_NAME),
	SOC_ENUM_EXT("MTK_SPK_TYPE_GET", mt6985_spk_type_enum[0],
		     mt6985_spk_type_get, NULL),
	SOC_ENUM_EXT("MTK_SPK_I2S_OUT_TYPE_GET", mt6985_spk_type_enum[1],
		     mt6985_spk_i2s_out_type_get, NULL),
	SOC_ENUM_EXT("MTK_SPK_I2S_IN_TYPE_GET", mt6985_spk_type_enum[1],
		     mt6985_spk_i2s_in_type_get, NULL),
	SND_SOC_BYTES_TLV("MTK_COMPRESS_INFO",
			  sizeof(struct mt6985_compress_info),
			  mt6985_compress_info_get, mt6985_compress_info_set),
};

/*
 * define mtk_spk_i2s_mck node in dts when need mclk,
 * BE i2s need assign snd_soc_ops = mt6985_mt6338_i2s_ops
 */
static int mt6985_mt6338_i2s_hw_params(struct snd_pcm_substream *substream,
...
static const struct snd_soc_ops mt6985_mt6338_i2s_ops = {
	.hw_params = mt6985_mt6338_i2s_hw_params,
};

static int mt6985_mt6338_mtkaif_calibration(struct snd_soc_pcm_runtime *rtd)
{
	...
	miso0_need_calib = mt6985_afe_gpio_is_prepared(MT6985_AFE_GPIO_DAT_MISO0_ON);
	miso1_need_calib = mt6985_afe_gpio_is_prepared(MT6985_AFE_GPIO_DAT_MISO1_ON);
	miso2_need_calib = mt6985_afe_gpio_is_prepared(MT6985_AFE_GPIO_DAT_MISO2_ON);
...
static int mt6985_mt6338_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
	...
	ops.enable_dc_compensation = mt6985_enable_dc_compensation;
	ops.set_lch_dc_compensation = mt6985_set_lch_dc_compensation;
	ops.set_rch_dc_compensation = mt6985_set_rch_dc_compensation;
	ops.adda_dl_gain_control = mt6985_adda_dl_gain_control;
	mt6338_set_codec_ops(codec_component, &ops);

	/* set mtkaif protocol */
	mt6338_set_mtkaif_protocol(codec_component,
				   MTKAIF_PROTOCOL_2_CLK_P2);
	afe_priv->mtkaif_protocol = MTKAIF_PROTOCOL_2_CLK_P2;

	/* mtkaif calibration */
	mt6985_mt6338_mtkaif_calibration(rtd);
...
static int mt6985_i2s_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
				      struct snd_pcm_hw_params *params)
{
...
static int mt6985_mt6338_vow_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
			snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
...
static void mt6985_mt6338_vow_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
			snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
...
static const struct snd_soc_ops mt6985_mt6338_vow_ops = {
	.startup = mt6985_mt6338_vow_startup,
	.shutdown = mt6985_mt6338_vow_shutdown,
};
...
SND_SOC_DAILINK_DEFS(capture11,
	...
	DAILINK_COMP_ARRAY(COMP_DUMMY()),
	DAILINK_COMP_ARRAY(COMP_EMPTY()));
SND_SOC_DAILINK_DEFS(capture12,
	DAILINK_COMP_ARRAY(COMP_CPU("UL12")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()),
	DAILINK_COMP_ARRAY(COMP_EMPTY()));
SND_SOC_DAILINK_DEFS(capture13,
	DAILINK_COMP_ARRAY(COMP_CPU("UL13")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()),
	DAILINK_COMP_ARRAY(COMP_EMPTY()));
...
SND_SOC_DAILINK_DEFS(hostless_src_aaudio,
	...
	DAILINK_COMP_ARRAY(COMP_DUMMY()),
	DAILINK_COMP_ARRAY(COMP_EMPTY()));
#if IS_ENABLED(CONFIG_DEVICE_MODULES_SND_SOC_MTK_BTCVSD) && !defined(SKIP_SB)
...
static struct snd_soc_dai_link mt6985_mt6338_dai_links[] = {
	...
	},
	{
		...
		.ops = &mt6985_mt6338_i2s_ops,
		.no_pcm = 1,
		.dpcm_capture = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = mt6985_i2s_hw_params_fixup,
...
static struct snd_soc_card mt6985_mt6338_soc_card = {
	.name = "mt6985-mt6338",
	.owner = THIS_MODULE,
	.dai_link = mt6985_mt6338_dai_links,
	.num_links = ARRAY_SIZE(mt6985_mt6338_dai_links),

	.controls = mt6985_mt6338_controls,
	.num_controls = ARRAY_SIZE(mt6985_mt6338_controls),
	.dapm_widgets = mt6985_mt6338_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt6985_mt6338_widgets),
	.dapm_routes = mt6985_mt6338_routes,
	.num_dapm_routes = ARRAY_SIZE(mt6985_mt6338_routes),
};

static int mt6985_mt6338_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt6985_mt6338_soc_card;
...
static const struct of_device_id mt6985_mt6338_dt_match[] = {
	{.compatible = "mediatek,mt6985-mt6338-sound",},
	{}
};
#endif

static const struct dev_pm_ops mt6985_mt6338_pm_ops = {
	.poweroff = snd_soc_poweroff,
	.restore = snd_soc_resume,
};

static struct platform_driver mt6985_mt6338_driver = {
	.driver = {
		.name = "mt6985-mt6338",
#if IS_ENABLED(CONFIG_OF)
		.of_match_table = mt6985_mt6338_dt_match,
#endif
		.pm = &mt6985_mt6338_pm_ops,
	},
	.probe = mt6985_mt6338_dev_probe,
};

module_platform_driver(mt6985_mt6338_driver);

/* Module information */
MODULE_DESCRIPTION("MT6985 mt6338 ALSA SoC machine driver");
MODULE_AUTHOR("Cheng-yi Cheng <Cheng-yi.cheng@mediatek.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("mt6985 mt6338 soc card");
