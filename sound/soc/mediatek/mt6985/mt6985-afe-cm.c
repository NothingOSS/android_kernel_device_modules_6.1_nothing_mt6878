...
static int mt6985_convert_cm_rate(unsigned int rate)
{
	switch (rate) {
...
static int mt6985_convert_cm_ch(unsigned int ch)
{
	return ch - 1;
}

int mt6985_set_cm(struct mtk_base_afe *afe, int id, unsigned int rate,
	       unsigned int update, bool swap, unsigned int ch)
{
	...
	/* rate */
	rate = mt6985_convert_cm_rate(rate);
	mtk_regmap_update_bits(afe->regmap, cm_reg,
			       cm_rate_mask, rate, cm_rate_shift);

	/* ch num */
	ch = mt6985_convert_cm_ch(ch);
	mtk_regmap_update_bits(afe->regmap, cm_reg,
...
EXPORT_SYMBOL_GPL(mt6985_enable_cm_bypass);
...
EXPORT_SYMBOL_GPL(mt6985_enable_cm);
...
EXPORT_SYMBOL_GPL(mt6985_is_need_enable_cm);
