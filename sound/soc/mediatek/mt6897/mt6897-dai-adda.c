...
static int mtk_adda_ul_src_dmic(struct mtk_base_afe *afe, int id)
{
	...
	switch (id) {
	case MT6897_DAI_ADDA:
	case MT6897_DAI_AP_DMIC:
		reg = AFE_ADDA_UL0_SRC_CON0;
		break;
	case MT6897_DAI_ADDA_CH34:
	case MT6897_DAI_AP_DMIC_CH34:
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_MTKAIF0_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6897_DAI_ADDA);
		}
		break;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6897_DAI_ADDA_CH34);
		}
...
static bool is_adda_mtkaif_need_phase_delay(struct mt6897_afe_private *afe_priv)
{
	if (mt6897_afe_gpio_is_prepared(MT6897_AFE_GPIO_DAT_MISO0_ON) &&
	    afe_priv->mtkaif_chosen_phase[0] < 0) {
		AUDIO_AEE("adda mtkaif miso0 calib fail");
		return false;
	}

	if (mt6897_afe_gpio_is_prepared(MT6897_AFE_GPIO_DAT_MISO1_ON) &&
...
/* mtkaif dmic */
static const char *const mt6897_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6897_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6897_adda_off_on_str),
			    mt6897_adda_off_on_str),
};

static int mt6897_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adda6_only_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adda6_only_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adda_dl_max_vol_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adda_dl_max_vol_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_vow_enable_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_vow_enable_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_SINGLE("ADDA_DL_GAIN", AFE_ADDA_DL_SRC_CON1,
		   AFE_DL_GAIN_CTL_PRE_SFT, AFE_DL_GAIN_CTL_PRE_MASK, 0),
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6897_adda_enum[0],
		     mt6897_adda_dmic_get, mt6897_adda_dmic_set),
	SOC_ENUM_EXT("MTKAIF_ADDA6_ONLY", mt6897_adda_enum[0],
		     mt6897_adda6_only_get, mt6897_adda6_only_set),
	SOC_SINGLE_EXT("ADDA_DL_MAX_VOL",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6897_adda_dl_max_vol_get,
		       mt6897_adda_dl_max_vol_set),
	SOC_SINGLE_EXT("VOW_ENABLE",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6897_vow_enable_get,
		       mt6897_vow_enable_set),
};
...
static const struct snd_soc_dapm_widget mtk_dai_adda_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY_S("ADDA CH34 Playback Enable",
			      ...
			      AFE_2ND_DL_SRC_ON_TMP_CTL_PRE_SFT, 0,
				  ...
				  struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (id == MT6897_DAI_ADDA) {
			...
			/* clean predistortion */
			regmap_write(afe->regmap,
				     AFE_ADDA_2ND_DL_PREDIS_CON0, 0);
			regmap_write(afe->regmap,
				     AFE_ADDA_2ND_DL_PREDIS_CON1, 0);

			regmap_write(afe->regmap, AFE_ADDA_2ND_DL_SRC_CON0,
				     dl_src_con0);
			regmap_write(afe->regmap, AFE_ADDA_2ND_DL_SRC_CON1,
				     dl_src_con1);

			/* set sdm gain */
			regmap_update_bits(afe->regmap,
					   AFE_ADDA_2ND_DL_SDM_DCCOMP_CON,
					   AFE_2ND_DL_ATTGAIN_CTL_MASK_SFT,
					   AUDIO_SDM_LEVEL_NORMAL <<
					   AFE_2ND_DL_ATTGAIN_CTL_SFT);

			/* 2nd sdm */
			regmap_update_bits(afe->regmap,
					   AFE_ADDA_2ND_DL_SDM_DCCOMP_CON,
					   AFE_2ND_DL_USE_3RD_SDM_MASK_SFT,
					   AUDIO_SDM_2ND << AFE_2ND_DL_USE_3RD_SDM_SFT);
			...
			regmap_update_bits(
				afe->regmap,
				AFE_ADDA_2ND_DL_SDM_AUTO_RESET_CON,
				AFE_2ND_DL_SDM_AUTO_RESET_TEST_ON_SFT,
				0x1 << AFE_2ND_DL_SDM_AUTO_RESET_TEST_ON_SFT);
		}
	} else {
...
int mt6897_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6897_dai_set_priv(afe, MT6897_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6897_dai_set_priv(afe, MT6897_DAI_ADDA_CH34,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	/* ap dmic priv share with adda */
	afe_priv->dai_priv[MT6897_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6897_DAI_ADDA];
	afe_priv->dai_priv[MT6897_DAI_AP_DMIC_CH34] =
		afe_priv->dai_priv[MT6897_DAI_ADDA_CH34];
