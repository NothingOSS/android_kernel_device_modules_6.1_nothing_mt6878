...
/*
 *  Mediatek ALSA SoC AFE platform driver for 6897
 *
...
#include "mt6897-afe-cm.h"
#include "mt6897-afe-common.h"
#include "mt6897-afe-clk.h"
#include "mt6897-afe-gpio.h"
#include "mt6897-interconnection.h"
...
static ssize_t mt6897_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe);

static const struct snd_pcm_hardware mt6897_afe_hardware = {
...
static int mt6897_fe_startup(struct snd_pcm_substream *substream,
...
void mt6897_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6897_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8")) {
				if (mt6897_is_need_enable_cm(afe, CM0))
					mt6897_enable_cm(afe, CM0, 1);
			}
			if (!strcmp(memif->data->name, "VUL9")) {
				if (mt6897_is_need_enable_cm(afe, CM1))
					mt6897_enable_cm(afe, CM1, 1);
			}
		}
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8"))
				mt6897_enable_cm(afe, CM0, 0);
			if (!strcmp(memif->data->name, "VUL9"))
				mt6897_enable_cm(afe, CM1, 0);
		}
...
static int mt6897_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	struct snd_soc_dai *cpu_dai = asoc_rtd_to_cpu(rtd, 0);
	int id = cpu_dai->id;
	unsigned int rate_reg = mt6897_rate_transform(afe->dev, rate, id);
	int cm = 0;

	switch (id) {
	case MT6897_MEMIF_VUL8:
	case MT6897_MEMIF_VUL_CM0:
		cm = 0x0;
		break;
	case MT6897_MEMIF_VUL9:
	case MT6897_MEMIF_VUL_CM1:
		cm = 0x1;
		break;
	default:
		cm = 0x0;
	...
	mt6897_set_cm_rate(cm, rate_reg);

	return rate_reg;
}

static int mt6897_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6897_rate_transform(afe->dev, rate, dai_id);
}

static int mt6897_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	return mt6897_general_rate_transform(afe->dev, rate);
}

int mt6897_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6897_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6897_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6897_memif_dai_ops = {
	.startup        = mt6897_fe_startup,
	.shutdown       = mt6897_fe_shutdown,
	.hw_params      = mtk_afe_fe_hw_params,
	.hw_free        = mtk_afe_fe_hw_free,
	.prepare        = mtk_afe_fe_prepare,
	.trigger        = mt6897_fe_trigger,
};
...
/* kcontrol */
static int mt6897_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6897_PRIMARY_MEMIF];
	return 0;
}

static int mt6897_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6897_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6897_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6897_RECORD_MEMIF];
	return 0;
}

static int mt6897_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6897_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6897_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6897_DEEP_MEMIF];
	return 0;
}

static int mt6897_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6897_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6897_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6897_VOIP_MEMIF];
	return 0;
}

static int mt6897_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6897_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6897_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6897_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6897_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6897_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6897_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6897_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6897_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6897_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6897_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6897_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

static int mt6897_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6897_BARGE_IN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6897_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_mmap_dl_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6897_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6897_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6897_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6897_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6897_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6897_mmap_ion_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6897_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6897_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static const struct snd_kcontrol_new mt6897_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6897_adsp_ref_mem_get,
		       mt6897_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6897_mmap_dl_scene_get, mt6897_mmap_dl_scene_set),
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6897_mmap_ul_scene_get, mt6897_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6897_dl_mmap_fd_get,
		       mt6897_dl_mmap_fd_set),
	SOC_SINGLE_EXT("aaudio_ul_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6897_ul_mmap_fd_get,
		       mt6897_ul_mmap_fd_set),
};
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		mt6897_enable_cm_bypass(afe, CM0, 0x0);
		mt6897_set_cm(afe, CM0, 0x7, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt6897_enable_cm_bypass(afe, CM0, 0x1);
		break;
	default:
		break;
	}
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		channels = mtk_get_channel_value();
		mt6897_enable_cm_bypass(afe, CM1, 0x0);
		mt6897_set_cm(afe, CM1, 0x7, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt6897_enable_cm_bypass(afe, CM1, 0x1);
		break;
	default:
		break;
	}
...
static const struct snd_kcontrol_new memif_ul4_ch1_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("DL_24CH_CH1", AFE_CONN026_1,
				    I_DL_24CH_CH1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("DL24_CH1", AFE_CONN026_2,
				    I_DL24_CH1, 1, 0),
...
static const struct snd_kcontrol_new memif_ul4_ch2_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("DL_24CH_CH2", AFE_CONN027_1,
				    I_DL_24CH_CH2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("DL24_CH2", AFE_CONN027_2,
				    I_DL24_CH2, 1, 0),
...
static const struct snd_soc_dapm_widget mt6897_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6897_memif_routes[] = {
	{"UL0", NULL, "UL0_CH1"},
	{"UL0", NULL, "UL0_CH2"},
	/* Normal record */
	...
	{"UL4_CH1", "DL24_CH1", "Hostless_UL4 UL"},
	{"UL4_CH2", "DL24_CH2", "Hostless_UL4 UL"},
...
static const struct mtk_base_irq_data irq_data[MT6897_IRQ_NUM] = {
	[MT6897_IRQ_0] = {
		.id = MT6897_IRQ_0,
	...
	[MT6897_IRQ_31] = {
		...
		.irq_scp_en_shift = CUS_IRQ0_MCU_SCP_EN_SFT,
	},
};
...
static const int memif_irq_usage[MT6897_MEMIF_NUM] = {
	...
	[MT6897_MEMIF_DL1] = MT6897_IRQ_1,
	[MT6897_MEMIF_DL2] = MT6897_IRQ_2,
	[MT6897_MEMIF_DL3] = MT6897_IRQ_3,
	[MT6897_MEMIF_DL4] = MT6897_IRQ_4,
	[MT6897_MEMIF_DL5] = MT6897_IRQ_5,
	[MT6897_MEMIF_DL6] = MT6897_IRQ_6,
	[MT6897_MEMIF_DL7] = MT6897_IRQ_7,
	[MT6897_MEMIF_DL8] = MT6897_IRQ_8,
	[MT6897_MEMIF_DL23] = MT6897_IRQ_9,
	[MT6897_MEMIF_DL24] = MT6897_IRQ_10,
	[MT6897_MEMIF_DL25] = MT6897_IRQ_11,
	[MT6897_MEMIF_DL_24CH] = MT6897_IRQ_12,
	[MT6897_MEMIF_VUL0] = MT6897_IRQ_13,
	[MT6897_MEMIF_VUL1] = MT6897_IRQ_14,
	[MT6897_MEMIF_VUL2] = MT6897_IRQ_15,
	[MT6897_MEMIF_VUL3] = MT6897_IRQ_16,
	[MT6897_MEMIF_VUL4] = MT6897_IRQ_17,
	[MT6897_MEMIF_VUL5] = MT6897_IRQ_18,
	[MT6897_MEMIF_VUL6] = MT6897_IRQ_19,
	[MT6897_MEMIF_VUL7] = MT6897_IRQ_20,
	[MT6897_MEMIF_VUL8] = MT6897_IRQ_21,
	[MT6897_MEMIF_VUL9] = MT6897_IRQ_22,
	[MT6897_MEMIF_VUL10] = MT6897_IRQ_23,
	[MT6897_MEMIF_VUL24] = MT6897_IRQ_24,
	[MT6897_MEMIF_VUL25] = MT6897_IRQ_25,
	[MT6897_MEMIF_VUL_CM0] = MT6897_IRQ_26,
	[MT6897_MEMIF_VUL_CM1] = MT6897_IRQ_0,
	[MT6897_MEMIF_ETDM_IN0] = MT6897_IRQ_0,
	[MT6897_MEMIF_ETDM_IN1] = MT6897_IRQ_0,
	[MT6897_MEMIF_ETDM_IN2] = MT6897_IRQ_0,
	[MT6897_MEMIF_ETDM_IN4] = MT6897_IRQ_0,
	[MT6897_MEMIF_ETDM_IN6] = MT6897_IRQ_0,
	[MT6897_MEMIF_HDMI] = MT6897_IRQ_31,
};

static bool mt6897_is_volatile_reg(struct device *dev, unsigned int reg)
{
	...
	case AFE_ADDA_DL_IPM_VER_MON:
	case AFE_ADDA_DL_SRC_DEBUG_MON0:
	...
	case AFE_ADDA_2ND_DL_SRC_DEBUG_MON0:
	case AFE_ADDA_2ND_DL_SRC_LCH_MON:
	case AFE_ADDA_2ND_DL_SRC_RCH_MON:
	case AFE_ADDA_2ND_DL_SDM_OUT_MON:
	case AFE_STF_MON:
	...
	case AFE_ADDA_DMIC0_SRC_CON0_MASK_MON:
	case AFE_ADDA_DMIC1_SRC_CON0_MASK_MON:
...
static const struct regmap_config mt6897_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6897_is_volatile_reg,
...
static irqreturn_t mt6897_afe_irq_handler(int irq_id, void *dev)
{
...
static int mt6897_afe_runtime_suspend(struct device *dev)
{
...
skip_regmap:
	mt6897_afe_sram_release(afe);
	mt6897_afe_disable_clock(afe);
	return 0;
}

static int mt6897_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	int ret;
	...
	ret = mt6897_afe_enable_clock(afe);
	if (ret)
		return ret;
	mt6897_afe_sram_request(afe);
...
static int mt6897_afe_pcm_copy(struct snd_pcm_substream *substream,
...
static int mt6897_set_memif_sram_mode(struct device *dev,
...
static int mt6897_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6897_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6897_sram_ops = {
	.set_sram_mode = mt6897_set_sram_mode,
};
				    ...
				    char *buf, loff_t offset, size_t size)
{
	...
	read_size = mt6897_debug_read_reg(buffer, AFE_SYS_DEBUG_SIZE, afe);
...
static int mt6897_afe_component_probe(struct snd_soc_component *component)
{
	...
	if (component) {
		...
		mt6897_add_misc_control(component);
...
static const struct snd_soc_component_driver mt6897_afe_component = {
	.name = AFE_PCM_NAME,
	.probe = mt6897_afe_component_probe,
...
static ssize_t mt6897_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe)
{
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_PCM1_INTF_CON0 = 0x%x\n",
		       AFE_PCM1_INTF_CON0, value);
	regmap_read(afe->regmap, AFE_PCM1_INTF_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_PCM1_INTF_CON1 = 0x%x\n",
		       AFE_PCM1_INTF_CON1, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_IRQ_MCU_CLR = 0x%x\n",
		       AFE_IRQ_MCU_CLR, value);
	regmap_read(afe->regmap, AFE_IRQ_MCU_MISS_CLR, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_IRQ_MCU_MISS_CLR = 0x%x\n",
		       AFE_IRQ_MCU_MISS_CLR, value);
	regmap_read(afe->regmap, AFE_CUSTOM_IRQ_MCU_CLR, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_CUSTOM_IRQ_MCU_CLR = 0x%x\n",
		       AFE_CUSTOM_IRQ_MCU_CLR, value);
	regmap_read(afe->regmap, AFE_CUSTOM_IRQ_MCU_MISS_CLR, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_CUSTOM_IRQ_MCU_MISS_CLR = 0x%x\n",
		       AFE_CUSTOM_IRQ_MCU_MISS_CLR, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_CUSTOM_IRQ_MCU_CON_CFG0 = 0x%x\n",
		       AFE_CUSTOM_IRQ_MCU_CON_CFG0, value);
	regmap_read(afe->regmap, AFE_CUSTOM_IRQ0_MCU_CNT, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_CUSTOM_IRQ0_MCU_CNT = 0x%x\n",
		       AFE_CUSTOM_IRQ0_MCU_CNT, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_CUSTOM_IRQ0_CNT_MON = 0x%x\n",
		       AFE_CUSTOM_IRQ0_CNT_MON, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_R_CFG0 = 0x%x\n",
		       AFE_DL_NLE_R_CFG0, value);
	regmap_read(afe->regmap, AFE_DL_NLE_R_CFG1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_R_CFG1 = 0x%x\n",
		       AFE_DL_NLE_R_CFG1, value);
	regmap_read(afe->regmap, AFE_DL_NLE_L_CFG0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_L_CFG0 = 0x%x\n",
		       AFE_DL_NLE_L_CFG0, value);
	regmap_read(afe->regmap, AFE_DL_NLE_L_CFG1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_L_CFG1 = 0x%x\n",
		       AFE_DL_NLE_L_CFG1, value);
	regmap_read(afe->regmap, AFE_DL_NLE_R_MON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_R_MON0 = 0x%x\n",
		       AFE_DL_NLE_R_MON0, value);
	regmap_read(afe->regmap, AFE_DL_NLE_R_MON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_R_MON1 = 0x%x\n",
		       AFE_DL_NLE_R_MON1, value);
	regmap_read(afe->regmap, AFE_DL_NLE_R_MON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_R_MON2 = 0x%x\n",
		       AFE_DL_NLE_R_MON2, value);
	regmap_read(afe->regmap, AFE_DL_NLE_L_MON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_L_MON0 = 0x%x\n",
		       AFE_DL_NLE_L_MON0, value);
	regmap_read(afe->regmap, AFE_DL_NLE_L_MON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_L_MON1 = 0x%x\n",
		       AFE_DL_NLE_L_MON1, value);
	regmap_read(afe->regmap, AFE_DL_NLE_L_MON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_L_MON2 = 0x%x\n",
		       AFE_DL_NLE_L_MON2, value);
	regmap_read(afe->regmap, AFE_DL_NLE_GAIN_CFG0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_NLE_GAIN_CFG0 = 0x%x\n",
		       AFE_DL_NLE_GAIN_CFG0, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SRC_CON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SRC_CON0 = 0x%x\n",
		       AFE_ADDA_2ND_DL_SRC_CON0, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SRC_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SRC_CON1 = 0x%x\n",
		       AFE_ADDA_2ND_DL_SRC_CON1, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SRC_DEBUG_MON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SRC_DEBUG_MON0 = 0x%x\n",
		       AFE_ADDA_2ND_DL_SRC_DEBUG_MON0, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_PREDIS_CON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_PREDIS_CON0 = 0x%x\n",
		       AFE_ADDA_2ND_DL_PREDIS_CON0, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_PREDIS_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_PREDIS_CON1 = 0x%x\n",
		       AFE_ADDA_2ND_DL_PREDIS_CON1, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_PREDIS_CON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_PREDIS_CON2 = 0x%x\n",
		       AFE_ADDA_2ND_DL_PREDIS_CON2, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_PREDIS_CON3, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_PREDIS_CON3 = 0x%x\n",
		       AFE_ADDA_2ND_DL_PREDIS_CON3, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SDM_DCCOMP_CON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SDM_DCCOMP_CON = 0x%x\n",
		       AFE_ADDA_2ND_DL_SDM_DCCOMP_CON, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SDM_TEST, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SDM_TEST = 0x%x\n",
		       AFE_ADDA_2ND_DL_SDM_TEST, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_DC_COMP_CFG0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_DC_COMP_CFG0 = 0x%x\n",
		       AFE_ADDA_2ND_DL_DC_COMP_CFG0, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_DC_COMP_CFG1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_DC_COMP_CFG1 = 0x%x\n",
		       AFE_ADDA_2ND_DL_DC_COMP_CFG1, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SRC_DEBUG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SRC_DEBUG = 0x%x\n",
		       AFE_ADDA_2ND_DL_SRC_DEBUG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SRC_LCH_MON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SRC_LCH_MON = 0x%x\n",
		       AFE_ADDA_2ND_DL_SRC_LCH_MON, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SRC_RCH_MON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SRC_RCH_MON = 0x%x\n",
		       AFE_ADDA_2ND_DL_SRC_RCH_MON, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SDM_OUT_MON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SDM_OUT_MON = 0x%x\n",
		       AFE_ADDA_2ND_DL_SDM_OUT_MON, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SDM_DITHER_CON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SDM_DITHER_CON = 0x%x\n",
		       AFE_ADDA_2ND_DL_SDM_DITHER_CON, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_SDM_AUTO_RESET_CON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_SDM_AUTO_RESET_CON = 0x%x\n",
		       AFE_ADDA_2ND_DL_SDM_AUTO_RESET_CON, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP1_TAP2_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP1_TAP2_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP1_TAP2_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP3_TAP4_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP3_TAP4_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP3_TAP4_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP5_TAP6_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP5_TAP6_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP5_TAP6_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP7_TAP8_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP7_TAP8_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP7_TAP8_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP9_TAP10_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP9_TAP10_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP9_TAP10_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP11_TAP12_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP11_TAP12_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP11_TAP12_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP13_TAP14_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP13_TAP14_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP13_TAP14_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP15_TAP16_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP15_TAP16_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP15_TAP16_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP17_TAP18_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP17_TAP18_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP17_TAP18_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP19_TAP20_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP19_TAP20_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP19_TAP20_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP21_TAP22_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP21_TAP22_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP21_TAP22_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP23_TAP24_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP23_TAP24_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP23_TAP24_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP25_TAP26_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP25_TAP26_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP25_TAP26_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP27_TAP28_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP27_TAP28_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP27_TAP28_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP29_TAP30_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP29_TAP30_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP29_TAP30_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP31_TAP32_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP31_TAP32_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP31_TAP32_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP33_TAP34_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP33_TAP34_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP33_TAP34_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP35_TAP36_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP35_TAP36_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP35_TAP36_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP37_TAP38_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP37_TAP38_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP37_TAP38_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP39_TAP40_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP39_TAP40_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP39_TAP40_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP41_TAP42_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP41_TAP42_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP41_TAP42_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP43_TAP44_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP43_TAP44_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP43_TAP44_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP45_TAP46_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP45_TAP46_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP45_TAP46_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP47_TAP48_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP47_TAP48_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP47_TAP48_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP49_TAP50_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP49_TAP50_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP49_TAP50_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP51_TAP52_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP51_TAP52_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP51_TAP52_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP53_TAP54_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP53_TAP54_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP53_TAP54_CONFIG, value);
	regmap_read(afe->regmap, AFE_ADDA_2ND_DL_HBF1_SCF1_TAP55_TAP56_CONFIG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_2ND_DL_HBF1_SCF1_TAP55_TAP56_CONFIG = 0x%x\n",
		       AFE_ADDA_2ND_DL_HBF1_SCF1_TAP55_TAP56_CONFIG, value);
	regmap_read(afe->regmap, AFE_STF_CON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_STF_CON0 = 0x%x\n",
		       AFE_STF_CON0, value);
	regmap_read(afe->regmap, AFE_STF_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_STF_CON1 = 0x%x\n",
		       AFE_STF_CON1, value);
	regmap_read(afe->regmap, AFE_STF_COEFF, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_STF_COEFF = 0x%x\n",
		       AFE_STF_COEFF, value);
	regmap_read(afe->regmap, AFE_STF_GAIN, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_STF_GAIN = 0x%x\n",
		       AFE_STF_GAIN, value);
	regmap_read(afe->regmap, AFE_STF_MON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_STF_MON = 0x%x\n",
		       AFE_STF_MON, value);
	regmap_read(afe->regmap, AFE_STF_IP_VERSION, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_STF_IP_VERSION = 0x%x\n",
		       AFE_STF_IP_VERSION, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_CON0 = 0x%x\n",
		       AFE_ADDA_UL1_SRC_CON0, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_SRC_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_CON1 = 0x%x\n",
		       AFE_ADDA_UL1_SRC_CON1, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_SRC_CON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_CON2 = 0x%x\n",
		       AFE_ADDA_UL1_SRC_CON2, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_SRC_DEBUG, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_DEBUG = 0x%x\n",
		       AFE_ADDA_UL1_SRC_DEBUG, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_SRC_DEBUG_MON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_DEBUG_MON0 = 0x%x\n",
		       AFE_ADDA_UL1_SRC_DEBUG_MON0, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_SRC_MON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_MON0 = 0x%x\n",
		       AFE_ADDA_UL1_SRC_MON0, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_SRC_MON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_MON1 = 0x%x\n",
		       AFE_ADDA_UL1_SRC_MON1, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_IIR_COEF_02_01, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_IIR_COEF_02_01 = 0x%x\n",
		       AFE_ADDA_UL1_IIR_COEF_02_01, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_IIR_COEF_04_03, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_IIR_COEF_04_03 = 0x%x\n",
		       AFE_ADDA_UL1_IIR_COEF_04_03, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_IIR_COEF_06_05, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_IIR_COEF_06_05 = 0x%x\n",
		       AFE_ADDA_UL1_IIR_COEF_06_05, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_IIR_COEF_08_07, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_IIR_COEF_08_07 = 0x%x\n",
		       AFE_ADDA_UL1_IIR_COEF_08_07, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_IIR_COEF_10_09, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_IIR_COEF_10_09 = 0x%x\n",
		       AFE_ADDA_UL1_IIR_COEF_10_09, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_02_01, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_02_01 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_02_01, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_04_03, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_04_03 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_04_03, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_06_05, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_06_05 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_06_05, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_08_07, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_08_07 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_08_07, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_10_09, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_10_09 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_10_09, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_12_11, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_12_11 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_12_11, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_14_13, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_14_13 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_14_13, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_16_15, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_16_15 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_16_15, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_18_17, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_18_17 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_18_17, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_20_19, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_20_19 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_20_19, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_22_21, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_22_21 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_22_21, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_24_23, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_24_23 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_24_23, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_26_25, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_26_25 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_26_25, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_28_27, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_28_27 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_28_27, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_30_29, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_30_29 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_30_29, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_ULCF_CFG_32_31, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_ULCF_CFG_32_31 = 0x%x\n",
		       AFE_ADDA_UL1_ULCF_CFG_32_31, value);
	regmap_read(afe->regmap, AFE_ADDA_UL1_IP_VERSION, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_IP_VERSION = 0x%x\n",
		       AFE_ADDA_UL1_IP_VERSION, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_PROXIMITY_CON0 = 0x%x\n",
		       AFE_ADDA_PROXIMITY_CON0, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON0 = 0x%x\n",
		       ETDM_OUT2_CON0, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON1 = 0x%x\n",
		       ETDM_OUT2_CON1, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON2 = 0x%x\n",
		       ETDM_OUT2_CON2, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON3, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON3 = 0x%x\n",
		       ETDM_OUT2_CON3, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON4, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON4 = 0x%x\n",
		       ETDM_OUT2_CON4, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON5, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON5 = 0x%x\n",
		       ETDM_OUT2_CON5, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON6, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON6 = 0x%x\n",
		       ETDM_OUT2_CON6, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON7, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON7 = 0x%x\n",
		       ETDM_OUT2_CON7, value);
	regmap_read(afe->regmap, ETDM_OUT2_CON8, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT2_CON8 = 0x%x\n",
		       ETDM_OUT2_CON8, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON0 = 0x%x\n",
		       ETDM_OUT6_CON0, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON1 = 0x%x\n",
		       ETDM_OUT6_CON1, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON2 = 0x%x\n",
		       ETDM_OUT6_CON2, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON3, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON3 = 0x%x\n",
		       ETDM_OUT6_CON3, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON4, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON4 = 0x%x\n",
		       ETDM_OUT6_CON4, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON5, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON5 = 0x%x\n",
		       ETDM_OUT6_CON5, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON6, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON6 = 0x%x\n",
		       ETDM_OUT6_CON6, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON7, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON7 = 0x%x\n",
		       ETDM_OUT6_CON7, value);
	regmap_read(afe->regmap, ETDM_OUT6_CON8, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] ETDM_OUT6_CON8 = 0x%x\n",
		       ETDM_OUT6_CON8, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_CONN_16BIT_4 = 0x%x\n",
		       AFE_CONN_16BIT_4, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_CONN_24BIT_4 = 0x%x\n",
		       AFE_CONN_24BIT_4, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_DL_24CH_CON1 = 0x%x\n",
		       AFE_DL_24CH_CON1, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN2_BASE_MSB = 0x%x\n",
		       AFE_ETDM_IN2_BASE_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN2_BASE, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN2_BASE = 0x%x\n",
		       AFE_ETDM_IN2_BASE, value);
	regmap_read(afe->regmap, AFE_ETDM_IN2_CUR_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN2_CUR_MSB = 0x%x\n",
		       AFE_ETDM_IN2_CUR_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN2_CUR, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN2_CUR = 0x%x\n",
		       AFE_ETDM_IN2_CUR, value);
	regmap_read(afe->regmap, AFE_ETDM_IN2_END_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN2_END_MSB = 0x%x\n",
		       AFE_ETDM_IN2_END_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN2_END, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN2_END = 0x%x\n",
		       AFE_ETDM_IN2_END, value);
	regmap_read(afe->regmap, AFE_ETDM_IN2_CON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN2_CON0 = 0x%x\n",
		       AFE_ETDM_IN2_CON0, value);
	regmap_read(afe->regmap, AFE_ETDM_IN4_BASE_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN4_BASE_MSB = 0x%x\n",
		       AFE_ETDM_IN4_BASE_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN4_BASE, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN4_BASE = 0x%x\n",
		       AFE_ETDM_IN4_BASE, value);
	regmap_read(afe->regmap, AFE_ETDM_IN4_CUR_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN4_CUR_MSB = 0x%x\n",
		       AFE_ETDM_IN4_CUR_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN4_CUR, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN4_CUR = 0x%x\n",
		       AFE_ETDM_IN4_CUR, value);
	regmap_read(afe->regmap, AFE_ETDM_IN4_END_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN4_END_MSB = 0x%x\n",
		       AFE_ETDM_IN4_END_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN4_END, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN4_END = 0x%x\n",
		       AFE_ETDM_IN4_END, value);
	regmap_read(afe->regmap, AFE_ETDM_IN4_CON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN4_CON0 = 0x%x\n",
		       AFE_ETDM_IN4_CON0, value);
	regmap_read(afe->regmap, AFE_ETDM_IN6_BASE_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN6_BASE_MSB = 0x%x\n",
		       AFE_ETDM_IN6_BASE_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN6_BASE, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN6_BASE = 0x%x\n",
		       AFE_ETDM_IN6_BASE, value);
	regmap_read(afe->regmap, AFE_ETDM_IN6_CUR_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN6_CUR_MSB = 0x%x\n",
		       AFE_ETDM_IN6_CUR_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN6_CUR, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN6_CUR = 0x%x\n",
		       AFE_ETDM_IN6_CUR, value);
	regmap_read(afe->regmap, AFE_ETDM_IN6_END_MSB, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN6_END_MSB = 0x%x\n",
		       AFE_ETDM_IN6_END_MSB, value);
	regmap_read(afe->regmap, AFE_ETDM_IN6_END, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN6_END = 0x%x\n",
		       AFE_ETDM_IN6_END, value);
	regmap_read(afe->regmap, AFE_ETDM_IN6_CON0, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ETDM_IN6_CON0 = 0x%x\n",
		       AFE_ETDM_IN6_CON0, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_PCM0_INTF_CON0_MASK_MON = 0x%x\n",
		       AFE_PCM0_INTF_CON0_MASK_MON, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_TDM_CON2_MASK_MON = 0x%x\n",
		       AFE_TDM_CON2_MASK_MON, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_UL1_SRC_CON0_MASK_MON = 0x%x\n",
		       AFE_ADDA_UL1_SRC_CON0_MASK_MON, value);
	regmap_read(afe->regmap, AFE_ADDA_DMIC0_SRC_CON0_MASK_MON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_DMIC0_SRC_CON0_MASK_MON = 0x%x\n",
		       AFE_ADDA_DMIC0_SRC_CON0_MASK_MON, value);
	regmap_read(afe->regmap, AFE_ADDA_DMIC1_SRC_CON0_MASK_MON, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_ADDA_DMIC1_SRC_CON0_MASK_MON = 0x%x\n",
		       AFE_ADDA_DMIC1_SRC_CON0_MASK_MON, value);
	...
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON0 = 0x%x\n",
		       AFE_GASRC3_NEW_CON0, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON1, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON1 = 0x%x\n",
		       AFE_GASRC3_NEW_CON1, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON2, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON2 = 0x%x\n",
		       AFE_GASRC3_NEW_CON2, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON3, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON3 = 0x%x\n",
		       AFE_GASRC3_NEW_CON3, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON4, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON4 = 0x%x\n",
		       AFE_GASRC3_NEW_CON4, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON5, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON5 = 0x%x\n",
		       AFE_GASRC3_NEW_CON5, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON6, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON6 = 0x%x\n",
		       AFE_GASRC3_NEW_CON6, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON7, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON7 = 0x%x\n",
		       AFE_GASRC3_NEW_CON7, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON8, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON8 = 0x%x\n",
		       AFE_GASRC3_NEW_CON8, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON9, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON9 = 0x%x\n",
		       AFE_GASRC3_NEW_CON9, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON10, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON10 = 0x%x\n",
		       AFE_GASRC3_NEW_CON10, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON11, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON11 = 0x%x\n",
		       AFE_GASRC3_NEW_CON11, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON12, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON12 = 0x%x\n",
		       AFE_GASRC3_NEW_CON12, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON13, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON13 = 0x%x\n",
		       AFE_GASRC3_NEW_CON13, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_CON14, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_CON14 = 0x%x\n",
		       AFE_GASRC3_NEW_CON14, value);
	regmap_read(afe->regmap, AFE_GASRC3_NEW_IP_VERSION, &value);
	n += scnprintf(buffer + n, size - n,
		       "[0x%x] AFE_GASRC3_NEW_IP_VERSION = 0x%x\n",
		       AFE_GASRC3_NEW_IP_VERSION, value);
	return n;
}
				   ...
				   size_t count, loff_t *pos)
{
	...
	n = mt6897_debug_read_reg(buffer, size, afe);
...
static const struct mtk_afe_debug_cmd mt6897_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6897_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6897_debugfs_read,
};
#endif

static int mt6897_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6897_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6897_memif_dai_driver);

	dai->controls = mt6897_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6897_pcm_kcontrols);
	dai->dapm_widgets = mt6897_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6897_memif_widgets);
	dai->dapm_routes = mt6897_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6897_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6897_dai_adda_register,
	mt6897_dai_i2s_register,
	mt6897_dai_hw_gain_register,
	mt6897_dai_src_register,
	mt6897_dai_pcm_register,
	mt6897_dai_tdm_register,
	mt6897_dai_hostless_register,
	mt6897_dai_memif_register,
};

static int mt6897_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
	...
	struct mt6897_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	regmap_read(afe->regmap, AUDIO_TOP_CON0, &tmp_reg);
	dev_info(dev, "%s(), before write AUDIO_TOP_CON0 = 0x%x\n", __func__, tmp_reg);
	regmap_write(afe->regmap, AUDIO_TOP_CON0, 0xc08043);
	regmap_read(afe->regmap, AUDIO_TOP_CON0, &tmp_reg);
	dev_info(dev, "%s(), after write AUDIO_TOP_CON0 = 0x%x\n", __func__, tmp_reg);

	regmap_read(afe->regmap, AUDIO_TOP_CON1, &tmp_reg);
	dev_info(dev, "%s(), before write AUDIO_TOP_CON1 = 0x%x\n", __func__, tmp_reg);
	regmap_write(afe->regmap, AUDIO_TOP_CON1, 0x0);
	regmap_read(afe->regmap, AUDIO_TOP_CON1, &tmp_reg);
	dev_info(dev, "%s(), after write AUDIO_TOP_CON1 = 0x%x\n", __func__, tmp_reg);

	regmap_read(afe->regmap, AUDIO_TOP_CON2, &tmp_reg);
	dev_info(dev, "%s(), before write AUDIO_TOP_CON2 = 0x%x\n", __func__, tmp_reg);
	regmap_write(afe->regmap, AUDIO_TOP_CON2, 0x142b3);
	regmap_read(afe->regmap, AUDIO_TOP_CON2, &tmp_reg);
	dev_info(dev, "%s(), after write AUDIO_TOP_CON2 = 0x%x\n", __func__, tmp_reg);
	/* IPM2.0 clock flow, need debug */
	regmap_read(afe->regmap, AUDIO_TOP_CON3, &tmp_reg);
	dev_info(dev, "%s(), before write AUDIO_TOP_CON3 = 0x%x\n", __func__, tmp_reg);
	regmap_write(afe->regmap, AUDIO_TOP_CON3, 0x0);
	regmap_read(afe->regmap, AUDIO_TOP_CON3, &tmp_reg);
	dev_info(dev, "%s(), after write AUDIO_TOP_CON3 = 0x%x\n", __func__, tmp_reg);
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6897_sram_ops);
	if (ret)
		return ret;
	...
	afe->memif_size = MT6897_MEMIF_NUM;
	...
	ret = devm_request_irq(dev, irq_id, mt6897_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6897_afe_hardware;
	afe->memif_fs = mt6897_memif_fs;
	afe->irq_fs = mt6897_irq_fs;
	afe->get_dai_fs = mt6897_get_dai_fs;
	afe->get_memif_pbuf_size = mt6897_get_memif_pbuf_size;

	afe->runtime_resume = mt6897_afe_runtime_resume;
	afe->runtime_suspend = mt6897_afe_runtime_suspend;

	afe->request_dram_resource = mt6897_afe_dram_request;
	afe->release_dram_resource = mt6897_afe_dram_release;

	afe->copy = mt6897_afe_pcm_copy;
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* debugfs */
	afe->debug_cmds = mt6897_debug_cmds;
	afe->debugfs = debugfs_create_file("mtksocaudio",
					   S_IFREG | 0444, NULL,
					   afe, &mt6897_debugfs_ops);
#endif
	/* register component */
	ret = devm_snd_soc_register_component(&pdev->dev,
					      &mt6897_afe_component,
					      afe->dai_drivers,
					      afe->num_dai_drivers);
	if (ret) {
...
static int mt6897_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6897_afe_runtime_suspend(&pdev->dev);
...
static const struct of_device_id mt6897_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6897-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6897_afe_pcm_dt_match);

static const struct dev_pm_ops mt6897_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6897_afe_runtime_suspend,
			   mt6897_afe_runtime_resume, NULL)
};

static struct platform_driver mt6897_afe_pcm_driver = {
	.driver = {
		.name = "mt6897-audio",
		.of_match_table = mt6897_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		.pm = &mt6897_afe_pm_ops,
#endif
	},
	.probe = mt6897_afe_pcm_dev_probe,
	.remove = mt6897_afe_pcm_dev_remove,
};

module_platform_driver(mt6897_afe_pcm_driver);

MODULE_DESCRIPTION("Mediatek ALSA SoC AFE platform driver for 6897");
